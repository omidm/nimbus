...
def decompose_time(e1, e2, e3, e4, e5):
        comp_cores = min(cores, e1+e2)
	sched_block_cores = min(cores-comp_cores, e5)
	io_block_cores = min(cores-comp_cores-sched_block_cores, e4)
	comp_block_cores = min(cores-comp_cores-sched_block_cores-io_block_cores, e3)
	empty_cores = cores-comp_cores-sched_block_cores-io_block_cores-comp_block_cores
	return comp_cores, sched_block_cores, io_block_cores, comp_block_cores, empty_cores
