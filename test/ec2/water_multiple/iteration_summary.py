...
print '--------------------------------------------------------------------------------------'
print '   worker    steps                        run      block     idle    synch     total'
print '--------------------------------------------------------------------------------------'
...
for rank in range(1, N+1):
  ...
  for line in f:
      ...
      if (args.collapse):
        print '          {:8.0f}                   {:8.2f}   {:8.2f} {:8.2f} {:8.2f}  {:8.2f}'.format(idx, run/CN, block/CN, idle/CN, (block+idle)/CN, (idle+block+run)/CN)
      if idx <= TI:
        continue
      iter_num  += 1
      run_sum   += run
  ...
  print '{:8.0f}: {:8.0f}                   {:8.2f}   {:8.2f} {:8.2f} {:8.2f}  {:8.2f}'.format(
            rank, iter_num, run_sum/CN/iter_num, block_sum/CN/iter_num, idle_sum/CN/iter_num, (block_sum+idle_sum)/CN/iter_num, total_sum/CN/iter_num)

print '--------------------------------------------------------------------------------------'
print ' Average: {:8.0f}                   {:8.2f}   {:8.2f} {:8.2f} {:8.2f}  {:8.2f}'.format(iter_nums[0], total_run_sum/N/CN, total_block_sum/N/CN, total_idle_sum/N/CN, (total_block_sum+total_idle_sum)/N/CN, total_total_sum/N/CN)
print '--------------------------------------------------------------------------------------'
