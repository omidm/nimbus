    ...
    VECTOR_ND<T>& x_unpermuted,const ARRAY<PAIR<bool,T> >& x_min,const ARRAY<PAIR<bool,T> >& x_max,const T tolerance,const T step_tolerance,const bool debug_optimization)
{
    ...
    for(;;iteration++){
        ...
        if(x_S.n){
            ...
            if(debug_optimization){
                std::stringstream ss;
                ss << "alpha="<<alpha<<", limiting_index="<<limiting_index<<", limiting_value="<<limiting_value<<std::endl;
                LOG::filecout(ss.str());}
#endif
        ...
        if(!limiting_index){assert(alpha==1); // check lagrange multipliers
...
#ifndef COMPILE_WITHOUT_READ_WRITE_SUPPORT
            if(!index_to_release){if(debug_optimization)LOG::filecout("QP: Nothing to release!\n");break;} // TODO: how handle this case?

            if(debug_optimization) {std::stringstream ss;ss << "Releasing index " << index_to_release << " (muscle " << permute_N(index_to_release) << ")" << std::endl;LOG::filecout(ss.str());}
#endif
            ...
            if(debug_optimization){
                std::stringstream ss;
                ss << "Constraining S index " << limiting_index << " (muscle " << permute_S(limiting_index) << ") to " << limiting_value << std::endl;
                LOG::filecout(ss.str());}
#endif
...
#ifndef COMPILE_WITHOUT_READ_WRITE_SUPPORT
                std::stringstream ss;
                ss << "Constraining B index " << limiting_index << " (muscle " << permute_B(limiting_index) << ") to " << limiting_value << std::endl;
                ss << "BEFORE COLUMN SWITCH\nB:\n" << B << "\nS:\n" << S << "\nN:\n" << N << std::endl;
                LOG::filecout(ss.str());
#endif
            }
            ...
            if(debug_optimization){
                std::stringstream ss;
                ss<<"AFTER COLUMN B->N AND REFACTORING B\nB:\n" << B << "\nS:\n" << S << "\nN:\n" << N << std::endl;
                LOG::filecout(ss.str());}
#endif
            ...
            if(!s_column){
#ifndef COMPILE_WITHOUT_READ_WRITE_SUPPORT
                LOG::filecout("Bottom row of B,S is all zero!\n");
#endif
                break;}
            ...
            if(debug_optimization){
                std::stringstream ss;
                ss << "AFTER MOVING S COLUMN ("<<s_column<<") TO B\nB:\n" << B << "\nS:\n" << S << "\nN:\n" << N << std::endl;
                LOG::filecout(ss.str());}
#endif
        }
    }
...
#ifndef COMPILE_WITHOUT_READ_WRITE_SUPPORT
    std::stringstream ss;
    if(debug_optimization) ss << "Result x_B:\n"<<x_B<<"\nResult x_S:\n"<<x_S<<"\nResult permute:\n"<<permute<<"\nResult x:\n"<<x<<std::endl;
    ss << "QP finished in " << iteration << " iterations" << std::endl;
    LOG::filecout(ss.str());
#endif
}
//####################################################################################
