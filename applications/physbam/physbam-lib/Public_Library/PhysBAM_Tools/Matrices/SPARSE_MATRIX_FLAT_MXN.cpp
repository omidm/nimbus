...
//#####################################################################
template<class T> void SPARSE_MATRIX_FLAT_MXN<T>::
Times_Diagonal_Times(const VECTOR_ND<T> diagonal,const SPARSE_MATRIX_FLAT_MXN<T>& rhs,SPARSE_MATRIX_FLAT_NXN<T>& result,bool enforce_diagonal_term)
{
    assert(rhs.m==n && diagonal.n==n && rhs.n==m);
    const int columns=rhs.n,rows=m;
    ARRAY<int> row_lengths(rows);
    ARRAY<int> column_indices;
    for(int row=1;row<=rows;row++){
        ...
        if(enforce_diagonal_term) column_indices.Append_Unique(row);
        row_lengths(row)=column_indices.m;
    }

    result.Set_Row_Lengths(row_lengths);
    ARRAYS_COMPUTATIONS::Fill(result.A,SPARSE_MATRIX_ENTRY<T>());
    result.n=columns;
    ...
    for(int row=1;row<=rows;row++){
        ...
        if(enforce_diagonal_term) result.Add_Element(row,row,0);
    }
}
//#####################################################################
// Function Transpose_Times_Diagonal_Times
//#####################################################################
template<class T> SPARSE_MATRIX_FLAT_NXN<T> SPARSE_MATRIX_FLAT_MXN<T>::
Transpose_Times_Diagonal_Times(const VECTOR_ND<T> diagonal)
{
    assert(diagonal.n==m);
    SPARSE_MATRIX_FLAT_NXN<T> result;
    ARRAY<HASHTABLE<int,T> > row_entries(n);
    for(int row=1;row<=m;row++){
        int start=offsets(row),end=offsets(row+1);
        for(int i=start;i<end;i++) for(int j=i;j<end;j++){
            T& value=row_entries(A(i).j).Get_Or_Insert(A(j).j,(T)0);
            value+=A(i).a*A(j).a*diagonal(row);
            if(i!=j){
                T& value1=row_entries(A(j).j).Get_Or_Insert(A(i).j,(T)0);
                value1+=A(i).a*A(j).a*diagonal(row);}}}
    result.Reset();
    ARRAY<int> keys;
    ARRAY<T> data;
    for(int row=1;row<=n;row++){
        row_entries(row).Get_Keys(keys);
        row_entries(row).Get_Data(data);
        for(int i=1;i<=keys.m;i++) result.Append_Entry_To_Current_Row(keys(i),data(i));
        result.Finish_Row();}
    result.Sort_Entries();
...
//#####################################################################
template<class T> void SPARSE_MATRIX_FLAT_MXN<T>::
Transpose_Times_Diagonal_Times(const VECTOR_ND<T> diagonal,SPARSE_MATRIX_FLAT_NXN<T>& result)
{
    assert(diagonal.n==m);
    ARRAY<HASHTABLE<int,T> > row_entries(n);
    for(int row=1;row<=m;row++){
...
//#####################################################################
// Function Concatenate_Rows
//#####################################################################
template<class T> SPARSE_MATRIX_FLAT_MXN<T> SPARSE_MATRIX_FLAT_MXN<T>::
Concatenate_Rows(const SPARSE_MATRIX_FLAT_MXN<T>& B) const
{
    assert(n==B.n);
    SPARSE_MATRIX_FLAT_MXN<T> result;result.n=n;
    int rows=m+B.m;
    ARRAY<int> row_lengths(rows);
    for(int row=1;row<=rows;row++){
        if(row<=m) row_lengths(row)=offsets(row+1)-offsets(row);
        else row_lengths(row)=B.offsets(row+1-m)-B.offsets(row-m);}
    result.Set_Row_Lengths(row_lengths);
    for(int row=1;row<=rows;row++){
        if(row<=m){
            int start=offsets(row),end=offsets(row+1);
            for(int index=start;index<end;index++)
                result.Set_Element(row,A(index).j,A(index).a);}
        else{
            int start=B.offsets(row-m),end=B.offsets(row+1-m);
            for(int index=start;index<end;index++)
                result.Set_Element(row,B.A(index).j,B.A(index).a);}}
    return result;
}
//#####################################################################
// Function Concatenate_Rows
//#####################################################################
template<class T> SPARSE_MATRIX_FLAT_MXN<T> SPARSE_MATRIX_FLAT_MXN<T>::
Concatenate_Rows(const SPARSE_MATRIX_FLAT_MXN<T>& B,const INTERVAL<int>& range) const
{
    assert(n==B.n);
    assert(range.min_corner>=1 && range.max_corner<=B.m);
    SPARSE_MATRIX_FLAT_MXN<T> result;result.n=n;
    int rows=m+range.Size()+1,offset=range.min_corner-1;
    ARRAY<int> row_lengths(rows);
    for(int row=1;row<=rows;row++){
        if(row<=m) row_lengths(row)=offsets(row+1)-offsets(row);
        else row_lengths(row)=B.offsets(row+1-m+offset)-B.offsets(row-m+offset);}
    result.Set_Row_Lengths(row_lengths);
    for(int row=1;row<=rows;row++){
        if(row<=m){
            int start=offsets(row),end=offsets(row+1);
            ...
            int start=B.offsets(row-m+offset),end=B.offsets(row+1-m+offset);
            for(int index=start;index<end;index++)
                result.Set_Element(row,B.A(index).j,B.A(index).a);}}
    return result;
}
//#####################################################################
// Function Shift_Columns
//#####################################################################
template<class T> SPARSE_MATRIX_FLAT_MXN<T> SPARSE_MATRIX_FLAT_MXN<T>::
Shift_Columns(const int offset,const int total_column) const
{
    SPARSE_MATRIX_FLAT_MXN<T> result=(*this);result.n=total_column;
    for(int i=1;i<=result.A.m;i++){
        result.A(i).j+=offset;
        assert(result.A(i).j>0 && result.A(i).j<=total_column);}
    return result;
}
//#####################################################################
// Function Scale_Rows
...
//#####################################################################
template<class T> SPARSE_MATRIX_FLAT_MXN<T> SPARSE_MATRIX_FLAT_MXN<T>::
Scale_Columns(const VECTOR_ND<T>& d) const
{
    SPARSE_MATRIX_FLAT_MXN<T> result=*this;
    for(int i=1;i<=result.m;i++)
        for(int j=result.offsets(i);j<result.offsets(i+1);j++)
            result.A(j).a*=d(result.A(j).j);
    return result;
}
//#####################################################################
// Function operator+
...
{
    SPARSE_MATRIX_FLAT_MXN result=*this;
    for(int i=1;i<=result.A.Size();i++)
      result.A(i).a*=a;
    return result;
}
//#####################################################################
...
Write_Row_Lengths()
{
    ...
    std::stringstream ss;
    for(int i=1;i<=m;i++) ss<<offsets(i+1)-offsets(i)<<" ";
    ss<<std::endl;
    LOG::filecout(ss.str());
#endif
}
...
//#####################################################################
// Function Concatenate_Matrices
//#####################################################################
template<class T> SPARSE_MATRIX_FLAT_MXN<T> SPARSE_MATRIX_FLAT_MXN<T>::
Concatenate_Matrices(ARRAY<SPARSE_MATRIX_FLAT_MXN<T> >& matrices,bool block_diagonal)
{
    SPARSE_MATRIX_FLAT_MXN<T> result;
    result.m=0;
    int size=0;
    for(int i=1;i<=matrices.Size();i++){result.m+=matrices(i).m;size+=matrices(i).A.Size();}
    result.offsets.Resize(result.m+1);
    result.offsets(result.m+1)=size+1;
    result.A.Resize(size);
    int c=0,d=0,e=0;
    for(int i=1;i<=matrices.Size();i++)
    {
        //LOG::cout << "offsets " << i << " - " << matrices(i).offsets << std::endl;
        for(int j=1;j<=matrices(i).m;j++) result.offsets(c+j)=d+matrices(i).offsets(j);
        for(int j=1;j<=matrices(i).A.Size();j++) result.A(d+j)=matrices(i).A(j);
        if(block_diagonal)
            for(int j=1;j<=matrices(i).A.Size();j++) result.A(d+j).j+=c;
        c+=matrices(i).m;
        d+=matrices(i).A.Size();
        e+=matrices(i).n;
    }
    //LOG::cout << "result offsets " << result.offsets << std::endl;
    if(block_diagonal) result.n=e;
    else result.n=matrices(1).n;
    return result;
}
//#####################################################################
// Function Normalize_Row_Sum
//#####################################################################
template<class T> void SPARSE_MATRIX_FLAT_MXN<T>::
Normalize_Row_Sum()
{
    for(int i=1;i<=m;i++){T sum=0;
        for(int j=offsets(i);j<=offsets(i+1)-1;j++) sum+=A(j).a;
        if(sum!=0){T one_over_sum=Inverse(sum);
            for(int j=offsets(i);j<=offsets(i+1)-1;j++) A(j).a*=one_over_sum;}}
}
//#####################################################################
