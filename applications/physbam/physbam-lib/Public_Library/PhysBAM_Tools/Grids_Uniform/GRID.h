...
public:
    ...
    TV_INT Cell(const TV& location) const
    {return TV_INT(floor((location-domain.min_corner)*one_over_dX+(T)1));}
    ...
    {TV expand(dX*(typename TV::SCALAR)number_of_ghost_cells);return RANGE<TV>(domain.min_corner-expand,domain.max_corner+expand);}
