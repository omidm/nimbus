...
public:
    ...
    {std::stringstream ss;ss<<"Test "<<Identify_Test(n)<<(result==success?" PASSED.":result==failure?" FAILED.":" INCONCLUSIVE.")<<std::endl;LOG::filecout(ss.str());}

    virtual void Report_Out_Of_Range(int n) const
    {std::stringstream ss;ss<<"Test number "<<n<<" out of range.  Test suite "<<name<<" has "<<Number_Of_Tests()<<" tests."<<std::endl;LOG::filecout(ss.str());}

    static void Report_Registry_Failure(std::string suite_name)
    {std::stringstream ss;ss<<"Test suite "<<suite_name<<" not registered."<<std::endl;LOG::filecout(ss.str());}
