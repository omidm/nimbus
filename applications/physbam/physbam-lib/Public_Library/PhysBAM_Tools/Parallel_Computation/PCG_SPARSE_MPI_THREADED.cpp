...
Solve(RANGE<TV_INT>& domain,const ARRAY<int,TV_INT>& domain_index,const ARRAY<INTERVAL<int> >& all_interior_indices,const ARRAY<ARRAY<INTERVAL<int> > >& all_ghost_indices,SPARSE_MATRIX_FLAT_NXN<T>& A,VECTOR_ND<T>& x,VECTOR_ND<T>& b,const T tolerance)
{
    ...
    if(pcg.enforce_compatibility && pcg.remove_null_space_solution_component) x_interior-=(T)(Global_Sum((T)x_interior.Sum_Double_Precision(),tid)/global_n);
    ...
    if(pcg.enforce_compatibility) b_interior-=(T)(Global_Sum((T)b_interior.Sum_Double_Precision(),tid)/global_n);
    ...
    for(int iteration=1;;iteration++){
        ...
        if(pcg.enforce_compatibility) z_interior-=(T)(Global_Sum((T)z_interior.Sum_Double_Precision(),tid)/global_n);
