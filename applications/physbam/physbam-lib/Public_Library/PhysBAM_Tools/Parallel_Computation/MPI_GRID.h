...
//#####################################################################
    ...
    void Distribute_Weights(const ARRAY<TRIPLE<PAIR<TV_INT,int>,PAIR<TV_INT,int>,T> >& weights_sender,ARRAY<TRIPLE<PAIR<TV_INT,int>,PAIR<TV_INT,int>,T> >& weights_received);
    void Distribute_Weights(const ARRAY<PAIR<PAIR<TV_INT,int>,T> >& weights_sender,ARRAY<PAIR<PAIR<TV_INT,int>,T> >& weights_received);
    ...
    template<class T_MPI_GRID,class T_NODE_ARRAYS> void Exchange_Boundary_Node_Data(const T_MPI_GRID& mpi_grid,T_NODE_ARRAYS& data,const int bandwidth) const;
    ...
    template<class T_MPI_GRID,class T_FACE_ARRAYS> void Sum_Common_Face_Data(const T_MPI_GRID& mpi_grid,T_FACE_ARRAYS& data) const;
    ...
    template<class T2> T2 Reduce_Max(const T2& local_value) const;
