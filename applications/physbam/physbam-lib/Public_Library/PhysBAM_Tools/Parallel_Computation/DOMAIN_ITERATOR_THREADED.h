...
public:
        ...
        if(processes_per_dimension!=VECTOR<int,3>()) count=processes_per_dimension;
            ...
            for(test_count.z=1;test_count.z<=number_of_domains;test_count.z++) if(number_of_domains%test_count.z==0){
                if(Minimize_2D_Surface_Area(number_of_domains/test_count.z,x,y,test_count.x)){
                    ...
                    T surface_area=(T)(test_count.x*(y*z)+test_count.y*(x*z)+test_count.z*(x*y));
                    if(surface_area<minimum_surface_area){count=test_count;minimum_surface_area=surface_area;}}
                if(Minimize_2D_Surface_Area(number_of_domains/test_count.z,y,x,test_count.y)){
    ...
    template<class T1,class T2> void Run(TYPE& my_class,void (TYPE::*func)(int,RANGE<TV_INT>&,T1,T2),T1 arg1,T2 arg2)
    {
        if(!thread_queue){(my_class.*func)(1,domains(1),arg1,arg2);return;}
#ifdef USE_PTHREADS
        for(int i=1;i<=domains.m;i++){
            ITERATOR_TASK_2<TYPE,RANGE<TV_INT>,T1,T2>* task=new ITERATOR_TASK_2<TYPE,RANGE<TV_INT>,T1,T2>(my_class,func,i,domains(i),arg1,arg2);
            thread_queue->Queue(task);}
        if(do_wait)thread_queue->Wait();
#else
        PHYSBAM_FATAL_ERROR();
    ...
    template<class T1,class T2,class T3,class T4,class T5> void Run(TYPE& my_class,void (TYPE::*func)(int,RANGE<TV_INT>&,T1,T2,T3,T4,T5),T1 arg1,T2 arg2,T3 arg3,T4 arg4,T5 arg5)
    {
        if(!thread_queue){(my_class.*func)(1,domains(1),arg1,arg2,arg3,arg4,arg5);return;}
#ifdef USE_PTHREADS
        for(int i=1;i<=domains.m;i++){
            ITERATOR_TASK_5<TYPE,RANGE<TV_INT>,T1,T2,T3,T4,T5>* task=new ITERATOR_TASK_5<TYPE,RANGE<TV_INT>,T1,T2,T3,T4,T5>(my_class,func,i,domains(i),arg1,arg2,arg3,arg4,arg5);
            thread_queue->Queue(task);}
        if(do_wait)thread_queue->Wait();
#else
        PHYSBAM_FATAL_ERROR();
