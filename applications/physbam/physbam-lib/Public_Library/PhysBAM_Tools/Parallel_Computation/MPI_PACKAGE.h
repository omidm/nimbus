...
public:
    ...
    template<class T>
    explicit MPI_PACKAGE(VECTOR_ND<T>& vector,const ARRAY<int>& indices)
        :data(vector.Get_Array_Pointer()-1),count(1),type_needs_free(true)
    {
        ARRAY<int> lengths(CONSTANT_ARRAY<int>(indices.m,1));
        type=MPI_UTILITIES::Datatype<T>().Create_indexed(indices.m,lengths.Get_Array_Pointer(),indices.Get_Array_Pointer());
        type.Commit();
    }
