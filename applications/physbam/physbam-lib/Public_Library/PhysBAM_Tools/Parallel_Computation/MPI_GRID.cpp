...
//#####################################################################
#else
template<class T_GRID> void MPI_GRID<T_GRID>::Initialize_Topology()
...
static bool Minimize_2D_Surface_Area(const int number_of_processes,const int m,const int n,int& count_x)
{
    ...
    else count_x=m>n?number_of_processes:1;
    return true;
}
...
{
    int ghost_cells_local=ghost_cells;
    MPI_UTILITIES::Reduce(ghost_cells_local,ghost_cells,MPI::MAX,*comm);
}
//#####################################################################
...
//#####################################################################
// Function Exchange_Boundary_Node_Data
//#####################################################################
template<class T_GRID> template<class T_MPI_GRID,class T_NODE_ARRAYS> void MPI_GRID<T_GRID>::
Exchange_Boundary_Node_Data(const T_MPI_GRID& mpi_grid,T_NODE_ARRAYS& data,const int bandwidth) const
{
    RANGE<VECTOR<int,1> > boundary_band(0,bandwidth-1),ghost_band(-bandwidth,-1);
    RANGE<TV_INT> sentinels(TV_INT(),TV_INT::Constant_Vector(1));
    ARRAY<MPI_PACKAGE> packages;ARRAY<MPI::Request> requests;
    // send
    ARRAY<RANGE<TV_INT> > send_regions;
    Find_Boundary_Regions(send_regions,sentinels,true,boundary_band,true);
    for(int n=1;n<=send_regions.m;n++)if(all_neighbor_ranks(n)!=MPI::PROC_NULL){
        RANGE<TV_INT> send_regions_n;send_regions_n=send_regions(n);
        MPI_PACKAGE package=mpi_grid.Package_Cell_Data(data,send_regions_n);
        packages.Append(package);requests.Append(package.Isend(*comm,all_neighbor_ranks(n),Get_Send_Tag(all_neighbor_directions(n))));}
    // receive
    ARRAY<RANGE<TV_INT> > recv_regions;
    Find_Boundary_Regions(recv_regions,sentinels,true,ghost_band,true);
    for(int n=1;n<=recv_regions.m;n++)if(all_neighbor_ranks(n)!=MPI::PROC_NULL){
        RANGE<TV_INT> recv_regions_n;recv_regions_n=recv_regions(n);
        MPI_PACKAGE package=mpi_grid.Package_Cell_Data(data,recv_regions_n);
        packages.Append(package);requests.Append(package.Irecv(*comm,all_neighbor_ranks(n),Get_Recv_Tag(all_neighbor_directions(n))));}
    // finish
    MPI_UTILITIES::Wait_All(requests);MPI_PACKAGE::Free_All(packages);
}
...
//#####################################################################
// Function Sum_Common_Face_Data
//#####################################################################
template<class T_GRID> template<class T_MPI_GRID,class T_FACE_ARRAYS> void MPI_GRID<T_GRID>::
Sum_Common_Face_Data(const T_MPI_GRID& mpi_grid,T_FACE_ARRAYS& data) const
{
    //PHYSBAM_NOT_IMPLEMENTED(); // TODO: we probably don't need this function, but I'll leave it here for now
    ...
    for(int n=1;n<=regions(1).m;n++)if(side_neighbor_ranks(n)!=MPI::PROC_NULL){
        ...
        ARRAY<T> local_buffer(buffers(n).m);
        int position=0;T_type.Unpack(pack_buffer.Get_Array_Pointer(),pack_buffer.m,local_buffer.Get_Array_Pointer(),local_buffer.m,position,*comm);
        ARRAY<T>::Copy((T)1,buffers(n),(T)1,local_buffer,local_buffer); // sum
        position=0;T_type.Pack(local_buffer.Get_Array_Pointer(),local_buffer.m,pack_buffer.Get_Array_Pointer(),pack_buffer.m,position,*comm);
        packages(n).Unpack(pack_buffer,*comm);}
    MPI_PACKAGE::Free_All(packages);
}
...
{
    int tag=Get_Unique_Tag();
    RANGE<VECTOR<int,1> > boundary_band(0,ghost_cells-1),ghost_band(-ghost_cells,-1);
    ...
    for(int axis=1;axis<=T_GRID::dimension;axis++)Find_Boundary_Regions(send_regions(axis),RANGE<TV_INT>(TV_INT(),TV_INT::Axis_Vector(axis)),true,boundary_band,true);
    for(int axis=1;axis<=T_GRID::dimension;axis++)Find_Boundary_Regions(recv_regions(axis),RANGE<TV_INT>(TV_INT(),TV_INT::Axis_Vector(axis)),true,ghost_band,true);
    for(FACE_ITERATOR iterator(local_grid);iterator.Valid();iterator.Next()){FACE_INDEX<TV::dimension> cell=iterator.Full_Index();
        ARRAY<PAIR<FACE_INDEX<TV::dimension>,T> >& local_weights=weights_to(cell);
            for(int i=1;i<=local_weights.m;i++)for(int n=1;n<=recv_regions(cell.axis).m;n++)if(all_neighbor_ranks(n)!=MPI::PROC_NULL){
                if(recv_regions(cell.axis)(n).Lazy_Inside(local_weights(i).x.index) && local_weights(i).x.axis){
                    transfer_per_proc(all_neighbor_ranks(n)+1).Append(TRIPLE<FACE_INDEX<TV::dimension>,FACE_INDEX<TV::dimension>,T>(FACE_INDEX<TV::dimension>(cell.axis,cell.index+local_to_global_offset),
    ...
    for(int i=1;i<=number_of_processes;i++){int position=0;
        for(int j=1;j<=receive_per_proc(i).m;j++){
            ...
            if(index) weights_to(data.x)(index).y=data.z;
                ...
                if(data.y.axis) weights_from(data.y).Append(PAIR<FACE_INDEX<TV::dimension>,int>(data.x,weights_to(data.x).m));}}}
...
{   
    ...
    for(FACE_ITERATOR iterator(local_grid);iterator.Valid();iterator.Next()){FACE_INDEX<TV::dimension> cell=iterator.Full_Index();
        ARRAY<PAIR<FACE_INDEX<TV::dimension>,int> >& local_weights=weights_from(cell);
            for(int i=1;i<=local_weights.m;i++)for(int n=1;n<=recv_regions(cell.axis).m;n++)if(all_neighbor_ranks(n)!=MPI::PROC_NULL){
                if(recv_regions(cell.axis)(n).Lazy_Inside(local_weights(i).x.index)){
                    transfer_per_proc(all_neighbor_ranks(n)+1).Append(TRIPLE<FACE_INDEX<TV::dimension>,FACE_INDEX<TV::dimension>,T>(FACE_INDEX<TV::dimension>(cell.axis,cell.index+local_to_global_offset),
...
Sync_Common_Cell_Weights_From(ARRAY<ARRAY<PAIR<TV_INT,T> >,TV_INT>& weights_to,ARRAY<ARRAY<PAIR<TV_INT,int> >,TV_INT>& weights_from,const int ghost_cells)
{
    ...
    ARRAY<RANGE<TV_INT> > send_regions;Find_Boundary_Regions(send_regions,sentinels,false,RANGE<VECTOR<int,1> >(0,ghost_cells-1),include_corners,true,local_grid);
    ARRAY<RANGE<TV_INT> > recv_regions;Find_Boundary_Regions(recv_regions,sentinels,false,RANGE<VECTOR<int,1> >(-ghost_cells,-1),include_corners,true,local_grid);
    for(CELL_ITERATOR iterator(local_grid);iterator.Valid();iterator.Next()){TV_INT cell=iterator.Cell_Index();        
        ARRAY<PAIR<TV_INT,T> >& local_weights=weights_to(cell);
            for(int i=1;i<=local_weights.m;i++)for(int n=1;n<=recv_regions.m;n++)if(neighbor_ranks(n)!=MPI::PROC_NULL){
                if(recv_regions(n).Lazy_Inside(local_weights(i).x)){
                    transfer_per_proc(neighbor_ranks(n)+1).Append(TRIPLE<TV_INT,TV_INT,T>(local_to_global_offset+cell,local_to_global_offset+local_weights(i).x,local_weights(i).y));}}}
    for(int i=1;i<=number_of_processes;i++) all_sizes(rank+1,i)=transfer_per_proc(i).m;
    for(int i=1;i<=number_of_processes;i++) for(int j=1;j<=number_of_processes;j++){int output;MPI_UTILITIES::Reduce(all_sizes(i,j),output,MPI::MAX,*comm);all_sizes(i,j)=output;}
    for(int i=1;i<=number_of_processes;i++) receive_per_proc(i).Resize(all_sizes(i,rank+1));
    ARRAY<MPI::Request> requests;
...
Sync_Common_Cell_Weights_To(ARRAY<ARRAY<PAIR<TV_INT,T> >,TV_INT>& weights_to,ARRAY<ARRAY<PAIR<TV_INT,int> >,TV_INT>& weights_from,const int ghost_cells)
{
    ...
    for(CELL_ITERATOR iterator(local_grid);iterator.Valid();iterator.Next()){TV_INT cell=iterator.Cell_Index();        
        ARRAY<PAIR<TV_INT,int> >& local_weights=weights_from(cell);
            for(int i=1;i<=local_weights.m;i++)for(int n=1;n<=recv_regions.m;n++)if(neighbor_ranks(n)!=MPI::PROC_NULL){
                if(recv_regions(n).Lazy_Inside(local_weights(i).x)){
                    transfer_per_proc(neighbor_ranks(n)+1).Append(TRIPLE<TV_INT,TV_INT,T>(local_to_global_offset+cell,local_to_global_offset+local_weights(i).x,weights_to(local_weights(i).x)(local_weights(i).y).y));}}}
    for(int i=1;i<=number_of_processes;i++) all_sizes(rank+1,i)=transfer_per_proc(i).m;
    for(int i=1;i<=number_of_processes;i++) for(int j=1;j<=number_of_processes;j++){int output;MPI_UTILITIES::Reduce(all_sizes(i,j),output,MPI::MAX,*comm);all_sizes(i,j)=output;}
    for(int i=1;i<=number_of_processes;i++) receive_per_proc(i).Resize(all_sizes(i,rank+1));
    ARRAY<MPI::Request> requests;
...
template<class T_GRID> void MPI_GRID<T_GRID>::
Distribute_Weights(const ARRAY<TRIPLE<PAIR<TV_INT,int>,PAIR<TV_INT,int>,T> >& weights_sender,
                   ARRAY<TRIPLE<PAIR<TV_INT,int>,PAIR<TV_INT,int>,T> >& weights_received)
{
    const ARRAY<int>& neighbor_ranks=all_neighbor_ranks;
    const ARRAY<TV_INT>& neighbor_directions=all_neighbor_directions;
    RANGE<TV_INT> sentinels=RANGE<TV_INT>::Zero_Box();
    ARRAY<RANGE<TV_INT> > send_regions;Find_Boundary_Regions(send_regions,sentinels,false,RANGE<VECTOR<int,1> >(-1,-1),
                                                             true,true,local_grid);

    RANGE<VECTOR<int,2> > range(VECTOR<int,2>(1,1),VECTOR<int,2>(number_of_processes,number_of_processes));
    ARRAY<int,VECTOR<int,2> > all_sizes(range);
    ARRAY<ARRAY<TRIPLE<PAIR<TV_INT,int>,PAIR<TV_INT,int>,T> > > transfer_per_proc(neighbor_ranks.Size()); 
    ARRAY<ARRAY<TRIPLE<PAIR<TV_INT,int>,PAIR<TV_INT,int>,T> > > receive_per_proc(neighbor_ranks.Size());
    for(int n=1;n<=neighbor_ranks.Size();++n) if(neighbor_ranks(n)!=MPI::PROC_NULL){
        for(int i=1;i<=weights_sender.Size();++i){
            if(send_regions(n).Lazy_Inside(weights_sender(i).x.x)){
                transfer_per_proc(n).Append(TRIPLE<PAIR<TV_INT,int>,PAIR<TV_INT,int>,T>(
                                PAIR<TV_INT,int>(local_to_global_offset+weights_sender(i).x.x,weights_sender(i).x.y),
                                PAIR<TV_INT,int>(local_to_global_offset+weights_sender(i).y.x,weights_sender(i).y.y),
                                weights_sender(i).z));}}}

    for(int n=1;n<=neighbor_ranks.Size();++n) if(neighbor_ranks(n)!=MPI::PROC_NULL)
        all_sizes(rank+1,neighbor_ranks(n)+1)=transfer_per_proc(n).m;
    for(int i=1;i<=number_of_processes;i++) for(int j=1;j<=number_of_processes;j++){int output;
        MPI_UTILITIES::Reduce(all_sizes(i,j),output,MPI::MAX,*comm);all_sizes(i,j)=output;}
    for(int n=1;n<=neighbor_ranks.Size();++n) if(neighbor_ranks(n)!=MPI::PROC_NULL)
        receive_per_proc(n).Resize(all_sizes(neighbor_ranks(n)+1,rank+1));

    ARRAY<MPI::Request> requests;
    ARRAY<ARRAY<char> > recv_buffers(neighbor_ranks.Size());ARRAY<ARRAY<char> > send_buffer(neighbor_ranks.Size());
    for(int n=1;n<=neighbor_ranks.Size();++n) if(neighbor_ranks(n)!=MPI::PROC_NULL && transfer_per_proc(n).Size()){
        int buffer_size=
            MPI_UTILITIES::Datatype<TV_INT>().Pack_size(2*transfer_per_proc(n).m,*comm)
           +MPI_UTILITIES::Datatype<int>()   .Pack_size(2*transfer_per_proc(n).m,*comm)
           +MPI_UTILITIES::Datatype<T>()     .Pack_size(  transfer_per_proc(n).m,*comm);
        send_buffer(n).Resize(buffer_size);int position=0;
        for(int j=1;j<=transfer_per_proc(n).m;j++) MPI_UTILITIES::Pack(
                transfer_per_proc(n)(j).x.x,transfer_per_proc(n)(j).x.y,
                transfer_per_proc(n)(j).y.x,transfer_per_proc(n)(j).y.y,
                transfer_per_proc(n)(j).z,
                send_buffer(n),position,*comm);
        requests.Append(comm->Isend(&send_buffer(n)(1),position,MPI::PACKED,
                                    neighbor_ranks(n),Get_Send_Tag(neighbor_directions(n))));}

    for(int n=1;n<=neighbor_ranks.Size();++n) if(neighbor_ranks(n)!=MPI::PROC_NULL && receive_per_proc(n).Size()){
        int recv_tag=Get_Recv_Tag(neighbor_directions(n));
        MPI::Status status;comm->Probe(neighbor_ranks(n),recv_tag,status);
        recv_buffers(n).Resize(status.Get_count(MPI::PACKED));
        requests.Append(comm->Irecv(&recv_buffers(n)(1),recv_buffers(n).m,MPI_PACKED,neighbor_ranks(n),recv_tag));}

    MPI_UTILITIES::Wait_All(requests);
    for(int n=1;n<=neighbor_ranks.Size();++n) if(neighbor_ranks(n)!=MPI::PROC_NULL){int position=0;
        for(int j=1;j<=receive_per_proc(n).m;j++){
            TRIPLE<PAIR<TV_INT,int>,PAIR<TV_INT,int>,T>& data=receive_per_proc(n)(j);
            MPI_UTILITIES::Unpack(data.x.x,data.x.y,data.y.x,data.y.y,data.z,recv_buffers(n),position,*comm);
            weights_received.Append(TRIPLE<PAIR<TV_INT,int>,PAIR<TV_INT,int>,T>(
                            PAIR<TV_INT,int>(data.x.x-local_to_global_offset,data.x.y),
                            PAIR<TV_INT,int>(data.y.x-local_to_global_offset,data.y.y),
                            data.z));}}
}
template<class T_GRID> void MPI_GRID<T_GRID>::
Distribute_Weights(const ARRAY<PAIR<PAIR<TV_INT,int>,T> >& weights_sender,
                   ARRAY<PAIR<PAIR<TV_INT,int>,T> >& weights_received)
{
    const ARRAY<int>& neighbor_ranks=all_neighbor_ranks;
    const ARRAY<TV_INT>& neighbor_directions=all_neighbor_directions;
    RANGE<TV_INT> sentinels=RANGE<TV_INT>::Zero_Box();
    ...
    RANGE<VECTOR<int,2> > range(VECTOR<int,2>(1,1),VECTOR<int,2>(number_of_processes,number_of_processes));
    ARRAY<int,VECTOR<int,2> > all_sizes(range);
    ARRAY<ARRAY<PAIR<PAIR<TV_INT,int>,T> > > transfer_per_proc(neighbor_ranks.Size()); 
    ARRAY<ARRAY<PAIR<PAIR<TV_INT,int>,T> > > receive_per_proc(neighbor_ranks.Size());
    for(int n=1;n<=neighbor_ranks.Size();++n) if(neighbor_ranks(n)!=MPI::PROC_NULL){
        for(int i=1;i<=weights_sender.Size();++i){
            if(send_regions(n).Lazy_Inside(weights_sender(i).x.x)){
                transfer_per_proc(n).Append(PAIR<PAIR<TV_INT,int>,T>(
                                PAIR<TV_INT,int>(local_to_global_offset+weights_sender(i).x.x,weights_sender(i).x.y),
                                weights_sender(i).y));}}}
    ...
    for(int n=1;n<=neighbor_ranks.Size();++n) if(neighbor_ranks(n)!=MPI::PROC_NULL && transfer_per_proc(n).Size()){
        int buffer_size=
            MPI_UTILITIES::Datatype<TV_INT>().Pack_size(transfer_per_proc(n).m,*comm)
           +MPI_UTILITIES::Datatype<int>()   .Pack_size(transfer_per_proc(n).m,*comm)
           +MPI_UTILITIES::Datatype<T>()     .Pack_size(transfer_per_proc(n).m,*comm);
        send_buffer(n).Resize(buffer_size);int position=0;
        for(int j=1;j<=transfer_per_proc(n).m;j++) MPI_UTILITIES::Pack(
                transfer_per_proc(n)(j).x.x,transfer_per_proc(n)(j).x.y,
                transfer_per_proc(n)(j).y,
                send_buffer(n),position,*comm);
        requests.Append(comm->Isend(&send_buffer(n)(1),position,MPI::PACKED,
                                    neighbor_ranks(n),Get_Send_Tag(neighbor_directions(n))));}
    ...
    for(int n=1;n<=neighbor_ranks.Size();++n) if(neighbor_ranks(n)!=MPI::PROC_NULL){int position=0;
        for(int j=1;j<=receive_per_proc(n).m;j++){
            PAIR<PAIR<TV_INT,int>,T>& data=receive_per_proc(n)(j);
            MPI_UTILITIES::Unpack(data.x.x,data.x.y,data.y,recv_buffers(n),position,*comm);
            weights_received.Append(PAIR<PAIR<TV_INT,int>,T>(
                            PAIR<TV_INT,int>(data.x.x-local_to_global_offset,data.x.y),data.y));}}
}
//#####################################################################
// Function Reduce_Add
//#####################################################################
...
//#####################################################################
template<class T_GRID> template<class T2> T2 MPI_GRID<T_GRID>::
Reduce_Max(const T2& local_value) const
{
    T global_value;
...
template<class T_GRID> void MPI_GRID<T_GRID>::Synchronize_Ghost_Cells(int& ghost_cells) const {PHYSBAM_FUNCTION_IS_NOT_DEFINED();}
...
template<class T_GRID> template<class T_MPI_GRID,class T_FACE_ARRAYS> void MPI_GRID<T_GRID>::Exchange_Boundary_Face_Data(const T_MPI_GRID& mpi_grid,T_FACE_ARRAYS& data,const int bandwidth) const
    {PHYSBAM_FUNCTION_IS_NOT_DEFINED();}
template<class T_GRID> template<class T_MPI_GRID,class T_NODE_ARRAYS> void MPI_GRID<T_GRID>::Exchange_Boundary_Node_Data(const T_MPI_GRID& mpi_grid,T_NODE_ARRAYS& data,const int bandwidth) const
    {PHYSBAM_FUNCTION_IS_NOT_DEFINED();}
template<class T_GRID> template<class T_MPI_GRID,class T_FACE_ARRAYS> void MPI_GRID<T_GRID>::Average_Common_Face_Data(const T_MPI_GRID& mpi_grid,T_FACE_ARRAYS& data) const
    {PHYSBAM_FUNCTION_IS_NOT_DEFINED();}
template<class T_GRID> template<class T_MPI_GRID,class T_FACE_ARRAYS> void MPI_GRID<T_GRID>::Sum_Common_Face_Data(const T_MPI_GRID& mpi_grid,T_FACE_ARRAYS& data) const
    {PHYSBAM_FUNCTION_IS_NOT_DEFINED();}
...
template<class T_GRID> template<class T2> T2 MPI_GRID<T_GRID>::Reduce_Max(const T2& local_value) const {PHYSBAM_FUNCTION_IS_NOT_DEFINED();}
...
template<class T_GRID> void MPI_GRID<T_GRID>::Distribute_Weights(const ARRAY<TRIPLE<PAIR<TV_INT,int>,PAIR<TV_INT,int>,T> >& weights_sender,ARRAY<TRIPLE<PAIR<TV_INT,int>,PAIR<TV_INT,int>,T> >& weights_received) {PHYSBAM_FUNCTION_IS_NOT_DEFINED();}
template<class T_GRID> void MPI_GRID<T_GRID>::Distribute_Weights(const ARRAY<PAIR<PAIR<TV_INT,int>,T> >& weights_sender,ARRAY<PAIR<PAIR<TV_INT,int>,T> >& weights_received) {PHYSBAM_FUNCTION_IS_NOT_DEFINED();}
//#####################################################################
...
#define INSTANTIATION_HELPER(T,T_GRID,T_REAL_GRID) \
    ...
    template void MPI_GRID<T_GRID >::Exchange_Boundary_Node_Data(const MPI_GRID_POLICY<T_REAL_GRID>::MPI_GRID&,GRID_ARRAYS_POLICY<T_REAL_GRID>::ARRAYS_BASE&,const int) const; \
    template void MPI_GRID<T_GRID >::Exchange_Boundary_Node_Data(const MPI_GRID_POLICY<T_REAL_GRID>::MPI_GRID&,GRID_ARRAYS_POLICY<T_REAL_GRID>::ARRAYS_BASE::REBIND<bool>::TYPE&,const int) const; \
    template void MPI_GRID<T_GRID >::Exchange_Boundary_Node_Data(const MPI_GRID_POLICY<T_REAL_GRID>::MPI_GRID&,GRID_ARRAYS_POLICY<T_REAL_GRID>::ARRAYS_BASE::REBIND<VECTOR<T,T_REAL_GRID::dimension+2> >::TYPE&,const int) const; \
    template void MPI_GRID<T_GRID >::Exchange_Boundary_Node_Data(const MPI_GRID_POLICY<T_REAL_GRID>::MPI_GRID&,GRID_ARRAYS_POLICY<T_REAL_GRID>::ARRAYS_BASE::REBIND<T_REAL_GRID::VECTOR_T>::TYPE&,const int) const; \
    template void MPI_GRID<T_GRID >::Exchange_Boundary_Node_Data(const MPI_GRID_POLICY<T_REAL_GRID>::MPI_GRID&,GRID_ARRAYS_POLICY<T_REAL_GRID>::ARRAYS_BASE::REBIND<MATRIX<T,1> >::TYPE&,const int) const; \
    template void MPI_GRID<T_GRID >::Exchange_Boundary_Node_Data(const MPI_GRID_POLICY<T_REAL_GRID>::MPI_GRID&,GRID_ARRAYS_POLICY<T_REAL_GRID>::ARRAYS_BASE::REBIND<SYMMETRIC_MATRIX<T,2> >::TYPE&,const int) const; \
    template void MPI_GRID<T_GRID >::Exchange_Boundary_Node_Data(const MPI_GRID_POLICY<T_REAL_GRID>::MPI_GRID&,GRID_ARRAYS_POLICY<T_REAL_GRID>::ARRAYS_BASE::REBIND<SYMMETRIC_MATRIX<T,3> >::TYPE&,const int) const; \
    ...
    template void MPI_GRID<T_GRID >::Sum_Common_Face_Data(const MPI_GRID_POLICY<T_REAL_GRID>::MPI_GRID&,GRID_ARRAYS_POLICY<T_REAL_GRID>::FACE_ARRAYS&) const; \
...
#define INSTANTIATION_HELPER_UNIFORM(T,T_GRID,d)     \
    ...
    template T MPI_GRID<T_GRID >::Reduce_Max(const T&) const;
