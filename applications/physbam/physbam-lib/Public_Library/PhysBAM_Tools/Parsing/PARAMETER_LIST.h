...
public:
    ...
    if(Find_And_Remove_From_Commandline_Parameter_Map(long_name,short_name,value_string)){
        if(Find_Parameter(long_name)) {std::stringstream ss;ss<<"Note: '"<<long_name<<"' overridden by commandline value ("<<value_string<<")"<<std::endl;LOG::filecout(ss.str());}
    ...
    if(i){
        ...
        if(!String_To_Value<T>(parameter_list(i).y.value_string,value)){
            ...
            if(verbose) {std::stringstream ss;ss<<"[param] Got "<<long_name<<"="<<parameter_list(i).y.value_string<<std::endl;LOG::filecout(ss.str());}
            return value;}}
    else{
