...
//#####################################################################
// Parsing for VECTOR<T,3>
//#####################################################################
template<class T> struct PARAMETER_LIST_PARSER<VECTOR<T,3> >
{
    static void Value_To_String(const VECTOR<T,3>& value,std::string& value_string)
    {
        std::ostringstream value_string_stream;value_string_stream<<value.x<<" "<<value.y<<" "<<value.z;
        value_string=value_string_stream.str();
    }
    static bool String_To_Value(const std::string& value_string,VECTOR<T,3>& value,bool commandline_style)
    {
        std::istringstream value_string_stream(value_string);
        FILE_UTILITIES::Ignore(value_string_stream,'[');
        value_string_stream>>value.x>>value.y>>value.z;
        return value_string_stream!=0;
    }
};
DEFINE_PARSER_PARAMETER(VECTOR<int,3>);
DEFINE_PARSER_PARAMETER(VECTOR<float,3>);
DEFINE_PARSER_PARAMETER(VECTOR<double,3>);
//#####################################################################
...
Begin_Parse(int argc,char *argv[],const std::string &default_extra_parameters_filename)
{
    ...
    if(!param_filename.empty() && (param_defined || FILE_UTILITIES::File_Exists(param_filename))){
        std::stringstream ss;
        ss<<"Note: Reading parameters from "<<param_filename<<std::endl;
        LOG::filecout(ss.str());
        Read(param_filename);}
}
//#####################################################################
...
End_Parse(bool exit_on_error)
{
    ...
    if(!first_error_parameter.empty()){
        std::stringstream ss;
        ss<<"ERROR: Problems parsing parameter '"<<first_error_parameter<<"'"<<std::endl;
        int i=Find_Parameter(first_error_parameter);
        if(i) ss<<"       (Got value string '"<<parameter_list(i).y.value_string<<"')"<<std::endl;
        LOG::filecout(ss.str());}
    if(commandline_parameter_map.Size())
        LOG::filecout("ERROR: Unrecognized commandline parameters:");
    std::stringstream ss;
    ss<<commandline_parameter_map<<std::endl;
    LOG::filecout(ss.str());
...
{
    std::stringstream ss;
    if(!usage.empty())ss<<"Usage: "<<program_name<<" [-param parameterfile] "<<usage<<"\n";
    ...
    for(int i=1;i<=parameter_list.m;i++){
        ...
        ss.flags(std::ios::left);ss.width(width+2);
        ss<<name;
        ss<<(!parameter_list(i).y.description.empty()?parameter_list(i).y.description+" ":"");
        if(parameter_list(i).y.default_value_string.length()>0) ss<<"(default: "<<parameter_list(i).y.default_value_string<<")";
        ss<<std::endl;}
    LOG::filecout(ss.str());
}
//#####################################################################
