...
public:
    ...
    {for(int i=1-ghost_cells;i<=counts.x+ghost_cells;i++) LOG::filecout((operator()(VECTOR<int,1>(i))));LOG::filecout("\n");}

    void Print_Grid_Array(VECTOR<int,2> counts,int ghost_cells=0)
    {for(int i=1-ghost_cells;i<=counts.x+ghost_cells;i++){
        for(int j=1-ghost_cells;j<=counts.y+ghost_cells;j++) LOG::filecout((operator()(VECTOR<int,2>(i,j))));
            LOG::filecout("\n");}
        LOG::filecout("\n");}
    ...
    {for(int i=1-ghost_cells;i<=counts.x+ghost_cells;i++){
        for(int j=1-ghost_cells;j<=counts.y+ghost_cells;j++){
            for(int k=1-ghost_cells;k<=counts.z+ghost_cells;k++) LOG::filecout((operator()(VECTOR<int,3>(i,j,k))));
            LOG::filecout("\n");}
        LOG::filecout("\n");}
        LOG::filecout("\n");}
#endif
