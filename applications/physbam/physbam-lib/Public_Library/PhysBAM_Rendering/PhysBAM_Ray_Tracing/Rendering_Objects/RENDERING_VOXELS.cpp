...
Precompute_Light_Data(bool use_fast_precomputation,RENDER_WORLD<T>& world)
{
    ...
    {std::stringstream ss;ss<<"have "<<lights.m<<" lights and "<<location_list.m<<" locations"<<std::endl;LOG::filecout(ss.str());}
    for(int light_index=1;light_index<=lights.m;light_index++){
        PROGRESS_INDICATOR progress(location_list.m);
        std::stringstream ss;ss<<"Light "<<light_index<<": ";LOG::filecout(ss.str());
