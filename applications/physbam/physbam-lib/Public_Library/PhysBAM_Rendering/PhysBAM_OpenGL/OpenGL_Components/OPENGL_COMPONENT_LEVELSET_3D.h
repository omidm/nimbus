...
public:
    using OPENGL_COMPONENT::is_interactive;

    OPENGL_COMPONENT_LEVELSET_3D(bool write_generated_triangulated_surface = false,
    ...
    OPENGL_COMPONENT_LEVELSET_3D(const std::string& levelset_filename,
                                 ...
                                 bool check_triangulated_surface_file_time = true,
                                 bool is_moving_grid_input=false,
                                 const std::string rigid_grid_frame_filename_set_input="",
                                 bool use_chimera_grid_input=false);
    ...
    virtual void Slice_Has_Changed() {if(!use_chimera_grid){for(int i=1;i<=opengl_levelset_multiviews.m;i++) opengl_levelset_multiviews(i)->Set_Slice(slice);} else{for(int i=1;i<=opengl_levelset_multiviews.m;i++) opengl_levelset_multiviews(i)->Set_Slice(chimera_slice->opengl_uniform_slices(i));}}
...
private:
    void Reinitialize_From_Simulation();
    void Reinitialize();
...
public:
    int ghost_cells;
    bool use_chimera_grid;
    OPENGL_CHIMERA_SLICE<T>* chimera_slice;
    ARRAY<FRAME<TV> > rigid_grid_frames;
};
