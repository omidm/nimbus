...
#endif
    {
        OPENGL_COLOR::Gray(0.7f).Send_To_GL_Pipeline();
        vertices.Resize(0);
        ...
        OpenGL_Draw_Arrays(GL_LINES,2,vertices);
        OPENGL_COLOR::Gray(0.3f).Send_To_GL_Pipeline();
        vertices.Resize(0);
        ...
        if(draw_piecewise_constant){
            ...
            if(draw_flux && flux.counts.x){
                ...
                glLineWidth(2);
                vertices.Resize(0);
                T arrowhead_size=(T).3,sin_angle=(T)sin(0.5),cos_angle=(T)cos(0.5);
                for(int i=x.domain.min_corner.x;i<=x.domain.max_corner.x-1;i++){
...
Bounding_Box() const
{
    if(valid&&draw){
        ...
        return World_Space_Box(RANGE<VECTOR<float,3> >((float)xmin,(float)xmax,(float)umin,(float)umax,0,0));}
...
{
    T area=0;
    for(int i=0;i<=x.domain.max_corner.x-1-ghost_nodes;i++) area+=(T)(0.5*(u(i)+u(i+1))*(x(i+1)-x(i)));
    return area;
}
//#####################################################################
// Callbacks
...
Bounding_Box() const
{
    ...
    RANGE<VECTOR<float,3> > box(VECTOR<float,3>((float)curve.x(index),(float)curve.u(index),0));
...
Bounding_Box() const
{
    PHYSBAM_ASSERT(object);
    const OPENGL_COMPONENT_CURVE_2D<T,RW> &curve=*(OPENGL_COMPONENT_CURVE_2D<T,RW> *)object;
    RANGE<VECTOR<float,3> > box=RANGE<VECTOR<float,3> >::Bounding_Box(VECTOR<float,3>((float)curve.x(index),(float)curve.u(index),0),VECTOR<float,3>((float)curve.x(index+1),(float)curve.u(index+1),0));
    return object->World_Space_Box(box);
}
