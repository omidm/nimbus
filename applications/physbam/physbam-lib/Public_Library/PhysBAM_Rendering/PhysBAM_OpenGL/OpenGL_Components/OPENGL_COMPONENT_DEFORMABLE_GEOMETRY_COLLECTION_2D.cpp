...
OPENGL_COMPONENT_DEFORMABLE_GEOMETRY_COLLECTION_2D(const std::string& prefix,const int start_frame)
    :OPENGL_COMPONENT("Deformable Object List"),prefix(prefix),frame_loaded(-1),valid(false),display_mode(0),draw_velocities(false),velocity_scale((T)0.025),valid_object_index(0),
...
Reinitialize(bool force)
{
    ...
    if(read_static_variables && first_time) {std::stringstream ss;ss<< "Deformable bodies static variables will be read each frame" << std::endl;LOG::filecout(ss.str());}
    ...
    for(int i=1;i<=deformable_geometry_collection->structures.m;i++){
        ...
        if(FILE_UTILITIES::File_Exists(frame_prefix+"stress_map_of_triangulated_area"+suffix)){
            if(first_time) {std::stringstream ss;ss<<"adding stress map to triangulated area"<<std::endl;LOG::filecout(ss.str());}
...
#ifndef USE_OPENGLES
    for(int i=1;i<=segmented_curve_objects.m;i++)if(!valid_object_index || i==valid_object_index){
        glPushName(i);
        glPushName(1);
...
//#####################################################################
template<class T,class RW> void OPENGL_COMPONENT_DEFORMABLE_GEOMETRY_COLLECTION_2D<T,RW>::
Cycle_Valid_Object_Index()
{
    valid_object_index=(valid_object_index+1)%(segmented_curve_objects.m+1);
}
//#####################################################################
