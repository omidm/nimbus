...
OPENGL_COMPONENT_MAC_VELOCITY_FIELD_2D(const GRID<TV> &grid,const std::string &velocity_filename_input,const std::string directory_adaptive_input,const std::string filename_active_cells_input,const std::string filename_active_faces_input,bool is_moving_grid_input,std::string rigid_grid_frame_filename_input)
    ...
    use_levels(true),level_loaded(-1),valid(false),draw_divergence(false),draw_all_levels(true),draw_streamlines(false),use_seed_for_streamlines(false),opengl_divergence_field(0),
     streamlines(*new SEGMENT_MESH(),*new GEOMETRY_PARTICLES<TV>()),opengl_streamlines(streamlines),psi_N_psi_D_basedir(""),min_vorticity(FLT_MAX),max_vorticity(FLT_MIN),is_moving_grid(is_moving_grid_input),rigid_grid_frame_filename(rigid_grid_frame_filename_input),print_weights_i(false),print_weights_j(false)
{
...
template<class T,class RW> OPENGL_COMPONENT_MAC_VELOCITY_FIELD_2D<T,RW>::
OPENGL_COMPONENT_MAC_VELOCITY_FIELD_2D(const GRID<TV> &grid,const std::string &velocity_filename_input,const std::string directory_adaptive_input,const std::string filename_active_cells_input,const std::string filename_active_faces_input,const ARRAY<GRID<TV>*> grid_array_input,bool is_moving_grid_input,std::string rigid_grid_frame_filename_input)
    ...
    valid(false),draw_divergence(false),draw_all_levels(true),draw_streamlines(false),use_seed_for_streamlines(false),opengl_divergence_field(0),streamlines(*new SEGMENT_MESH(),*new GEOMETRY_PARTICLES<TV>()),opengl_streamlines(streamlines),psi_N_psi_D_basedir(""),min_vorticity(FLT_MAX),max_vorticity(FLT_MIN),is_moving_grid(is_moving_grid_input),rigid_grid_frame_filename(rigid_grid_frame_filename_input),print_weights_i(false),print_weights_j(false)
{
...
Initialize(const ARRAY<GRID<TV>*> &grid_array_input)
{
    ...
    while(directory_adaptive!=""){
        ...
        std::stringstream ss;
        ss<<"Checking "<<dirname<<std::endl;
        LOG::filecout(ss.str());
        if(FILE_UTILITIES::Directory_Exists(dirname)) number_of_levels++;
        else break;}
    std::stringstream ss;
    ss<<"Found "<<number_of_levels<<" levels for adaptive"<<std::endl;
    LOG::filecout(ss.str());
    ...
    for(int i=1;i<=opengl_adaptive_mac_velocity_fields.m;i++) opengl_adaptive_mac_velocity_fields(i)=new OPENGL_MAC_VELOCITY_FIELD_2D<T>(*(new GRID<TV>(*grid_array_input(i))),*(new ARRAY<T,FACE_INDEX<2> >),0,0,is_moving_grid);
    opengl_mac_velocity_field=opengl_adaptive_mac_velocity_fields(1);
    number_of_steps=2*opengl_mac_velocity_field->grid.counts.x;
    opengl_vorticity_magnitude=new OPENGL_SCALAR_FIELD_2D<T>(opengl_mac_velocity_field->grid,*(new ARRAY<T,VECTOR<int,2> >),OPENGL_COLOR_RAMP<T>::Matlab_Jet(0,1),OPENGL_SCALAR_FIELD_2D<T>::DRAW_TEXTURE,is_moving_grid);
    ...
    ramp->Add_Color(1,OPENGL_COLOR::Yellow());
    ramp->Add_Color((T)1e+2,OPENGL_COLOR::Red());
    opengl_divergence_field=new OPENGL_SCALAR_FIELD_2D<T>(opengl_mac_velocity_field->grid,divergence,ramp,OPENGL_SCALAR_FIELD_2D<T>::DRAW_TEXTURE,is_moving_grid);
...
template<class T,class RW> void OPENGL_COMPONENT_MAC_VELOCITY_FIELD_2D<T,RW>::
Print_Weights_I(std::ostream& stream,const FACE_INDEX<2>& face) const
{
    ARRAY<PAIR<FACE_INDEX<TV::dimension>,T> >& local_weights=const_cast<ARRAY<PAIR<FACE_INDEX<TV::dimension>,T> >&>(weights_to(face));
    for(int i=1;i<=local_weights.m;i++){
        stream<<"    "<<local_weights(i).y<<" to "<<local_weights(i).x.axis<<","<<local_weights(i).x.index<<std::endl;}
}

template<class T,class RW> void OPENGL_COMPONENT_MAC_VELOCITY_FIELD_2D<T,RW>::
Print_Weights_J(std::ostream& stream,const FACE_INDEX<2>& face) const
{
    ARRAY<PAIR<FACE_INDEX<TV::dimension>,int> >& local_weights=const_cast<ARRAY<PAIR<FACE_INDEX<TV::dimension>,int> >&>(weights_from(face));
    for(int i=1;i<=local_weights.m;i++){
        stream<<"    "<<weights_to(local_weights(i).x)(local_weights(i).y).y<<" to "<<local_weights(i).x.axis<<","<<local_weights(i).x.index<<std::endl;}
}
...
Print_Selection_Info(std::ostream& stream,OPENGL_SELECTION* selection) const
{
    if(Is_Up_To_Date(frame)){
        ...
        if(print_weights_i && ((OPENGL_SELECTION_GRID_CELL_2D<T>*)selection)){
            VECTOR<int,2> index=((OPENGL_SELECTION_GRID_CELL_2D<T>*)selection)->index;
            stream<<"bottom w_i"<<std::endl;
            Print_Weights_I(stream,FACE_INDEX<2>(2,index));
            stream<<"top w_i"<<std::endl;
            Print_Weights_I(stream,FACE_INDEX<2>(2,VECTOR<int,2>(index.x,index.y+1)));
            stream<<"left w_i"<<std::endl;
            Print_Weights_I(stream,FACE_INDEX<2>(1,index));
            stream<<"right w_i"<<std::endl;
            Print_Weights_I(stream,FACE_INDEX<2>(1,VECTOR<int,2>(index.x+1,index.y)));}
        if(print_weights_j && ((OPENGL_SELECTION_GRID_CELL_2D<T>*)selection)){
            VECTOR<int,2> index=((OPENGL_SELECTION_GRID_CELL_2D<T>*)selection)->index;
            stream<<"bottom w_j"<<std::endl;
            Print_Weights_J(stream,FACE_INDEX<2>(2,index));
            stream<<"top w_j"<<std::endl;
            Print_Weights_J(stream,FACE_INDEX<2>(2,VECTOR<int,2>(index.x,index.y+1)));
            stream<<"left w_j"<<std::endl;
            Print_Weights_J(stream,FACE_INDEX<2>(1,index));
            stream<<"right w_j"<<std::endl;
            Print_Weights_J(stream,FACE_INDEX<2>(1,VECTOR<int,2>(index.x+1,index.y)));}
    ...
    if (draw || draw_divergence){
        if ((is_animation && (frame_loaded!=frame || level_loaded!=level)) || (!is_animation && frame_loaded < 0)){
            ...
            if(use_levels) for(int i=1;i<=opengl_adaptive_mac_velocity_fields.m;i++){
                ...
                {std::stringstream ss;ss<<"Reading active cells from"<<tmp_filename<<std::endl;LOG::filecout(ss.str());}
                ...
                {std::stringstream ss;ss<<"Reading active faces from"<<tmp_filename<<std::endl;LOG::filecout(ss.str());}
                ...
                if(is_moving_grid){
                    ...
                    if(FILE_UTILITIES::File_Exists(grid_filename)) FILE_UTILITIES::Read_From_File<T>(grid_filename,opengl_mac_velocity_field->rigid_grid_frame);}
...
{
    for(int i=1;i<=opengl_adaptive_mac_velocity_fields.m;i++) opengl_adaptive_mac_velocity_fields(i)->Scale_Vector_Size((T)1.1);
}

template<class T,class RW> void OPENGL_COMPONENT_MAC_VELOCITY_FIELD_2D<T,RW>::
Decrease_Vector_Size()
{
    for(int i=1;i<=opengl_adaptive_mac_velocity_fields.m;i++) opengl_adaptive_mac_velocity_fields(i)->Scale_Vector_Size(1/(T)1.1);
}
