...
Display(const int in_color) const
{
    ...
    OPENGL_COLOR ground_color = OPENGL_COLOR::Gray(0.8f);
...
Bounding_Box() const
{
    if (valid && draw)
    {
        ...
        return RANGE<VECTOR<float,3> >((float)min_x,(float)max_x,(float)min_height,(float)max_height, 0, 0);
    }
...
{
    opengl_vector_field.size *= (T)1.1;
}

template<class T,class RW> void OPENGL_COMPONENT_HEIGHTFIELD_1D<T,RW>::
Decrease_Velocity_Scale()
{
    opengl_vector_field.size *= 1/(T)1.1;
}
