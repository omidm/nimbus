...
OPENGL_COMPONENT_LEVELSET_2D(const std::string& levelset_filename_input,const std::string filename_set_input,bool is_moving_grid_input,const std::string rigid_grid_frame_filename_input)
    :OPENGL_COMPONENT("Levelset 2D"),opengl_levelset(0),levelset_filename(levelset_filename_input),filename_set(filename_set_input),frame_loaded(-1),set(1),use_sets(true),set_loaded(-1),valid(false),draw_multiple_levelsets(false),is_moving_grid(is_moving_grid_input),rigid_grid_frame_filename(rigid_grid_frame_filename_input)
{
    ...
    std::stringstream ss;ss<<"Found "<<number_of_sets<<" levelsets for multiphase"<<std::endl;LOG::filecout(ss.str());
    ...
    for(int j=1;j<=opengl_levelsets.m;j++)
        opengl_levelsets(j)=new OPENGL_LEVELSET_2D<T>(*(new LEVELSET_2D<GRID<TV> >(*(new GRID<TV>),*(new ARRAY<T,VECTOR<int,2> >))),color_map->Lookup(j),OPENGL_COLOR::Transparent(),0,is_moving_grid_input);
        ...
        if ((is_animation && (frame_loaded!=frame || set_loaded!=set)) || (!is_animation && frame_loaded<0)){
            ...
            if(use_sets) for(int i=1;i<=opengl_levelsets.m;i++){
                ...
                if(is_moving_grid){
                    ...
                    if(FILE_UTILITIES::File_Exists(grid_filename)) FILE_UTILITIES::Read_From_File<T>(grid_filename,opengl_levelset->rigid_grid_frame);}
...
Next_Set()
{
    ...
    std::stringstream ss;
    ss<<"viewing levelset set "<<set<<std::endl;
    LOG::filecout(ss.str());
