...
Display(const int in_color) const
{
    ...
    if(draw_points){
        ...
        for(DYADIC_GRID_ITERATOR_FACE<OCTREE_GRID<T> > iterator(grid,grid.Map_All_Faces());iterator.Valid();iterator.Next()){
            color_map->Lookup(value(iterator.Face_Index())).Send_To_GL_Pipeline();OpenGL_Vertex(iterator.Location(),vertices);}
        ...
        for(DYADIC_GRID_ITERATOR_FACE<OCTREE_GRID<T> > iterator(grid,grid.Map_All_Faces());iterator.Valid();iterator.Next()){
            ...
            OpenGL_Line(iterator.Location(),VECTOR<T,3>(iterator.Location()+val*line_size*VECTOR<T,3>::Axis_Vector(iterator.Axis())),vertices);}
