...
OPENGL_SCALAR_FIELD_1D(const GRID<TV>& grid,ARRAY<T2,VECTOR<int,1> >& values,OPENGL_COLOR point_color,OPENGL_COLOR line_color)
    :grid(grid),values(values),point_color(point_color),line_color(line_color),scale(1),upsample_scale(1)
{}
//#####################################################################
// Function OPENGL_SCALAR_FIELD_1D
//#####################################################################
...
Bounding_Box() const
{
    ...
    for(int i=min_corner;i<=max_corner;i++) box.Enlarge_To_Include_Point(VECTOR<float,3>((float)grid.Axis_X(i,1),(float)values(i),0));
    std::stringstream ss;
    ss<<"box is "<<box<<std::endl;
    LOG::filecout(ss.str());
    return box;
}
//#####################################################################
template class OPENGL_SCALAR_FIELD_1D<float,int>;
template class OPENGL_SCALAR_FIELD_1D<float,float>;
template class OPENGL_SCALAR_FIELD_1D<float,bool>;
#ifndef COMPILE_WITHOUT_DOUBLE_SUPPORT
template class OPENGL_SCALAR_FIELD_1D<double,int>;
