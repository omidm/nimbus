...
public:
    OPENGL_GRID_2D(GRID<TV> &grid_input,const OPENGL_COLOR &color_input=OPENGL_COLOR::White(),const std::string basedir_input="",const int frame_input=0,bool is_moving_grid_input=false,const std::string rigid_grid_frame_filename_input="")
        :grid(grid_input),active_cell_mask(0),ghost_cell_mask(0),active_face_mask(0),ghost_face_mask(0),active_node_mask(0),ghost_node_mask(0),color(color_input),draw(true),draw_ghost_values(true),draw_mask_type(0),current_selection(0),basedir(basedir_input),frame(frame_input),is_moving_grid(is_moving_grid_input),rigid_grid_frame_filename(rigid_grid_frame_filename_input)
    {Reinitialize();}
