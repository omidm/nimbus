...
public:
    ...
    using OPENGL_SCALAR_FIELD_2D<T,T>::contour_values;

    LEVELSET_2D<GRID<TV> >& levelset;
    OPENGL_TRIANGULATED_AREA<T>* opengl_triangulated_area;
    ARRAY<OPENGL_SEGMENTED_CURVE_2D<T>*> opengl_segmented_curve_2d;
    ...
    FRAME<TV> rigid_grid_frame;
private:
    bool draw_normals;
    bool is_moving_grid;

public:
    OPENGL_LEVELSET_2D(LEVELSET_2D<GRID<TV> >& levelset_input,const OPENGL_COLOR& inside_color_input=OPENGL_COLOR::Blue(),const OPENGL_COLOR& outside_color_input=OPENGL_COLOR::Red((T).5),ARRAY<bool,VECTOR<int,2> > *active_cells_input=0,bool is_moving_grid_input=false)
        ...
        gradient_color_map(0),draw_cells(false),draw_area(true),draw_curve(true),dominant_sign(-1),draw_normals(false),is_moving_grid(is_moving_grid_input)
    {
    ...
    ~OPENGL_LEVELSET_2D()
    {for(int i=1;i<=opengl_segmented_curve_2d.m;i++) delete opengl_segmented_curve_2d(i);delete opengl_triangulated_area;}
