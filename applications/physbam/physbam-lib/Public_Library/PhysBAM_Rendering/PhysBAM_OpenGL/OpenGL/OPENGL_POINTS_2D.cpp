...
OPENGL_POINTS_2D(T_ARRAY& points_input,const OPENGL_COLOR& color_input,const T point_size_input)
...
#endif
    {
        if(point_colors){
            if(point_radii && draw_radii){
                for(int i=1;i<=points.Size();i++){
                    glPushMatrix();
                    glTranslatef((GLfloat)points(i).x,(GLfloat)points(i).y,0);
                ...
                for(int i=1;i<=points.Size();i++){
                    OpenGL_Color((*point_colors)(i).rgba,colors);
                    OpenGL_Vertex(points(i),vertices);}
                OpenGL_Draw_Arrays(GL_POINTS,2,vertices,colors);}}
        else{
            if(point_radii && draw_radii){
                color.Send_To_GL_Pipeline();
                for(int i=1;i<=points.Size();i++){
