...
OPENGL_SCALAR_FIELD_2D(GRID<TV> &grid_input,ARRAY<T2,VECTOR<int,2> > &values_input,OPENGL_COLOR_MAP<T2>* color_map_input,DRAW_MODE draw_mode_input,bool is_moving_grid_input)
    : grid(grid_input),values(values_input),active_cells(0),draw_ghost_values(true),current_color_map(1),opengl_textured_rect(0),opengl_points(0),scale_range(false),is_moving_grid(is_moving_grid_input),upsample_scale(1)
{
...
template<class T,class T2> OPENGL_SCALAR_FIELD_2D<T,T2>::
OPENGL_SCALAR_FIELD_2D(GRID<TV> &grid_input,ARRAY<T2,VECTOR<int,2> > &values_input,OPENGL_COLOR_MAP<T2>* color_map_input,ARRAY<bool,VECTOR<int,2> >* active_cells_input,DRAW_MODE draw_mode_input,bool is_moving_grid_input)
    : grid(grid_input),values(values_input),active_cells(active_cells_input),draw_ghost_values(true),current_color_map(1),opengl_textured_rect(0),opengl_points(0),scale_range(false),is_moving_grid(is_moving_grid_input),upsample_scale(1)
{
    PHYSBAM_ASSERT(color_map_input);
    Initialize_Color_Maps(color_map_input);
    Set_Draw_Mode(draw_mode_input);
...
Display(const int in_color) const
{
    if(is_moving_grid){
        glPushMatrix();
        glTranslatef((GLfloat)rigid_grid_frame.t(1),(GLfloat)rigid_grid_frame.t(2),0);
        glRotatef((GLfloat)(rigid_grid_frame.r.Angle()/pi*180),0,0,1);
...
Display_2D() const
{
    ...
    for(int i=1;i<=grid.counts.x;i++) for(int j=1;j<=grid.counts.y;j++) {
        ...
        OpenGL_Vertex(VECTOR<T,2>(pos.x-(T)0.5*grid.dX.x,pos.y-(T)0.5*grid.dX.y),vertices);
        OpenGL_Vertex(VECTOR<T,2>(pos.x-(T)0.5*grid.dX.x,pos.y+(T)0.5*grid.dX.y),vertices);
        OpenGL_Vertex(VECTOR<T,2>(pos.x+(T)0.5*grid.dX.x,pos.y-(T)0.5*grid.dX.y),vertices);
        OpenGL_Vertex(VECTOR<T,2>(pos.x+(T)0.5*grid.dX.x,pos.y+(T)0.5*grid.dX.y),vertices);
...
Bounding_Box() const
{
    ...
    return World_Space_Box(RANGE<VECTOR<float,3> >((float)grid.domain.min_corner.x,(float)grid.domain.max_corner.x,(float)grid.domain.min_corner.y,(float)grid.domain.max_corner.y,0,0));
}
