...
Capture_Frames(const std::string& filename_pattern,int capture_start_frame,int capture_end_frame,int jpeg_quality,bool swap_buffers)
{
    ...
    if(movie && MOV_WRITER<float>::Enabled()){
        mov=new MOV_WRITER<float>(filename_pattern,frame_rate);
        std::stringstream ss;
        ss<<"Capturing to quicktime '"<<filename_pattern<<"'"<<std::endl;
        LOG::filecout(ss.str());
    }

    {std::stringstream ss;
    ss<<"Capturing frames "<<capture_start_frame<<" to ";
    if(capture_end_frame==INT_MAX) ss<<"last valid frame";else ss<<capture_end_frame;
    ss<<" into '"<<filename_pattern<<"'"<<std::endl;
    LOG::filecout(ss.str());}
    ...
    for(;;){
        ...
        if(mov){
            ...
            std::string filename=STRING_UTILITIES::string_sprintf(filename_pattern.c_str(),frame);
            {std::stringstream ss;
            ss<<"Capturing frame "<<frame<<" to "<<filename<<std::endl;
            LOG::filecout(ss.str());}
    ...
    for(int i=1;i<=component_list.m;i++){
        ...
        while(!done && attempts++ < 10){
            try{
                ...
                PROCESS_UTILITIES::PB_Sleep(100);}}
#else
...
Next_Frame()
{
    ...
    else if(play) opengl_world.Set_Idle_Callback(Next_Frame_CB(),.2f);
}
