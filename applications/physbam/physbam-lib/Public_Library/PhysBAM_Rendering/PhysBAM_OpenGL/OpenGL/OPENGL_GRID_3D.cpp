...
OPENGL_GRID_3D(GRID<TV> &grid_input,const OPENGL_COLOR &color_input,bool is_moving_grid_input) 
    :grid(grid_input),color(color_input),draw_ghost_values(true),hide_non_selected_grid(false),scale(1),current_selection(0),is_moving_grid(is_moving_grid_input)
{
}
//#####################################################################
// Display
...
Print_Selection_Info(std::ostream& stream,OPENGL_SELECTION* selection) const
{
    if(current_selection && current_selection->type==OPENGL_SELECTION::GRID_NODE_3D){
        ...
        stream<<"Selected node "<<index<<" ("<<rigid_grid_frame*grid.Get_Regular_Grid().X(index)<<")"<<std::endl;}
        ...
        stream<<"Selected cell "<<index<<" ("<<rigid_grid_frame*grid.Get_MAC_Grid().X(index)<<")"<<std::endl;}
}
//#####################################################################
// Function Bounding_Box
//#####################################################################
