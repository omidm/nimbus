...
Display(const int in_color) const
{
    ...
    if(mode==GL_SELECT){
        ...
        for(DYADIC_GRID_ITERATOR_CELL<QUADTREE_GRID<T> > iterator(grid,grid.number_of_ghost_cells);iterator.Valid();iterator.Next()){
            ...
            OpenGL_Quad_2D(grid.Node_Location(0,cell),grid.Node_Location(3,cell),vertices);
...
#endif
    {
        ...
        for (i=1,x=grid.uniform_grid.domain.min_corner.x;i<=m+1;i++,x+=grid.uniform_grid.dX.x) OpenGL_Line(VECTOR<T,3>(x,grid.uniform_grid.domain.min_corner.y,0),VECTOR<T,3>(x,grid.uniform_grid.domain.max_corner.y,0),vertices);
        for (j=1,y=grid.uniform_grid.domain.min_corner.y;j<=n+1;j++,y+=grid.uniform_grid.dX.y) OpenGL_Line(VECTOR<T,3>(grid.uniform_grid.domain.min_corner.x,y,0),VECTOR<T,3>(grid.uniform_grid.domain.max_corner.x,y,0),vertices);
        OpenGL_Draw_Arrays(GL_LINES,3,vertices); // dimension should be 3, because 0 is added in the third dimension!
        ...
        for(DYADIC_GRID_ITERATOR_CELL<QUADTREE_GRID<T> > iterator(grid,grid.number_of_ghost_cells);iterator.Valid();iterator.Next()){
            QUADTREE_CELL<T>* cell=iterator.Cell_Pointer();
            OpenGL_Line(grid.Node_Location(0,cell),grid.Node_Location(1,cell),vertices);
            OpenGL_Line(grid.Node_Location(1,cell),grid.Node_Location(3,cell),vertices);
            OpenGL_Line(grid.Node_Location(3,cell),grid.Node_Location(2,cell),vertices);
            OpenGL_Line(grid.Node_Location(2,cell),grid.Node_Location(0,cell),vertices);}
