...
public:
    GRID<TV> grid,*coarse_grid;
    ARRAY<T2,VECTOR<int,3> > &values;
    ARRAY<T2,VECTOR<int,3> > *values_simulated;
    ...
    OPENGL_SCALAR_FIELD_3D(const GRID<TV> &grid_input,ARRAY<T2,VECTOR<int,3> > &values_input,OPENGL_COLOR_MAP<T2> *color_map_input,DRAW_MODE draw_mode_input=DRAW_TEXTURE,bool is_moving_grid_input=false);
    ~OPENGL_SCALAR_FIELD_3D();
    void Initialize_Color_Maps(OPENGL_COLOR_MAP<T2>* color_map);
    void Initialize_Upsampled_Scale(const int upsampled_scale);
