...
OPENGL_MAC_VELOCITY_FIELD_2D(GRID<TV> &grid, ARRAY<T,FACE_INDEX<2> > &face_velocities_input,T_ARRAYS_BOOL *active_cells_input,T_FACE_ARRAYS_BOOL *active_faces_input,bool is_moving_grid_input)
     ...
     grid(grid),face_velocities(face_velocities_input),u(face_velocities.Component(1)),v(face_velocities.Component(2)),active_cells(active_cells_input),active_faces(active_faces_input)
{
    ...
    is_moving_grid=is_moving_grid_input;
}
...
Bounding_Box() const
{
    return RANGE<VECTOR<float,3> >((float)grid.domain.min_corner.x,(float)grid.domain.max_corner.x,(float)grid.domain.min_corner.y,(float)grid.domain.max_corner.y,0,0);
}
...
Update()
{
    ...
    if(velocity_mode == FACE_CENTERED){
        ...
        for(int i=u.domain.min_corner.x;i<=u.domain.max_corner.x-1;i++) for(int j=v.domain.min_corner.y;j<=v.domain.max_corner.y-1;j++) if(!active_cells||(*active_cells)(i,j)){vector_field(idx)=VECTOR<T,2>((T).5*(u(i,j)+u(i+1,j)),(T).5*(v(i,j)+v(i,j+1)));vector_locations(idx)=grid.Center(i,j);idx++;}
        vector_field.Resize(idx-1);
        vector_locations.Resize(idx-1);}
}
//#####################################################################
...
Print_Selection_Info(std::ostream& output_stream,OPENGL_SELECTION* selection) const
{
    if(selection && selection->type==OPENGL_SELECTION::GRID_CELL_2D && grid.Is_MAC_Grid()){
        ...
        TV center_velocity((T)0.5*(u_left+u_right),(T)0.5*(v_bottom+v_top));
