...
Display(const int in_color) const
{
...
#endif
    {
        ...
        ARRAY<typename OPENGL_POLICY<T>::T_GL> vertices;
        if(point_colors){
            ARRAY<GLfloat> colors;
            if(draw_mask){for(int i=1;i<=points.Size();i++)if((*draw_mask)(i)){OpenGL_Color((*point_colors)(i).rgba,colors);OpenGL_Vertex(points(i), vertices);}}
            else{for(int i=1;i<=points.Size();i++){OpenGL_Color((*point_colors)(i).rgba,colors);OpenGL_Vertex(points(i), vertices);}
            OpenGL_Draw_Arrays(GL_POINTS,3,vertices,colors);}}
        else{
            if(draw_mask){color.Send_To_GL_Pipeline();for(int i=1;i<=points.Size();i++)if((*draw_mask)(i)) OpenGL_Vertex(points(i), vertices);}
