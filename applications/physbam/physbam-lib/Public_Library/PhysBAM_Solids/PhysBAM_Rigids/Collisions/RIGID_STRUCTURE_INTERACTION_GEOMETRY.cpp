...
//#####################################################################
// Class RIGID_STRUCTURE_INTERACTION_GEOMETRY
//#####################################################################
...
//#####################################################################
template<class TV> typename TOPOLOGY_BASED_SIMPLEX_POLICY<TV,1>::OBJECT* RIGID_STRUCTURE_INTERACTION_GEOMETRY<TV>::
...
//#####################################################################
template<class TV> TRIANGULATED_SURFACE<typename TV::SCALAR>* RIGID_STRUCTURE_INTERACTION_GEOMETRY<TV>::
...
//#####################################################################
template<class TV> void RIGID_STRUCTURE_INTERACTION_GEOMETRY<TV>::
...
//#####################################################################
template<class TV> template<class T_OBJECT,class T_HIERARCHY> void RIGID_STRUCTURE_INTERACTION_GEOMETRY<TV>::
...
//#####################################################################
template<class TV> void RIGID_STRUCTURE_INTERACTION_GEOMETRY<TV>::
Save_Current_State(const TV& X,const ROTATION<TV>& rotation,const TV& V)
{
    this->X=FRAME<TV>(X,rotation)*full_particles.X;
    this->V.Resize(full_particles.array_collection->Size());
    for(int i=1;i<=full_particles.array_collection->Size();i++) this->V(i)=V;
}
//#####################################################################
// Function Save_Self_Collision_Free_State
//#####################################################################
template<class TV> void RIGID_STRUCTURE_INTERACTION_GEOMETRY<TV>::
Save_Self_Collision_Free_State(const TV& X,const ROTATION<TV>& rotation,const TV& V) // assumes mass does not change
{
    X_self_collision_free=FRAME<TV>(X,rotation)*full_particles.X;
    V_self_collision_free.Resize(full_particles.array_collection->Size());
    for(int i=1;i<=full_particles.array_collection->Size();i++) V_self_collision_free(i)=V;
}
//#####################################################################
// Function Save_Self_Collision_Free_State
//#####################################################################
...
{
    X_self_collision_free=X;
    V_self_collision_free=V;
}
//#####################################################################
// Function Save_Pseudo_State
//#####################################################################
template<class TV> void RIGID_STRUCTURE_INTERACTION_GEOMETRY<TV>::
Save_Pseudo_State(const T dt)
{
    V=(X-X_self_collision_free)/dt;
}
//#####################################################################
...
{
    X=X_self_collision_free;
    V=V_self_collision_free;
}
//####################################################################
template class RIGID_STRUCTURE_INTERACTION_GEOMETRY<VECTOR<float,1> >;
template class RIGID_STRUCTURE_INTERACTION_GEOMETRY<VECTOR<float,2> >;
template class RIGID_STRUCTURE_INTERACTION_GEOMETRY<VECTOR<float,3> >;
#ifndef COMPILE_WITHOUT_DOUBLE_SUPPORT
template class RIGID_STRUCTURE_INTERACTION_GEOMETRY<VECTOR<double,1> >;
template class RIGID_STRUCTURE_INTERACTION_GEOMETRY<VECTOR<double,2> >;
template class RIGID_STRUCTURE_INTERACTION_GEOMETRY<VECTOR<double,3> >;
#endif
