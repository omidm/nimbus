...
public:
    ...
    {return Minimum_Edge_Length(X(1),X(2),X(3));}

    T Maximum_Edge_Length() const
    {return Maximum_Edge_Length(X(1),X(2),X(3));}
    ...
    TV First_Two_Barycentric_Coordinates(const TV& location)
    {return First_Two_Barycentric_Coordinates(location,X(1),X(2),X(3));}
    ...
    {return Barycentric_Coordinates(location,X(1),X(2),X(3));}
    ...
    template<class T_ARRAY>
    static TV Circumcenter(const T_ARRAY& x)
    {TV x1x2=x(2)-x(1),x1x3=x(3)-x(1),m1=(T).5*(x(1)+x(2)),m2=(T).5*(x(1)+x(3)),m1m2=m2-m1,x1x2_perp(-x1x2.y,x1x2.x);
    return m1+x1x2_perp*(TV::Dot_Product(m1m2,x1x3)/TV::Dot_Product(x1x2_perp,x1x3));}
    ...
    TV Circumcenter()
    {return Circumcenter(X[1],X[2],X[3]);}

    VECTOR<T,3> Circumcenter_Barycentric_Coordinates()
    {return Circumcenter_Barycentric_Coordinates(X[1],X[2],X[3]);}
...
//#####################################################################
    ...
    void Clip_To_Simplex(const TRIANGLE_2D<T>& simplex,ARRAY<TRIANGLE_2D<T> >& clipped_simplices) const;
