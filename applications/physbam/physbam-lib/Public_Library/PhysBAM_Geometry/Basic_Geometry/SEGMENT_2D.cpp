                            ...
                            T& distance,VECTOR<T,2>& normal,T& a,T& b,T& relative_speed,const T small_number) const
{
    ...
    if(distance > small_number) normal/=distance;
        ...
        if(normal_magnitude > small_number) normal/=normal_magnitude;
            ...
            std::stringstream ss;
            ss<<"                                            PICKING RANDOM NORMAL !!!!!!!!!!!!!!!!!!!!!!!" <<  std::endl;
            LOG::filecout(ss.str());
#endif
    }}
...
// Function Clip_To_Box
//#####################################################################
template<class T> VECTOR<ARRAY<SEGMENT_2D<T> >,3> SEGMENT_2D<T>::
Clip_To_Outside_Box_Helper(const int axis,const TV& min,const TV& max,const ARRAY<SEGMENT_2D<T> >& simplices) const
{
    VECTOR<ARRAY<SEGMENT_2D<T> >,3> clipped_simplices;
    for(int i=1;i<=simplices.Size();i++)
    {
        const SEGMENT_2D<T>& segment=simplices(i);

        Cut_With_Hyperplane_And_Discard_Outside_Simplices(segment,LINE_2D<T>(TV::Axis_Vector(axis),min),clipped_simplices(1));
        ARRAY<SEGMENT_2D<T> > temp_simplices;
        Cut_With_Hyperplane_And_Discard_Outside_Simplices(segment,LINE_2D<T>(-TV::Axis_Vector(axis),min),temp_simplices);
        for(int j=1;j<=temp_simplices.Size();j++)
            Cut_With_Hyperplane_And_Discard_Outside_Simplices(temp_simplices(j),LINE_2D<T>(TV::Axis_Vector(axis),max),clipped_simplices(2));
        Cut_With_Hyperplane_And_Discard_Outside_Simplices(segment,LINE_2D<T>(-TV::Axis_Vector(axis),max),clipped_simplices(3));
    }
    return clipped_simplices;
}
template<class T> void SEGMENT_2D<T>::
Clip_To_Outside_Box(const RANGE<TV>& box,ARRAY<SEGMENT_2D<T> >& clipped_simplices) const
{
    clipped_simplices.Remove_All();
    ARRAY<SEGMENT_2D<T> > segment;
    segment.Append(*this);
    VECTOR<ARRAY<SEGMENT_2D<T> >,3> segments1=Clip_To_Outside_Box_Helper(1,box.min_corner,box.max_corner,segment);
    for(int axis1=1;axis1<=3;axis1++)
    {
        VECTOR<ARRAY<SEGMENT_2D<T> >,3> segments12=Clip_To_Outside_Box_Helper(2,box.min_corner,box.max_corner,segments1(axis1));
        for(int axis2=1;axis2<=3;axis2++)
            if(!(axis1==2 && axis2==2))
                clipped_simplices.Append_Elements(segments12(axis2));
    }
}
//#####################################################################
...
Clip_To_Box_Helper(T z1,T z2,T& a,T& b,const T& thickness_over_two)
{
    if(z1>z2){z1=1-z1;z2=1-z2;}
    if(z1<-thickness_over_two){
        if(z2<-thickness_over_two) return false;
        a=max(a,z1/(z1-z2));}
    if(z2>1+thickness_over_two){
        if(z1>1+thickness_over_two) return false;
        b=min(b,(z1-1)/(z1-z2));}
    return true;
}
//#####################################################################
...
//#####################################################################
template<class T> bool SEGMENT_2D<T>::
Clip_To_Box(const RANGE<TV>& box,T& a,T& b,const T thickness_over_two) const
{
    TV z1=(x1-box.min_corner)/box.Edge_Lengths();
    TV z2=(x2-box.min_corner)/box.Edge_Lengths();
    TV thickness=TV::Constant_Vector(thickness_over_two)/box.Edge_Lengths();
    a=0;
    b=1;
    return Clip_To_Box_Helper(z1.x,z2.x,a,b,thickness.x) && Clip_To_Box_Helper(z1.y,z2.y,a,b,thickness.y) && a<b;
}
//#####################################################################
