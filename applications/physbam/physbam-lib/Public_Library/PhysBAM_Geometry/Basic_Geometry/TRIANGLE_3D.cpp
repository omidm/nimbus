...
//#####################################################################
// Function Clip_To_Outside_Box
//#####################################################################
template<class T> VECTOR<ARRAY<TRIANGLE_3D<T> >,3> TRIANGLE_3D<T>::
Clip_To_Outside_Box_Helper(const int axis,const TV& min,const TV& max,const ARRAY<TRIANGLE_3D<T> >& simplices) const
{
    VECTOR<ARRAY<TRIANGLE_3D<T> >,3> clipped_simplices;
    for(int i=1;i<=simplices.Size();i++)
    {
        const TRIANGLE_3D<T>& triangle=simplices(i);

        Cut_With_Hyperplane_And_Discard_Outside_Simplices(triangle,PLANE<T>(TV::Axis_Vector(axis),min),clipped_simplices(1));
        ARRAY<TRIANGLE_3D<T> > temp_simplices;
        Cut_With_Hyperplane_And_Discard_Outside_Simplices(triangle,PLANE<T>(-TV::Axis_Vector(axis),min),temp_simplices);
        for(int j=1;j<=temp_simplices.Size();j++)
            Cut_With_Hyperplane_And_Discard_Outside_Simplices(temp_simplices(j),PLANE<T>(TV::Axis_Vector(axis),max),clipped_simplices(2));
        Cut_With_Hyperplane_And_Discard_Outside_Simplices(triangle,PLANE<T>(-TV::Axis_Vector(axis),max),clipped_simplices(3));
    }
    return clipped_simplices;
}
template<class T> void TRIANGLE_3D<T>::
Clip_To_Outside_Box(const RANGE<TV>& box,ARRAY<TRIANGLE_3D<T> >& clipped_simplices) const
{
    clipped_simplices.Remove_All();
    ARRAY<TRIANGLE_3D<T> > triangle;
    triangle.Append(*this);
    VECTOR<ARRAY<TRIANGLE_3D<T> >,3> triangles1=Clip_To_Outside_Box_Helper(1,box.min_corner,box.max_corner,triangle);
    for(int axis1=1;axis1<=3;axis1++)
    {
        VECTOR<ARRAY<TRIANGLE_3D<T> >,3> triangles12=Clip_To_Outside_Box_Helper(2,box.min_corner,box.max_corner,triangles1(axis1));
        for(int axis2=1;axis2<=3;axis2++)
        {
            VECTOR<ARRAY<TRIANGLE_3D<T> >,3> triangles123=Clip_To_Outside_Box_Helper(3,box.min_corner,box.max_corner,triangles12(axis2));
            for(int axis3=1;axis3<=3;axis3++)
                if(!(axis1==2 && axis2==2 && axis3==2))
                    clipped_simplices.Append_Elements(triangles123(axis3));
        }
    }
}
//#####################################################################
