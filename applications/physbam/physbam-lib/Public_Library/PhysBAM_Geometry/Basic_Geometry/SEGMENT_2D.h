...
public:
    ...
    template<class T_ARRAY>
    static TV Center(const T_ARRAY& X_input)
    {return (T).5*(X_input(1)+X_input(2));}
...
//        const T collision_thickness,T& collision_time,TV& normal,T& a,T& b,T& relative_speed,const T small_number=0) const;
    ...
    VECTOR<ARRAY<SEGMENT_2D<T> >,3> Clip_To_Outside_Box_Helper(const int axis,const TV& min,const TV& max,const ARRAY<SEGMENT_2D<T> >& simplices) const;
    void Clip_To_Outside_Box(const RANGE<TV>& box,ARRAY<SEGMENT_2D<T> >& clipped_simplices) const;
    ...
    bool Clip_To_Box(const RANGE<TV>& box,T& a,T& b,const T thickness_over_two=0) const;
//#####################################################################
};

template<class T> SEGMENT_2D<T> operator*(const FRAME<VECTOR<T,2> >& frame,const SEGMENT_2D<T>& segment)
{return SEGMENT_2D<T>(frame*segment.x1,frame*segment.x2);}
