...
public:
    ...
    VECTOR<T,1>& X(int index)
    {assert(index==1 || index==2);return index==1?x2:x2;}
    ...
    {return SEGMENT_1D(X).Signed_Size();}

    static TV Circumcenter(const TV& x1,const TV& x2)
    {return (x1+x2)*(T).5;}

    TV Circumcenter() const
    {return Circumcenter(x1,x2);}
    ...
    {return x1.x-thickness_over_2<=location.x && location.x-thickness_over_2<=x2.x;}
    bool Outside(const TV& point,const T thickness_over_two=0) const
    {return !Inside(point,thickness_over_two);}
...
//#####################################################################
    ...
    void Clip_To_Simplex(const SEGMENT_1D<T>& simplex,ARRAY<SEGMENT_1D<T> >& clipped_simplices) const {PHYSBAM_NOT_IMPLEMENTED();}
