...
public:
    ...
    T Signed_Distance(const TV& location) const // not the true signed distance, but has correct inside/outside
    {return Approximate_Signed_Distance(location);}

    VECTOR<T,2> Principal_Curvatures(const TV& X) const //this is not correct, can't be used now
    {return VECTOR<T,2>::All_Ones_Vector()/radii.x11;}
