...
//#####################################################################
template<class T> void TRIANGLE_2D<T>::
Clip_To_Simplex(const TRIANGLE_2D<T>& simplex,ARRAY<TRIANGLE_2D<T> >& clipped_simplices) const
{
    // cut with all sides of box
    clipped_simplices.Remove_All();
    clipped_simplices.Append(*this);
    for(int side=1;side<=3;side++){
        SEGMENT_2D<T> segment(simplex.X(side),simplex.X(side%3+1));
        TV normal=segment.Normal();
        if(TV::Dot_Product(normal,simplex.X((side+1)%3+1)-simplex.X(side))>0)
            normal*=-1;
        for(int index=clipped_simplices.Size();index>=1;index--)
        {
            Cut_With_Hyperplane_And_Discard_Outside_Simplices(clipped_simplices(index),LINE_2D<T>(normal,simplex.X(side)),clipped_simplices);
