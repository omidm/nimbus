...
template <class T> T Area_Helper(const POLYGON<VECTOR<T,3> >& p)
{
    ...
    //should project into 2d and take signed areas for concave polygons
    T area=0;
    for(int k=2;k<p.X.m;k++)
        area+=TRIANGLE_3D<T>::Area(p.X(1),p.X(k),p.X(k+1));
    return area;
}
};
...
{
    assert(p.X.Size()==2);
    return (p.X(1).x <= X_point.x && X_point.x <= p.X(2).x) || (p.X(1).x >= X_point.x && X_point.x >= p.X(2).x);
}
...
// Function Center
//#####################################################################
template<class TV> TV POLYGON<TV>::Center() const
{
    TV centroid;
    for(int i=2;i<X.Size();i++)
        centroid+=TV::Cross_Product(X(1)-X(0),X(2)-X(0)).Magnitude()/6;
    return centroid;
}
//#####################################################################
