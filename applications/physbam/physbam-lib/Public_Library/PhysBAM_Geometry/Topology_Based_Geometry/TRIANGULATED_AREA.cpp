...
Check_Signed_Area_And_Make_Consistent(const int triangle,const bool verbose)
{
    ...
    if(TRIANGLE_2D<T>::Signed_Area(particles.X(nodes[1]),particles.X(nodes[2]),particles.X(nodes[3])) < 0){
        ...
        if(verbose){std::stringstream ss;ss<<"triangle number "<<triangle<<" is oriented improperly."<<std::endl;LOG::filecout(ss.str());}
#endif
...
//#####################################################################
template<class T> template<class T2> bool TRIANGULATED_AREA<T>::
Barycentric_Interpolate(const VECTOR<T,2>& x,ARRAY_VIEW<T2> values,T2& result)const
{
    int element_index=Inside(x);
    if(element_index==0) return false;
    VECTOR<T,3> weights=TRIANGLE_2D<T>::Barycentric_Coordinates(x,particles.X.Subset(mesh.elements(element_index)));
    INDIRECT_ARRAY<ARRAY_VIEW<T2>,VECTOR<int,3>&> sub_values=values.Subset(mesh.elements(element_index));
    result=weights(1)*sub_values(1)+weights(2)*sub_values(2)+weights(3)*sub_values(3);
    return true;
}
//#####################################################################
template class TRIANGULATED_AREA<float>;
template bool TRIANGULATED_AREA<float>::Barycentric_Interpolate(const VECTOR<float,2>&,ARRAY_VIEW<float>,float&)const;
template bool TRIANGULATED_AREA<float>::Barycentric_Interpolate(const VECTOR<float,2>&,ARRAY_VIEW<VECTOR<float,2> >,VECTOR<float,2>&)const;
#ifndef COMPILE_WITHOUT_DOUBLE_SUPPORT
template class TRIANGULATED_AREA<double>;
template bool TRIANGULATED_AREA<double>::Barycentric_Interpolate(const VECTOR<double,2>&,ARRAY_VIEW<double>,double&)const;
template bool TRIANGULATED_AREA<double>::Barycentric_Interpolate(const VECTOR<double,2>&,ARRAY_VIEW<VECTOR<double,2> >,VECTOR<double,2>&)const;
#endif
