...
Inside(const TV& location,T thickness_over_two) const
{
    ...
    for(int i=1;i<=mesh.elements.m;i++){
        ...
        if(segment.Inside(location,thickness_over_two)) return true;
    ...
    const bool update_bounding_boxes)
{
    if(Segment_Segment_Intersection(test_segment_mesh,X,thickness_over_2,update_bounding_boxes)){
        ...
        std::stringstream ss;
        ss<<"SELF INTERSECTIONS !"<<std::endl;
        LOG::filecout(ss.str());
#endif
        return true;}
    else{
...
#ifndef COMPILE_WITHOUT_READ_WRITE_SUPPORT
                std::stringstream ss;
                ss<<"collision at a proximity < "<<distance<<std::endl;
                LOG::filecout(ss.str());
#endif
                return true;}
#ifndef COMPILE_WITHOUT_READ_WRITE_SUPPORT
            else{std::stringstream ss;ss<<"ok at a proximity = "<<distance<<std::endl;LOG::filecout(ss.str());}
#endif
    }}
    return false;
}
