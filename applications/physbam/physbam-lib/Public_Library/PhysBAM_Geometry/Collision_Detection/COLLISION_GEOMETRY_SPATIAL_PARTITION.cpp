...
Compute_Voxel_Size(const SPATIAL_PARTITION_VOXEL_SIZE_HEURISTIC heuristic,const int number_of_boxes,const T voxel_size_scale_factor)
{
    ...
    std::stringstream ss;
    ss<<"Collision Body Spatial Partition voxel size="<<voxel_size<<" heuristic="<<heuristic<<" # of boxes="<<number_of_boxes<<" voxel_size_scale_factor="<<voxel_size_scale_factor<<std::endl;
    LOG::filecout(ss.str());
#endif
}
//#####################################################################
