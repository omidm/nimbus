...
Set_External_Velocities(ARRAY_VIEW<TV> V,ARRAY_VIEW<T_SPIN> angular_velocity,const T velocity_time,const T current_position_time)
{
    for(int i=1;i<=rigid_geometry_collection.kinematic_rigid_geometry.m;i++){int p=rigid_geometry_collection.kinematic_rigid_geometry(i);Set_External_Velocities(V(p),angular_velocity(p),velocity_time,p);}
    rigid_geometry_collection.rigid_geometry_example_velocities->Set_External_Velocities(V,angular_velocity,velocity_time,current_position_time);
}
//#####################################################################
// Function Set_External_Velocities
//#####################################################################
...
{
    for(int i=1;i<=rigid_geometry_collection.kinematic_rigid_geometry.m;i++){int p=rigid_geometry_collection.kinematic_rigid_geometry(i);Set_External_Velocities(twist(p).linear,twist(p).angular,velocity_time,p);}
...
Set_External_Velocities(TV& V,T_SPIN& angular_velocity,const T time,const int id)
{
    ...
    if(rigid_geometry.is_static){V=TV();angular_velocity=T_SPIN();return;}
    if(!use_kinematic_keyframes){Set_Kinematic_Velocities(V,angular_velocity,(T)1e-3,time,id);return;} // Use 1e-3 for backward differencing if the example did not implement velocity calculations.
    ...
    V=interpolated_state.twist.linear;
    angular_velocity=interpolated_state.twist.angular;
}
//#####################################################################
...
{
    RIGID_GEOMETRY<TV>* rigid_geometry=&rigid_geometry_collection.Rigid_Geometry(id);
    if(rigid_geometry_collection.rigid_geometry_example_velocities->Set_Kinematic_Velocities(V,angular_velocity,time,id)) return;
    ...
    V=current_state.twist.linear;
    angular_velocity=current_state.twist.angular;
}
//#####################################################################
...
Get_Current_Kinematic_Keyframes(const T dt,const T time)
{
    ...
    for(int i=1;i<=rigid_geometry_collection.kinematic_rigid_geometry.m;i++){int p=rigid_geometry_collection.kinematic_rigid_geometry(i);
        ...
        Set_Kinematic_Velocities(kinematic_current_state(p).twist.linear,kinematic_current_state(p).twist.angular,dt,time,p);
        Set_Kinematic_Velocities(kinematic_next_state(p).twist.linear,kinematic_next_state(p).twist.angular,dt,time+dt,p);}
}
//#####################################################################
...
Set_External_Positions(TV& X,ROTATION<TV>& rotation,const T time,const int id)
{
    ...
    if(!use_kinematic_keyframes){
        FRAME<TV> frame;if(rigid_geometry_collection.rigid_geometry_example_velocities->Set_Kinematic_Positions(frame,time,new_id)){X=frame.t;rotation=frame.r;}return;}
    RIGID_GEOMETRY_STATE<TV> interpolated_state;
...
Reset_Kinematic_Rigid_Bodies(const T time)
{
    ...
    for(int i=1;i<=rigid_geometry_collection.kinematic_rigid_geometry.m;i++){
        ...
        Set_External_Velocities(rigid_geometry.V(),rigid_geometry.Angular_Velocity(),time,rigid_geometry.particle_index);}
}
//#####################################################################
