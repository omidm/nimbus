...
{
    Intersection_List(FRAME<TV>(),FRAME<TV>(),other_hierarchy,visitor,self_box,other_box,extra_thickness);
}
//#####################################################################
// Function Intersection_List
//#####################################################################
...
Intersection_List(const FRAME<TV>& frame,const FRAME<TV>& other_frame,const BOX_HIERARCHY<TV>& other_hierarchy,T_VISITOR& visitor,const T_THICKNESS extra_thickness) const
{
    ...
    if(&other_hierarchy != this || frame != other_frame) return Intersection_List(frame,other_frame,other_hierarchy,visitor,root,other_hierarchy.root,extra_thickness);
