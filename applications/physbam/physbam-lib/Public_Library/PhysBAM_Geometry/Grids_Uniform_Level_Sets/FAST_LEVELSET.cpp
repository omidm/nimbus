...
//#####################################################################
template<class T_GRID> void FAST_LEVELSET<T_GRID>::
Fast_Marching_Method(const T stopping_distance,const int local_advection_spatial_order, const T time)
{
    T_LEVELSET::Fast_Marching_Method(time,stopping_distance);
    boundary->Apply_Boundary_Condition(grid,phi,time); // time not incremented - pseudo-time
}
//#####################################################################
