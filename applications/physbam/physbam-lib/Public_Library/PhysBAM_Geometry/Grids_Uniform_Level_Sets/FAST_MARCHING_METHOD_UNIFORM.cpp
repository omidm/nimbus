        ...
        if(levelset.collision_body_list)
            ...
            for(int axis=1;axis<=T_GRID::dimension;axis++){TV_INT axis_vector=TV_INT::Axis_Vector(axis);
                ...
                if(index[axis] != dimension_end[axis] && !done(index+axis_vector))
                    Update_Or_Add_Neighbor(phi_ghost,done,close_k,heap,heap_length,index+axis_vector);}}*/
    ARRAY<T_ARRAYS_SCALAR> phi_new_array(thread_queue?thread_queue->Number_Of_Threads():1);
    DOMAIN_ITERATOR_THREADED_ALPHA<FAST_MARCHING_METHOD_UNIFORM<T_GRID>,TV> threaded_iterator(cell_grid.Domain_Indices(ghost_cells),thread_queue,1,ghost_cells,2,1);
    threaded_iterator.template Run<T_ARRAYS_SCALAR&,ARRAY<T_ARRAYS_SCALAR>&>(*this,&FAST_MARCHING_METHOD_UNIFORM<T_GRID>::Fast_Marching_Method_Threaded_Part_One,phi_ghost,phi_new_array);
    threaded_iterator.template Run<T_ARRAYS_SCALAR&,ARRAY<T_ARRAYS_SCALAR>&,T,const ARRAY<TV_INT>*,bool>(*this,&FAST_MARCHING_METHOD_UNIFORM<T_GRID>::Fast_Marching_Method_Threaded_Part_Two,phi_ghost,phi_new_array,stopping_distance,seed_indices,add_seed_indices_for_ghost_cells);
}
//#####################################################################
// Function Fast_Marching_Method
//#####################################################################
template<class T_GRID> void FAST_MARCHING_METHOD_UNIFORM<T_GRID>::
Fast_Marching_Method_Threaded_Part_One(int tid,RANGE<TV_INT>& domain,T_ARRAYS_SCALAR& phi_ghost,ARRAY<T_ARRAYS_SCALAR>& phi_new_array)
{
    phi_new_array(tid).Resize(domain);
    for(CELL_ITERATOR iterator(cell_grid,domain);iterator.Valid();iterator.Next()){TV_INT cell=iterator.Cell_Index();phi_new_array(tid)(cell)=phi_ghost(cell);}
}
template<class T_GRID> void FAST_MARCHING_METHOD_UNIFORM<T_GRID>::
Fast_Marching_Method_Threaded_Part_Two(int tid,RANGE<TV_INT>& domain,T_ARRAYS_SCALAR& phi_ghost,ARRAY<T_ARRAYS_SCALAR>& phi_new_array,const T stopping_distance,const ARRAY<TV_INT>* seed_indices,const bool add_seed_indices_for_ghost_cells)
{   
    int heap_length=0;
    T_ARRAYS_SCALAR& phi_new=phi_new_array(tid);
    ...
    while(heap_length != 0){
        ...
        if(stopping_distance && abs(phi_new(index)) > stopping_distance){ // exit early
            for(CELL_ITERATOR iterator(cell_grid,interior_domain);iterator.Valid();iterator.Next()){TV_INT cell=iterator.Cell_Index();
                if(!done(cell)) phi_ghost(cell)=LEVELSET_UTILITIES<T>::Sign(phi_new(cell))*stopping_distance;
                else phi_ghost(cell)=phi_new(cell);}
            return;}
        done(index)=true;close_k(index)=0; // add to done, remove from close
    ...
    if(heap_length ==0 && stopping_distance){
        for(CELL_ITERATOR iterator(cell_grid,interior_domain);iterator.Valid();iterator.Next()){
            if(!done(iterator.Cell_Index())){
