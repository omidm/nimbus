...
struct ALLOCATE_GEOMETRY_HELPER:public ALLOCATE_HELPER<TV>
{
    ...
    RIGID_GEOMETRY<TV>* Create(int index=0) PHYSBAM_OVERRIDE {return new RIGID_GEOMETRY<TV>(collection,true,index);}
...
Add_Rigid_Geometry(const STREAM_TYPE stream_type,const std::string& basename,const T scaling_factor,const bool read_simplicial_boundary,const bool read_implicit_object,const bool read_simplicial_interior,const bool read_rgd_file)
{
...
//#####################################################################
template<class TV> int RIGID_GEOMETRY_COLLECTION<TV>::
Add_Rigid_Geometry(RIGID_GEOMETRY<TV>* rigid_geometry,const STREAM_TYPE stream_type,const std::string& basename,const T scaling_factor,
    const bool read_simplicial_boundary,const bool read_implicit_object,const bool read_simplicial_interior,const bool read_rgd_file)
{
    ...
    if(TV::dimension==2){
        ...
        if(read_simplicial_interior && !Find_Or_Read_Structure(stream_type,structure_ids,basename+".tri2d",scaling_factor,structure_center))
            ss<<"Note: No tri2d file for "<<basename<<std::endl;}
    else{
        if(read_simplicial_boundary && !Find_Or_Read_Structure(stream_type,structure_ids,basename+".tri",scaling_factor,structure_center))
            ss<<"Note: No tri file for "<<basename<<std::endl;
        if(read_implicit_object && !Find_Or_Read_Structure(stream_type,structure_ids,basename+".phi",scaling_factor,structure_center) && 
            ...
            ss<<"Note: No phi or oct file for "<<basename<<std::endl;
        if(read_simplicial_interior && !Find_Or_Read_Structure(stream_type,structure_ids,basename+".tet",scaling_factor,structure_center))
            ss<<"Note: No tet file for "<<basename<<std::endl;}
    LOG::filecout(ss.str());
...
//#####################################################################
template<class TV> int RIGID_GEOMETRY_COLLECTION<TV>::
Add_Rigid_Geometry(RIGID_GEOMETRY<TV>* rigid_geometry)
{
    int id=rigid_geometry->particle_index;
    assert(rigid_geometry->structures.m<=3);
    particles.structure_ids(id)=VECTOR<int,3>();
    for(int s=1;s<=rigid_geometry->structures.m;s++) particles.structure_ids(id)(s)=structure_list.Add_Element(rigid_geometry->structures(s));
    return id;
}
//#####################################################################
...
Destroy_Unreferenced_Geometry() 
{
    ...
    for(int i=1;i<=particles.array_collection->Size();i++) for(int j=1;j<=particles.structure_ids(i).m;j++) if(particles.structure_ids(i)(j) && structure_list.Element_Index(particles.structure_ids(i)(j)))
