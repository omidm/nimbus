...
#include <PhysBAM_Rendering/PhysBAM_OpenGL/OpenGL_Components/OPENGL_COMPONENT_CHIMERA_GRID_3D.h>
...
#include <PhysBAM_Rendering/PhysBAM_OpenGL_Dynamics/OpenGL_Dynamics_Components/OPENGL_COMPONENT_VORONOI_3D.h>
#include <fstream>
#include <sstream>
#include <string.h>
...
private:
    ...
    OPENGL_COMPONENT_CHIMERA_GRID_3D<T>* chimera_grid_component;
    ...
    ARRAY<GRID<TV> > chimera_grid,chimera_mac_grid,chimera_regular_grid;
    bool has_valid_grid,has_valid_rle_grid,has_valid_octree_grid,has_valid_coarse_grid,has_valid_chimera_grid;
    bool node_based,coarse_node_based,chimera_node_based;
...
Read_Grid()
{
    ...
    std::string chimera_filename=STRING_UTILITIES::string_sprintf("%s/%d/chimera_grid",basedir.c_str(),start_frame);
    ...
    if(FILE_UTILITIES::File_Exists(coarse_filename)){
        ...
        std::stringstream ss;ss<<"Reading coarse grid from '"<<coarse_filename<<"'..."<<std::flush;LOG::filecout(ss.str());
    ...
    std::string chimera_filename_suffix=STRING_UTILITIES::string_sprintf("_%d",number_of_grids+1);
    while(FILE_UTILITIES::File_Exists(chimera_filename+chimera_filename_suffix)){
        number_of_grids++;
        chimera_filename_suffix=STRING_UTILITIES::string_sprintf("_%d",number_of_grids+1);}
    if(number_of_grids>=1) has_valid_chimera_grid=true;
    else has_valid_chimera_grid=false;
    chimera_grid.Resize(number_of_grids);
    chimera_mac_grid.Resize(number_of_grids);
    chimera_regular_grid.Resize(number_of_grids);
    std::cout<<number_of_grids<<std::endl;
    for(int i=1;i<=number_of_grids;i++){
        chimera_filename_suffix=STRING_UTILITIES::string_sprintf("_%d",i); 
        std::cout<<"Reading chimera grid from '"<<chimera_filename+chimera_filename_suffix<<"'..."<<std::endl<<std::flush;
        std::string chimera_filename_full=chimera_filename+chimera_filename_suffix;
        FILE_UTILITIES::Read_From_File<RW>(chimera_filename_full,chimera_grid(i));}  
    ...
    if(has_valid_chimera_grid){
        chimera_node_based=!chimera_grid(1).Is_MAC_Grid();
        for(int i=1;i<=chimera_grid.Size();i++){
            chimera_mac_grid(i)=chimera_grid(i).Get_MAC_Grid();
            chimera_regular_grid(i)=chimera_grid(i).Get_Regular_Grid();}}
}
...
Initialize_Components_And_Key_Bindings()
{
    ...
    if(has_valid_chimera_grid){
        chimera_slice=new OPENGL_CHIMERA_SLICE<T>(opengl_world);chimera_slice->Initialize(chimera_grid);
        slice_manager.slice=chimera_slice;
        std::cout<<"Using Chimera grid slice"<<std::endl;}
...
//        levelset_component->Set_Surface_Material(OPENGL_MATERIAL::Plastic(OPENGL_COLOR((T).6,(T).65,1)),OPENGL_MATERIAL::Plastic(OPENGL_COLOR((T).6,(T).65,1)));
    ...
    if(FILE_UTILITIES::Frame_File_Exists(coarse_filename,start_frame)){
        ...
        levelset_component->Set_Surface_Material(OPENGL_MATERIAL::Plastic(OPENGL_COLOR(.6f,1.f,.65f)),OPENGL_MATERIAL::Plastic(OPENGL_COLOR(.6f,1.f,.65f)));
        if(slice_manager.slice) slice_manager.Add_Object(levelset_component);
    ...
    if(has_valid_chimera_grid && FILE_UTILITIES::Frame_File_Exists(basedir+"/%d/chimera_levelset_1",start_frame)){
        OPENGL_COMPONENT_LEVELSET_3D<T>* chimera_levelset_component=new OPENGL_COMPONENT_LEVELSET_3D<T>("/%d/chimera_levelset_1","",basedir+"/%d/chimera_levelset_%d","",true,true,true,basedir+"/%d/rigid_grid_frame_%d",true);
        opengl_world.Set_Key_Binding_Category("Chimera Levelset");
        Add_Component(chimera_levelset_component,"Chimera Levelset",'l',BASIC_VISUALIZATION::OWNED);
        //chimera_levelset_component->Set_Surface_Material(OPENGL_MATERIAL::Plastic(OPENGL_COLOR((T).6,(T).65,1)),OPENGL_MATERIAL::Plastic(OPENGL_COLOR((T).6,(T).65,1)));
        opengl_world.Append_Bind_Key('L',chimera_levelset_component->Toggle_Slice_Color_Mode_CB());
        opengl_world.Append_Bind_Key("^l",chimera_levelset_component->Toggle_Display_Overlay_CB());
        opengl_world.Append_Bind_Key('`',chimera_levelset_component->Toggle_Smooth_Slice_CB());
        opengl_world.Append_Bind_Key('M',chimera_levelset_component->Toggle_Draw_Multiple_Levelsets_CB());
        opengl_world.Append_Bind_Key('>',chimera_levelset_component->Next_Set_CB());
        opengl_world.Append_Bind_Key('<',chimera_levelset_component->Previous_Set_CB());
        chimera_levelset_component->Set_Chimera_Slice(chimera_slice);
        slice_manager.Add_Object(chimera_levelset_component);}
    ...
    filename=basedir+"/%d/density_1";
    if(has_valid_chimera_grid && FILE_UTILITIES::Frame_File_Exists(filename,start_frame)){
        OPENGL_COMPONENT_CHIMERA_SCALAR_FIELD_3D<T>* chimera_density_component=new OPENGL_COMPONENT_CHIMERA_SCALAR_FIELD_3D<T>(chimera_grid,basedir+"/%d/density_%d",OPENGL_COLOR_RAMP<T>::Two_Color_Ramp(0,1,OPENGL_COLOR::Gray(0,0),OPENGL_COLOR::Gray(1,1)),true,basedir+"/%d/rigid_grid_frame_%d");
        opengl_world.Set_Key_Binding_Category("Density");
        Add_Component(chimera_density_component,"Chimera density",'d',BASIC_VISUALIZATION::OWNED);
        opengl_world.Append_Bind_Key('D',chimera_density_component->Toggle_Color_Map_CB());
        opengl_world.Append_Bind_Key(OPENGL_KEY(OPENGL_KEY::F8),chimera_density_component->Toggle_Draw_Mode_CB());
        opengl_world.Append_Bind_Key('`',chimera_density_component->Toggle_Smooth_Slice_CB());
        opengl_world.Append_Bind_Key('>',chimera_density_component->Next_Grid_CB());
        opengl_world.Append_Bind_Key('<',chimera_density_component->Previous_Grid_CB());
        opengl_world.Append_Bind_Key('M',chimera_density_component->Toggle_Draw_All_Grids_CB());
        slice_manager.Add_Object(chimera_density_component);
        chimera_density_component->Set_Chimera_Slice(chimera_slice);}
    ...
    if(has_valid_grid && FILE_UTILITIES::Frame_File_Exists(filename,start_frame)){
        OPENGL_COMPONENT_SCALAR_FIELD_3D<T>* soot_component=new OPENGL_COMPONENT_SCALAR_FIELD_3D<T>(grid,filename,OPENGL_COLOR_RAMP<T>::Two_Color_Ramp(0,(T).01,OPENGL_COLOR::Gray(0,0),OPENGL_COLOR::Gray(1,1)));
    ...
    if(has_valid_grid && FILE_UTILITIES::Frame_File_Exists(filename,start_frame)){
        OPENGL_COMPONENT_SCALAR_FIELD_3D<T>* soot_fuel_component=new OPENGL_COMPONENT_SCALAR_FIELD_3D<T>(grid,filename,OPENGL_COLOR_RAMP<T>::Two_Color_Ramp(0,(T).01,OPENGL_COLOR::Gray(0,0),OPENGL_COLOR::Gray(1,1)));
    ...
    if(has_valid_grid && FILE_UTILITIES::Frame_File_Exists(filename,start_frame)){
        ...
        T min_temp=temp0.Min(),max_temp=temp0.Max();min_temp=(T)283.15;
    ...
    { // regular and mac velocity TODO: kill node based stuff?
        ...
        OPENGL_COMPONENT_CHIMERA_MAC_VELOCITY_FIELD_3D<T>* chimera_mac_velocity_component=0;
        filename=basedir+"/%d/mac_velocities_1";
        if(has_valid_chimera_grid && FILE_UTILITIES::Frame_File_Exists(filename,start_frame)){
            chimera_mac_velocity_component=new OPENGL_COMPONENT_CHIMERA_MAC_VELOCITY_FIELD_3D<T>(chimera_mac_grid,basedir+"/%d/mac_velocities_%d",true,basedir+"/%d/rigid_grid_frame_%d");
            for(int i=1;i<=chimera_mac_velocity_component->opengl_mac_velocity_fields.m;i++){
                chimera_mac_velocity_component->opengl_mac_velocity_fields(i)->size=.01;
                chimera_mac_velocity_component->opengl_mac_velocity_fields(i)->vector_color=OPENGL_COLOR::Magenta();
                chimera_mac_velocity_component->opengl_mac_velocity_fields(i)->Set_Velocity_Mode(OPENGL_MAC_VELOCITY_FIELD_3D<T>::FACE_CENTERED);}
            Add_Component(chimera_mac_velocity_component,"Chimera MAC velocities",'\0',BASIC_VISUALIZATION::OWNED|BASIC_VISUALIZATION::START_HIDDEN);
            slice_manager.Add_Object(chimera_mac_velocity_component);
            chimera_mac_velocity_component->Set_Chimera_Slice(chimera_slice);}
        if(chimera_mac_velocity_component){
            opengl_world.Append_Bind_Key('C',chimera_mac_velocity_component->Toggle_Draw_CB());
            opengl_world.Append_Bind_Key('V',chimera_mac_velocity_component->Toggle_Velocity_Mode_And_Draw_CB());
            opengl_world.Append_Bind_Key('=',chimera_mac_velocity_component->Increase_Vector_Size_CB());
            opengl_world.Append_Bind_Key('-',chimera_mac_velocity_component->Decrease_Vector_Size_CB());
            opengl_world.Append_Bind_Key('h',chimera_mac_velocity_component->Toggle_Arrowhead_CB());
            opengl_world.Append_Bind_Key('v',chimera_mac_velocity_component->Toggle_Draw_Vorticity_CB());
            opengl_world.Append_Bind_Key('N',chimera_mac_velocity_component->Normalize_Vorticity_Color_Map_CB());
            opengl_world.Append_Bind_Key('<',chimera_mac_velocity_component->Previous_Grid_CB());
            opengl_world.Append_Bind_Key('>',chimera_mac_velocity_component->Next_Grid_CB());
            opengl_world.Append_Bind_Key('M',chimera_mac_velocity_component->Toggle_Draw_All_Grids_CB());}
    ...
    if(has_valid_grid && FILE_UTILITIES::Frame_File_Exists(filename,start_frame)){
        ...
        pseudo_dirichlet_component->Set_Vector_Size((T)0.1);
    ...
    /*if(has_valid_chimera_grid){
        chimera_grid_component=new OPENGL_COMPONENT_CHIMERA_GRID_3D<T>(basedir+"/%d/chimera_grid_%d",true,basedir+"/%d/rigid_grid_frame_%d");
        Add_Component(chimera_grid_component,"Chimera Grid",'6',BASIC_VISUALIZATION::START_HIDDEN|BASIC_VISUALIZATION::SELECTABLE);
        opengl_world.Append_Bind_Key('>',chimera_grid_component->Next_Grid_CB());
        opengl_world.Append_Bind_Key('<',chimera_grid_component->Previous_Grid_CB());
        opengl_world.Append_Bind_Key('M',chimera_grid_component->Toggle_Draw_All_Grids_CB());
        opengl_world.Append_Bind_Key('G',chimera_grid_component->Toggle_Draw_Ghost_Values_CB());
        chimera_grid_component->Set_Chimera_Slice(chimera_slice);
        slice_manager.Add_Object(chimera_grid_component);
        std::string voronoi_filename=basedir+"/%d/voronoi";
        if(FILE_UTILITIES::Frame_File_Exists(voronoi_filename,start_frame))
        {
            OPENGL_COMPONENT_VORONOI_3D<T>* voronoi_component=new OPENGL_COMPONENT_VORONOI_3D<T>(voronoi_filename,chimera_grid_component->opengl_grids);
            Add_Component(voronoi_component,"Voronoi Geometry",'I',BASIC_VISUALIZATION::SELECTABLE);
            opengl_world.Append_Bind_Key('>',voronoi_component->Next_Grid_CB());
            opengl_world.Append_Bind_Key('<',voronoi_component->Previous_Grid_CB());
            opengl_world.Append_Bind_Key('M',voronoi_component->Toggle_Draw_All_Grids_CB());
            voronoi_component->Set_Chimera_Slice(chimera_slice);
        }
    }*/
...
#endif
        if(has_valid_grid || has_valid_chimera_grid) particles_stored_per_cell_uniform=true;
    ...
    if(!has_valid_chimera_grid){
        ...
        filename=basedir+"/%d/positive_particles";
        if(FILE_UTILITIES::Frame_File_Exists(filename,start_frame)||FILE_UTILITIES::Frame_File_Exists(basedir+"/%d/positive_particles_1",start_frame)){
            positive_particles_component=new OPENGL_COMPONENT_PARTICLES_3D<T,PARTICLE_LEVELSET_PARTICLES<TV> >(filename,basedir+"/%d/positive_particles_%d",true,particles_stored_per_cell_uniform,particles_stored_per_cell_adaptive,true,true,basedir+"/%d/rigid_grid_frame_%d");
            positive_particles_component->opengl_points->color=OPENGL_COLOR(1,0.5,0);
            positive_particles_component->opengl_points->point_size=2;
            Add_Component(positive_particles_component,"Positive particles",'1',BASIC_VISUALIZATION::OWNED|BASIC_VISUALIZATION::START_HIDDEN|BASIC_VISUALIZATION::SELECTABLE);
            opengl_world.Append_Bind_Key('!',positive_particles_component->Toggle_Draw_Point_Numbers_CB());
            ...
            positive_particles_component->Set_Chimera_Slice(chimera_slice);
            slice_manager.Add_Object(positive_particles_component);}
        
        filename=basedir+"/%d/negative_particles";
        if(FILE_UTILITIES::Frame_File_Exists(filename,start_frame)||FILE_UTILITIES::Frame_File_Exists(basedir+"/%d/negative_particles_1",start_frame)){
            negative_particles_component=new OPENGL_COMPONENT_PARTICLES_3D<T,PARTICLE_LEVELSET_PARTICLES<TV> >(filename,basedir+"/%d/negative_particles_%d",true,particles_stored_per_cell_uniform,particles_stored_per_cell_adaptive,true,true,basedir+"/%d/rigid_grid_frame_%d");
            negative_particles_component->opengl_points->color=OPENGL_COLOR(0,0.5,1);
            negative_particles_component->opengl_points->point_size=2;
            Add_Component(negative_particles_component,"Negative particles",'2',BASIC_VISUALIZATION::OWNED|BASIC_VISUALIZATION::START_HIDDEN|BASIC_VISUALIZATION::SELECTABLE);
            opengl_world.Append_Bind_Key('@',negative_particles_component->Toggle_Draw_Point_Numbers_CB());
            ...
            negative_particles_component->Set_Chimera_Slice(chimera_slice);
            slice_manager.Add_Object(negative_particles_component);}
        
        filename=basedir+"/%d/removed_positive_particles";
        if(FILE_UTILITIES::Frame_File_Exists(filename,start_frame)||FILE_UTILITIES::Frame_File_Exists(basedir+"/%d/removed_positive_particles_1",start_frame)){
            removed_positive_particles_component=new OPENGL_COMPONENT_PARTICLES_3D<T,PARTICLE_LEVELSET_REMOVED_PARTICLES<TV> >(filename,basedir+"/%d/removed_positive_particles_%d",true,particles_stored_per_cell_uniform,particles_stored_per_cell_adaptive,true,true,basedir+"/%d/rigid_grid_frame_%d");
            removed_positive_particles_component->opengl_points->color=OPENGL_COLOR::Green();
            removed_positive_particles_component->opengl_points->point_size=2;
            Add_Component(removed_positive_particles_component,"Removed positive particles",'3',BASIC_VISUALIZATION::OWNED|BASIC_VISUALIZATION::START_HIDDEN|BASIC_VISUALIZATION::SELECTABLE);
            opengl_world.Append_Bind_Key('#',removed_positive_particles_component->Toggle_Draw_Point_Numbers_CB());
            ...
            removed_positive_particles_component->Set_Chimera_Slice(chimera_slice);
            slice_manager.Add_Object(removed_positive_particles_component);}
        
        filename=basedir+"/%d/removed_negative_particles";
        if(FILE_UTILITIES::Frame_File_Exists(filename,start_frame)||FILE_UTILITIES::Frame_File_Exists(basedir+"/%d/removed_negative_particles_1",start_frame)){
            removed_negative_particles_component=new OPENGL_COMPONENT_PARTICLES_3D<T,PARTICLE_LEVELSET_REMOVED_PARTICLES<TV> >(filename,basedir+"/%d/removed_negative_particles_%d",true,particles_stored_per_cell_uniform,particles_stored_per_cell_adaptive,true,true,basedir+"/%d/rigid_grid_frame_%d");
            removed_negative_particles_component->opengl_points->color=OPENGL_COLOR::Cyan();
            removed_negative_particles_component->opengl_points->point_size=2;
            Add_Component(removed_negative_particles_component,"Removed negative particles",'4',BASIC_VISUALIZATION::OWNED|BASIC_VISUALIZATION::START_HIDDEN|BASIC_VISUALIZATION::SELECTABLE);
            opengl_world.Append_Bind_Key('$',removed_negative_particles_component->Toggle_Draw_Point_Numbers_CB());
            ...
            removed_negative_particles_component->Set_Chimera_Slice(chimera_slice);
            slice_manager.Add_Object(removed_negative_particles_component);}}
    ...
    if(FILE_UTILITIES::Frame_File_Exists(filename,start_frame)){
        ...
        spray_particles_component->opengl_points->color=OPENGL_COLOR(.7f,.8f,.9f);
    ...
    filename=basedir+"/%d/fluid_particles";
    if(FILE_UTILITIES::Frame_File_Exists(filename,start_frame)){
        OPENGL_COMPONENT_PARTICLES_3D<T,FLUID_PARTICLES<TV> >* fluid_particles_component=new OPENGL_COMPONENT_PARTICLES_3D<T,FLUID_PARTICLES<TV> >(filename,"",false);
        fluid_particles_component->opengl_points->color=OPENGL_COLOR::Blue();
        fluid_particles_component->opengl_points->point_size=1;
        Add_Component(fluid_particles_component,"Fluid particles",'1',BASIC_VISUALIZATION::OWNED|BASIC_VISUALIZATION::SELECTABLE);
        opengl_world.Append_Bind_Key('J',fluid_particles_component->Toggle_Draw_Point_Numbers_CB());
        opengl_world.Append_Bind_Key('h',fluid_particles_component->Toggle_Arrowhead_CB());
        opengl_world.Append_Bind_Key("*",fluid_particles_component->Toggle_Draw_Velocities_CB());
        opengl_world.Append_Bind_Key('.',fluid_particles_component->Increase_Vector_Size_CB());
        opengl_world.Append_Bind_Key(',',fluid_particles_component->Decrease_Vector_Size_CB());
        if(slice_manager.slice) slice_manager.Add_Object(fluid_particles_component);}
    ...
    if(has_valid_chimera_grid && FILE_UTILITIES::Frame_File_Exists(basedir+"/%d/psi_N_1",start_frame)){
        OPENGL_COMPONENT_CHIMERA_FACE_SCALAR_FIELD_3D<T,bool>* chimera_psi_N_component=new OPENGL_COMPONENT_CHIMERA_FACE_SCALAR_FIELD_3D<T,bool>(chimera_grid,basedir+"/%d/psi_N_%d",new OPENGL_CONSTANT_COLOR_MAP<bool>(OPENGL_COLOR::Cyan()),true,basedir+"/%d/rigid_grid_frame_%d");
        Add_Component(chimera_psi_N_component,"Chimera Psi_N points",'\0',BASIC_VISUALIZATION::START_HIDDEN|BASIC_VISUALIZATION::OWNED);
        opengl_world.Append_Bind_Key(OPENGL_KEY(OPENGL_KEY::F1),chimera_psi_N_component->Toggle_Draw_CB());
        opengl_world.Append_Bind_Key('>',chimera_psi_N_component->Next_Grid_CB());
        opengl_world.Append_Bind_Key('<',chimera_psi_N_component->Previous_Grid_CB());
        opengl_world.Append_Bind_Key('M',chimera_psi_N_component->Toggle_Draw_All_Grids_CB());
        slice_manager.Add_Object(chimera_psi_N_component);
        chimera_psi_N_component->Set_Chimera_Slice(chimera_slice);}
    ...
    filename=basedir+"/%d/psi_D_1";
    if(has_valid_chimera_grid && FILE_UTILITIES::Frame_File_Exists(filename,start_frame)){
        OPENGL_COMPONENT_CHIMERA_SCALAR_FIELD_3D<T,bool>* chimera_psi_D_component=new OPENGL_COMPONENT_CHIMERA_SCALAR_FIELD_3D<T,bool>(chimera_mac_grid,filename=basedir+"/%d/psi_D_%d",new OPENGL_CONSTANT_COLOR_MAP<bool>(OPENGL_COLOR::Magenta()),OPENGL_SCALAR_FIELD_3D<T,bool>::DRAW_POINTS,true,basedir+"/%d/rigid_grid_frame_%d");
        Add_Component(chimera_psi_D_component,"Chimera Psi_D points",'\0',BASIC_VISUALIZATION::START_HIDDEN|BASIC_VISUALIZATION::OWNED);
        opengl_world.Append_Bind_Key(OPENGL_KEY(OPENGL_KEY::F1),chimera_psi_D_component->Toggle_Draw_CB());
        opengl_world.Append_Bind_Key('>',chimera_psi_D_component->Next_Grid_CB());
        opengl_world.Append_Bind_Key('<',chimera_psi_D_component->Previous_Grid_CB());
        opengl_world.Append_Bind_Key('M',chimera_psi_D_component->Toggle_Draw_All_Grids_CB());
        slice_manager.Add_Object(chimera_psi_D_component);
        chimera_psi_D_component->Set_Chimera_Slice(chimera_slice);}
...
{filename=basedir+"/%d/strain";
    ...
    if(FILE_UTILITIES::File_Exists(filename)){
        OPENGL_COMPONENT_MOTION_SEQUENCE<T>* motion_sequence_component=new OPENGL_COMPONENT_MOTION_SEQUENCE<T>(filename,false,basedir+"/motion_sequence_segments.0",(T)frame_rate,OPENGL_COLOR::Cyan());
    ...
    if(FILE_UTILITIES::Frame_File_Exists(filename,start_frame)){
        OPENGL_COMPONENT_MOTION_SEQUENCE<T>* motion_sequence_component1=new OPENGL_COMPONENT_MOTION_SEQUENCE<T>(filename,true,basedir+"/motion_sequence_segments1.0",(T)frame_rate,OPENGL_COLOR::Cyan());
    ...
    if(FILE_UTILITIES::Frame_File_Exists(filename,start_frame)){
        OPENGL_COMPONENT_MOTION_SEQUENCE<T>* motion_sequence_component2=new OPENGL_COMPONENT_MOTION_SEQUENCE<T>(filename,true,basedir+"/motion_sequence_segments2.0",(T)frame_rate,OPENGL_COLOR::Magenta());
