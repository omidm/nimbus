...
private:
    ...
    OPENGL_COMPONENT_SCALAR_FIELD_2D<T>* pressure_component,* coarse_pressure_component, *pressure_water_component;
    OPENGL_COMPONENT_PARTICLES_2D<T,BUBBLE_PARTICLES<TV> >* bubble_particles_component;
...
OPENGL_2D_VISUALIZATION()
    ...
    pressure_component(0),coarse_pressure_component(0),pressure_water_component(0),bubble_particles_component(0),positive_particles_component(0),negative_particles_component(0),
...
Read_Grid()
{
    ...
    if(FILE_UTILITIES::File_Exists(rle_filename)){
        std::stringstream ss;ss<<"Reading rle grid '"<<rle_filename<<"'"<<std::endl<<std::flush;LOG::filecout(ss.str());
    ...
    if(FILE_UTILITIES::File_Exists(quadtree_filename)){
        std::stringstream ss;ss<<"Reading quadtree '"<<quadtree_filename<<"'"<<std::endl<<std::flush;LOG::filecout(ss.str());
    ...
    if(FILE_UTILITIES::File_Exists(filename)){
        std::stringstream ss;ss<<"Reading grid from '"<<filename<<"'..."<<std::endl<<std::flush;LOG::filecout(ss.str());
        ...
        std::stringstream ss;ss<<"Reading grid from '"<<filename<<"'..."<<std::flush;LOG::filecout(ss.str());
        FILE_UTILITIES::Read_From_File<RW>(filename,grid);
        has_valid_grid=true;}

    if(has_valid_grid && FILE_UTILITIES::File_Exists(sub_filename)){
        std::stringstream ss;ss<<"Reading sub grids '"<<sub_filename<<"'"<<std::endl<<std::flush;LOG::filecout(ss.str());
...
Initialize_Components_And_Key_Bindings()
{
    ...
    if(has_valid_grid && FILE_UTILITIES::Frame_File_Exists(filename,start_frame)){
        ...
        density_component->opengl_scalar_field.Set_Uniform_Contour_Values((T).2568,(T)6.067,(T)((6.067-.2568)/30.));
        density_component->opengl_scalar_field.Update();
        filename=basedir+"/%d/weights_i_cell";
        if(FILE_UTILITIES::Frame_File_Exists(filename,start_frame)){
            density_component->weights_i_filename=filename;
            density_component->print_weights_i=true;}
        filename=basedir+"/%d/weights_j_cell";
        if(FILE_UTILITIES::Frame_File_Exists(filename,start_frame)){
            density_component->weights_j_filename=filename;
            density_component->print_weights_j=true;}
    ...
    if(has_valid_grid && FILE_UTILITIES::Frame_File_Exists(filename,start_frame)){
        OPENGL_COMPONENT_SCALAR_FIELD_2D<T>* soot_component=new OPENGL_COMPONENT_SCALAR_FIELD_2D<T>(grid,filename,OPENGL_COLOR_RAMP<T>::Two_Color_Ramp(0,(T).01,OPENGL_COLOR::Gray(0,0),OPENGL_COLOR::Gray(1,1)));
        soot_component->opengl_scalar_field.Set_Uniform_Contour_Values(0,2,.1f);
    ...
    if(has_valid_grid && has_valid_sub_grids && FILE_UTILITIES::Frame_File_Exists(filename,start_frame) && FILE_UTILITIES::Frame_File_Exists(filename2,start_frame)){
        ...
        sub_density_component->Set_Uniform_Contour_Values(0,2,(T).1);
    ...
    if(FILE_UTILITIES::Frame_File_Exists(filename,start_frame)){
        ...
        object_levelset_component->opengl_levelset->outside_color=OPENGL_COLOR::Red(.6f);
        object_levelset_component->opengl_levelset->inside_color=OPENGL_COLOR::Blue(.6f);
    ...
    filename=basedir+"/%d/bubble_particles";
    if(FILE_UTILITIES::Frame_File_Exists(filename,start_frame)){
        std::cout<<"in here-------------------------"<<std::endl;
        bubble_particles_component=new OPENGL_COMPONENT_PARTICLES_2D<T,BUBBLE_PARTICLES<TV> >(filename,basedir+"/%d/bubble_particles_%d",true,particles_stored_per_cell_uniform); 
        bubble_particles_component->particles->array_collection->template Add_Array<int>(ATTRIBUTE_ID_ID);
        bubble_particles_component->opengl_points->color=OPENGL_COLOR(0,1,0);
        Add_Component(bubble_particles_component,"bubble particles",'1',BASIC_VISUALIZATION::OWNED|BASIC_VISUALIZATION::SELECTABLE);}
    filename=basedir+"/%d/fluid_particles";
    if(FILE_UTILITIES::Frame_File_Exists(filename,start_frame)){
        OPENGL_COMPONENT_PARTICLES_2D<T,FLUID_PARTICLES<TV> >* fluid_particles_component=new OPENGL_COMPONENT_PARTICLES_2D<T,FLUID_PARTICLES<TV> >(filename,"",false);
        ...
        opengl_world.Append_Bind_Key('.',fluid_particles_component->Toggle_Increase_Vector_Size_CB());
        opengl_world.Append_Bind_Key(',',fluid_particles_component->Toggle_Decrease_Vector_Size_CB());}
    filename=basedir+"/%d/fluid_particles_1";
    if(FILE_UTILITIES::Frame_File_Exists(filename,start_frame)){
        OPENGL_COMPONENT_PARTICLES_2D<T,FLUID_PARTICLES<TV> >* fluid_particles_component=new OPENGL_COMPONENT_PARTICLES_2D<T,FLUID_PARTICLES<TV> >(basedir+"/%d/fluid_particles_1","",false);
        fluid_particles_component->opengl_points->color=OPENGL_COLOR::Red();
        Add_Component(fluid_particles_component,"Fluid particles",'2',BASIC_VISUALIZATION::OWNED|BASIC_VISUALIZATION::SELECTABLE);
        opengl_world.Append_Bind_Key('*',fluid_particles_component->Toggle_Draw_Velocities_CB());
        opengl_world.Append_Bind_Key('.',fluid_particles_component->Toggle_Increase_Vector_Size_CB());
        opengl_world.Append_Bind_Key(',',fluid_particles_component->Toggle_Decrease_Vector_Size_CB());}
    ...
    OPENGL_COMPONENT_MAC_VELOCITY_FIELD_2D<T>* mac_velocity_air_component=0;
    OPENGL_COMPONENT_MAC_VELOCITY_FIELD_2D<T>* mac_velocity_combined_component=0;
    ...
    if(has_valid_grid && FILE_UTILITIES::Frame_File_Exists(filename,start_frame)){
        mac_velocity_component=new OPENGL_COMPONENT_MAC_VELOCITY_FIELD_2D<T>(mac_grid,filename);
        filename=basedir+"/%d/weights_i";
        if(FILE_UTILITIES::Frame_File_Exists(filename,start_frame)){
            mac_velocity_component->weights_i_filename=filename;
            mac_velocity_component->print_weights_i=true;}
        filename=basedir+"/%d/weights_j";
        if(FILE_UTILITIES::Frame_File_Exists(filename,start_frame)){
            mac_velocity_component->weights_j_filename=filename;
            mac_velocity_component->print_weights_j=true;}
        ...
        Add_Component(mac_velocity_component,"MAC velocities",'\0',BASIC_VISUALIZATION::OWNED|BASIC_VISUALIZATION::START_HIDDEN);}
    filename=basedir+"/%d/mac_velocities_air";
    if(has_valid_grid && FILE_UTILITIES::Frame_File_Exists(filename,start_frame)){
        mac_velocity_air_component=new OPENGL_COMPONENT_MAC_VELOCITY_FIELD_2D<T>(mac_grid,filename);
        for(int i=1;i<=mac_velocity_air_component->opengl_adaptive_mac_velocity_fields.m;i++){
            mac_velocity_air_component->opengl_adaptive_mac_velocity_fields(i)->size=.01;
            mac_velocity_air_component->opengl_adaptive_mac_velocity_fields(i)->vector_color=OPENGL_COLOR::Green();
            mac_velocity_air_component->opengl_adaptive_mac_velocity_fields(i)->Set_Velocity_Mode(OPENGL_MAC_VELOCITY_FIELD_2D<T>::FACE_CENTERED);}
        mac_velocity_air_component->Set_Psi_N_Psi_D_Basedir_For_Divergence(basedir);
        Add_Component(mac_velocity_air_component,"MAC velocities air",'\0',BASIC_VISUALIZATION::OWNED|BASIC_VISUALIZATION::START_HIDDEN);}
    filename=basedir+"/%d/mac_velocities_combined";
    if(has_valid_grid && FILE_UTILITIES::Frame_File_Exists(filename,start_frame)){
        mac_velocity_combined_component=new OPENGL_COMPONENT_MAC_VELOCITY_FIELD_2D<T>(mac_grid,filename);
        for(int i=1;i<=mac_velocity_combined_component->opengl_adaptive_mac_velocity_fields.m;i++){
            mac_velocity_combined_component->opengl_adaptive_mac_velocity_fields(i)->size=.01;
            mac_velocity_combined_component->opengl_adaptive_mac_velocity_fields(i)->vector_color=OPENGL_COLOR::Red();
            mac_velocity_combined_component->opengl_adaptive_mac_velocity_fields(i)->Set_Velocity_Mode(OPENGL_MAC_VELOCITY_FIELD_2D<T>::FACE_CENTERED);}
        mac_velocity_combined_component->Set_Psi_N_Psi_D_Basedir_For_Divergence(basedir);
        Add_Component(mac_velocity_combined_component,"MAC velocities combined",'\0',BASIC_VISUALIZATION::OWNED|BASIC_VISUALIZATION::START_HIDDEN);}
    ...
    if(mac_velocity_air_component){
        opengl_world.Append_Bind_Key('A',mac_velocity_air_component->Toggle_Velocity_Mode_And_Draw_CB());
        opengl_world.Append_Bind_Key('=',mac_velocity_air_component->Increase_Vector_Size_CB());
        opengl_world.Append_Bind_Key('-',mac_velocity_air_component->Decrease_Vector_Size_CB());}
    if(mac_velocity_combined_component){
        opengl_world.Append_Bind_Key('B',mac_velocity_combined_component->Toggle_Velocity_Mode_And_Draw_CB());
        opengl_world.Append_Bind_Key('=',mac_velocity_combined_component->Increase_Vector_Size_CB());
        opengl_world.Append_Bind_Key('-',mac_velocity_combined_component->Decrease_Vector_Size_CB());}
    if(vector_velocity_component){
        opengl_world.Append_Bind_Key('E',vector_velocity_component->Toggle_Draw_CB());
    ...
    if(has_valid_grid && FILE_UTILITIES::Frame_File_Exists(filename,start_frame) && FILE_UTILITIES::Frame_File_Exists(filename2,start_frame)){
        ...
        sub_mac_velocities_component->Set_Size((T).01);
    ...
    if(has_valid_grid && FILE_UTILITIES::Frame_File_Exists(filename,start_frame)){
        OPENGL_COMPONENT_FACE_SCALAR_FIELD_2D<T,T>* beta_face_component=new OPENGL_COMPONENT_FACE_SCALAR_FIELD_2D<T,T>(mac_grid,filename,OPENGL_COLOR_RAMP<T>::Two_Color_Ramp(0,(T).002,OPENGL_COLOR::Gray(1),OPENGL_COLOR::Gray(0)));
    ...
    filename=basedir+"/%d/pressure_water";
    if(has_valid_grid && FILE_UTILITIES::Frame_File_Exists(filename,start_frame)){
        pressure_water_component=new OPENGL_COMPONENT_SCALAR_FIELD_2D<T>(mac_grid,filename,OPENGL_COLOR_RAMP<T>::Two_Color_Ramp(0,1,OPENGL_COLOR::Green(0,0),OPENGL_COLOR::Cyan(1)));
        pressure_water_component->opengl_scalar_field.Set_Scale_Range(0,30);
        pressure_water_component->opengl_scalar_field.Set_Uniform_Contour_Values(2,28,(T)26/(T)53);
        Add_Component(pressure_water_component,"Pressure_Water",'8',BASIC_VISUALIZATION::OWNED|BASIC_VISUALIZATION::START_HIDDEN);
        opengl_world.Append_Bind_Key(OPENGL_KEY(OPENGL_KEY::F10),pressure_water_component->Toggle_Draw_Mode_CB());}
    ...
    opengl_world.Set_Key_Binding_Category("Entropy");
    filename=basedir+"/%d/entropy";
    if(has_valid_grid && FILE_UTILITIES::Frame_File_Exists(filename,start_frame)){
        OPENGL_COMPONENT_SCALAR_FIELD_2D<T>* entropy_component=new OPENGL_COMPONENT_SCALAR_FIELD_2D<T>(mac_grid,filename,OPENGL_COLOR_RAMP<T>::Two_Color_Ramp(0,1,OPENGL_COLOR::Green(0,0),OPENGL_COLOR::Cyan(1)));
        entropy_component->opengl_scalar_field.Set_Uniform_Contour_Values(0,20,1);
        Add_Component(entropy_component,"Entropy",'\0',BASIC_VISUALIZATION::OWNED|BASIC_VISUALIZATION::START_HIDDEN);
        opengl_world.Append_Bind_Key(OPENGL_KEY(OPENGL_KEY::F4),entropy_component->Toggle_Draw_CB());}
    ...
    if(FILE_UTILITIES::File_Exists(deformable_object_filename)){
        ...
        deformable_objects_component->Set_Vector_Size((T).01);
    ...
    if(FILE_UTILITIES::Frame_File_Exists(basedir+"/%d/rigid_geometry_particles",start_frame)){
        ...
        for(int i=1;i<=rigid_bodies_no_draw_list.m;i++){
            std::stringstream ss;ss<<"Rigid bodies: not drawing object "<<rigid_bodies_no_draw_list(i)<<std::endl;LOG::filecout(ss.str());
    ...
    if(FILE_UTILITIES::File_Exists(soft_constraints_deformable_object_filename)){
        ...
        soft_constraints_deformable_objects_component->Set_Vector_Size((T).01);
    ...
    filename=basedir+"/%d/blend_mask";
    if(has_valid_grid && FILE_UTILITIES::Frame_File_Exists(filename,start_frame)){
        OPENGL_COMPONENT_SCALAR_FIELD_2D<T,bool>* blend_mask_component=new OPENGL_COMPONENT_SCALAR_FIELD_2D<T,bool>(mac_grid,filename,new OPENGL_CONSTANT_COLOR_MAP<bool>(OPENGL_COLOR::Magenta()),OPENGL_SCALAR_FIELD_2D<T,bool>::DRAW_POINTS);
        Add_Component(blend_mask_component,"LS and VF Blend Mask",'\0',BASIC_VISUALIZATION::START_HIDDEN|BASIC_VISUALIZATION::OWNED);
        opengl_world.Append_Bind_Key('K',blend_mask_component->Toggle_Draw_CB());}
            ...
            if(sstream>>next_word){
                if(next_word=="auto"){
                    if(pressure_component){
                        ...
                        std::stringstream ss;ss<<"min pressure="<<min_pressure<<", max pressure="<<max_pressure<<std::endl;LOG::filecout(ss.str());
                    ...
                    T min_pressure=(T)atof(next_word.c_str()),max_pressure;
