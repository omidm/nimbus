...
void Main::Execute(Parameter params, const DataArray& da) {
  ...
  READ_5(d[0],d[1],d[2],d[3],d[4]); WRITE_0();
  read.insert(d[0]);
  read.insert(d[0]);
  read.insert(d[0]);
  read.insert(d[0]);
...
void SpawnOneIterationIfNeeded::Execute(Parameter params, const DataArray& da) {
  ...
  for (IDSet<logical_data_id_t>::IDSetIter i = temp_par.begin();
      i != temp_par.end(); i++) {
    temp.push_back(*i);
  }
  ...
  bool move_on = true;
  {
    printf("[QUH] norm%f norm %f tol %f.\n", partial_norm_1->norm_,
	partial_norm_buffer->norm_, app_driver->projection_internal_data->global_tolerance);
    int desired_iterations=app_driver->projection_internal_data->global_n;
    app_driver->projection_internal_data->residual = max(partial_norm_1->norm_, partial_norm_buffer->norm_);
  ...
  if (move_on) {
    ...
    read.clear(); read.insert(temp[0]); write.clear(); 
    before.clear(); after.clear();
    SpawnComputeJob("finish", j[0], read, write, before, after, par);
    read.clear(); read.insert(temp[1]); write.clear(); 
    before.clear(); after.clear();
...
void OneIteration::Execute(Parameter params, const DataArray& da) {
  ...
  printf("[QUH] DEBUG %f.\n", partial_norm->norm_);
  PhysBAM::PROJECTION_DRIVER< PhysBAM::VECTOR<float,2> >* app_driver = projection_app->app_driver;
  printf("[QUH] DEBUG %f.\n", app_driver->projection_internal_data->alpha);
  printf("[QUH] DEBUG %f.\n", app_driver->projection_internal_data->beta);
  ...
  //    app_driver->projection_data);
  printf("[QUH] OneIteration finishes %f.\n", partial_norm->norm_);
}
