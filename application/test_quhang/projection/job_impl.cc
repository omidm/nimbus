...
void Main::Execute(Parameter params, const DataArray& da) {
  ...
  read.clear(); read.insert(d[0]); read.insert(d[2]);
  write.clear(); write.insert(d[2]);
  before.clear(); after.clear(); after.insert(j[2]);
  ...
  read.clear(); read.insert(d[0]); read.insert(d[1]);
  read.insert(d[2]); read.insert(d[3]);
  write.clear();
  before.clear(); before.insert(j[0]); before.insert(j[1]);
  after.clear(); 
  SpawnComputeJob("spawn_one_iteration_if_needed", j[2], read, write, before, after, par);
...
void SpawnOneIterationIfNeeded::Execute(Parameter params, const DataArray& da) {
  ...
  if (move_on) {
    ...
    read.clear(); read.insert(profile_data_1->logical_id());
    write.clear(); write.insert(partial_norm_1->logical_id());
    before.clear(); after.clear(); after.insert(j[2]);
    SpawnComputeJob("one_iteration", j[0], read, write, before, after, par);
    read.clear(); read.insert(profile_data_2->logical_id());
    write.clear(); write.insert(partial_norm_2->logical_id());
    before.clear(); after.clear(); after.insert(j[2]);
    SpawnComputeJob("one_iteration", j[1], read, write, before, after, par);

    read.clear(); read.insert(profile_data_1->logical_id()); read.insert(profile_data_2->logical_id());
    read.insert(partial_norm_1->logical_id());read.insert(partial_norm_2->logical_id());
    write.clear(); 
    before.clear(); before.insert(j[0]); before.insert(j[1]);
    after.clear();
    SpawnComputeJob("spawn_one_iteration_if_needed", j[2], read, write, before, after, par);
