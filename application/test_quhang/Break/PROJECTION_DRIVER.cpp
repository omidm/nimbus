...
Execute_Main_Program() {
  ...
  if (projection_internal_data->move_on) {
    ...
    do {
      ...
      pcg_mpi->UpdateSearchVector(projection_internal_data, projection_data);
      pcg_mpi->ExchangeSearchVector(projection_internal_data, projection_data);
      ...
      pcg_mpi->UpdateOtherVectors(projection_internal_data, projection_data);
      pcg_mpi->CalculateResidual(projection_internal_data, projection_data);
      pcg_mpi->DecideToSpawnNextIteration(projection_internal_data, projection_data);
...
PrepareForProjection() {
  ...
  example.boundary_scalar.Apply_Boundary_Condition_Face(example.mac_grid,example.face_velocities,time+dt);
  example.mpi_grid->Average_Common_Face_Data(example.face_velocities);
    //->MPI_GRID<T_GRID>::Average_Common_Face_Data(*example.mpi_grid, face_velocities);
...
ApplyPressureAndFinish() {
  ...
  T dt=example.Time_At_Frame(current_frame+1)-time;
  for(CELL_ITERATOR iterator(laplace->grid,1);iterator.Valid();iterator.Next()){
    int filled_region_color=laplace->filled_region_colors(iterator.Cell_Index());
    if(filled_region_color>0 && !laplace->filled_region_touches_dirichlet(filled_region_color))
...
PrepareForOneRegion() {
  ...
  laplace->pcg.Enforce_Compatibility(
      !laplace->filled_region_touches_dirichlet(color)&&laplace->enforce_compatibility);
  // Context.
  ...
  SPARSE_MATRIX_FLAT_NXN<T> &A =
    (*(projection_data->A_array))(color);
  A.Negate(); 
  VECTOR_ND<T> &b =
    (*(projection_data->b_array))(color);
  b*=(T)-1;
