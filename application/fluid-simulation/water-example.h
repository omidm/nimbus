...
class WATER_EXAMPLE: public LEVELSET_CALLBACKS<GRID<template_TV> >,
    RIGID_GEOMETRY_EXAMPLE_VELOCITIES<template_TV> {
...
public:
  ...
  ARRAY<T, FACE_INDEX<TV::dimension> > face_velocities; // OK.
  ...
  void Get_Levelset_Velocity(const GRID<TV>& grid, T_LEVELSET& levelset,
      ARRAY<T, FACE_INDEX<TV::dimension> >& V_levelset, const T time) const PHYSBAM_OVERRIDE
  ...
  void Adjust_Particle_For_Domain_Boundaries(
      ...
      const T time);
  void Get_Levelset_Velocity(const GRID<TV>& grid,
