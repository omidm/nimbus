...
Irradiance_Estimate(const TV& location,const TV& normal,const T max_distance_squared,const int number_of_photons,const RENDERING_RAY<T>& ray,const PHOTON_MAP_TYPE type)
{
    ...
    T one_over_max_distance,actual_max_distance_squared=0,max_distance_cubed;
...
Print_Photon_List()
{
    LOG::filecout("PHOTON LIST\n");
    for(int i=1;i<=photons.m;i++) {
        std::stringstream ss;
        ss<<"i="<<i<<" Photon "<<photons(i).location<<" Power "<<photons(i).power<<" Split on "<<photons(i).kdtree_split_axis<<std::endl;
        LOG::filecout(ss.str());
    }
}
//#####################################################################
...
Print_Photon_Tree(const int depth,const int index)
{
    ...
    std::stringstream ss;
    if(index==1)ss<<"PHOTON TREE"<<std::endl;
    for(int i=1;i<=depth;i++)ss<<" ";
    ss<<"Photon "<<photons(index).location<<" Split on "<<photons(index).kdtree_split_axis<<std::endl;
    LOG::filecout(ss.str());
