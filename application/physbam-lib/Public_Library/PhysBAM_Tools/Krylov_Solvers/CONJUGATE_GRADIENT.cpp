    ...
    KRYLOV_VECTOR_BASE<T>& k,KRYLOV_VECTOR_BASE<T>& z,const T tolerance,const int min_iterations,const int max_iterations)
{
    ...
    int iterations;for(iterations=0;;iterations++){
        ...
        if(restart){
            ...
            if(print_residuals) LOG::filecout("restarting cg\n");
#endif
...
#ifndef COMPILE_WITHOUT_READ_WRITE_SUPPORT
        if(s_dot_q<=0) {std::stringstream ss;ss<<"CG: matrix appears indefinite or singular, s_dot_q/s_dot_s="<<s_dot_q/(T)system.Inner_Product(s,s)<<std::endl;LOG::filecout(ss.str());}
#endif
    ...
    if(print_diagnostics) {std::stringstream ss;ss<<"cg not converged after "<<max_iterations<<" iterations, error = "<<convergence_norm<<std::endl;LOG::filecout(ss.str());}
#endif
    return false;
}
//#####################################################################
