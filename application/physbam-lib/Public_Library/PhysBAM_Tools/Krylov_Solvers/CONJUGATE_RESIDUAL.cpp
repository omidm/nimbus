    ...
    KRYLOV_VECTOR_BASE<T>& r,KRYLOV_VECTOR_BASE<T>& z,const T tolerance,const int min_iterations,const int max_iterations)
{
    ...
    int iterations;for(iterations=0;;iterations++){
        ...
        if(restart){
            ...
            if(print_residuals) LOG::filecout("restarting conjugate_residual\n");
#endif
    ...
    if(print_diagnostics) {std::stringstream ss;ss<<"conjugate_residual not converged after "<<max_iterations<<" iterations, error = "<<convergence_norm<<std::endl;LOG::filecout(ss.str());}
#endif
