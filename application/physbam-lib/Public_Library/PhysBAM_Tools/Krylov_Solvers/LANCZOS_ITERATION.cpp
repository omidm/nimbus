...
Tridiagonalize(const KRYLOV_SYSTEM_BASE<T>& system,KRYLOV_VECTOR_BASE<T>& w,KRYLOV_VECTOR_BASE<T>& v,KRYLOV_VECTOR_BASE<T>& q,const T tolerance,const int max_iterations)
{
    ...
    if(!magnitude_squared){
        ...
        if(print_diagnostics) {std::stringstream ss;ss<<"lanczos iterations = "<<0<<std::endl;LOG::filecout(ss.str());}
#endif
        return true;}
...
#ifndef COMPILE_WITHOUT_READ_WRITE_SUPPORT
            if(print_diagnostics) {std::stringstream ss;ss<<"lanczos iterations = "<<iterations<<std::endl;LOG::filecout(ss.str());}
#endif
            return true;}
...
#ifndef COMPILE_WITHOUT_READ_WRITE_SUPPORT
    if(print_diagnostics) {std::stringstream ss;ss<<"lanczos not converged after "<<max_iterations<<" iterations, error = "<<convergence_norm<<std::endl;LOG::filecout(ss.str());}
#endif
    return false;
}
//#####################################################################
...
Test_Symmetry(const KRYLOV_SYSTEM_BASE<T>& system,KRYLOV_VECTOR_BASE<T>& w,KRYLOV_VECTOR_BASE<T>& v,KRYLOV_VECTOR_BASE<T>& q,const int iterations)
{
    ...
    for(int i=1;i<=iterations;i++){
        ...
        std::stringstream ss;
        ss<<"Ax_dot_Ax = "<<Ax_dot_Ax<<", x_dot_AAx = "<<x_dot_AAx<<", relative_error = "<<abs(Ax_dot_Ax-x_dot_AAx)/Ax_dot_Ax<<std::endl;
        LOG::filecout(ss.str());
#endif
        V=Vector(V.y,V.z,V.x);
        // rescale to avoid overflow
        T scale=(T)1/(T)sqrt(Ax_dot_Ax);
        *V.x*=scale;
        *V.y*=scale;}
}
//#####################################################################
