...
public:
    ...
    //this function needs to be inlined to avoid linking errors with shared libraries on linux - gcc 4.4
    inline ATTRIBUTE_INDEX Find_Attribute_Index(const ATTRIBUTE_ID attribute_id) const
    {ATTRIBUTE_INDEX first(1),last(arrays.m+1);
    while(first<last){ATTRIBUTE_INDEX middle((Value(first)+Value(last))/2);
        if(arrays(middle)->id<attribute_id) first=middle+1;
        else last=middle;}
    return first;}

    //this function needs to be inlined to avoid linking errors with shared libraries on linux - gcc 4.4
    inline ATTRIBUTE_INDEX Add_Array(const ATTRIBUTE_ID attribute_id,ARRAY_COLLECTION_ELEMENT_BASE* array)
    ...
    if(index<=arrays.m && arrays(index)->id==attribute_id){
        ...
        return index;}
    array->id=attribute_id;array->Reallocate(buffer_size);array->Set_Size(number);arrays.Insert(array,index);
    return index;}
