//#####################################################################
// Copyright 2007-2012, Craig Schroeder, Rahul Sheth, Tamar Shinar.
...
public:
    ...
    template<class T_VECTOR2>
    T_VECTOR Householder_Transform(const VECTOR_BASE<T,T_VECTOR2>& v) const
    {Assert_Same_Size(*this,v);
    T v_dot_a=0,v_dot_v=0;for(int i=1;i<=Size();i++){v_dot_a+=v(i)*(*this)(i);v_dot_v+=sqr(v(i));}
    VECTOR_DIFFERENCE<T_VECTOR,VECTOR_SCALE<T,T_VECTOR2> > temp=*this-2*v_dot_a/v_dot_v*v;
    T_VECTOR returnVector;returnVector.Resize(temp.Size());for(int i=1;i<=temp.Size();i++){returnVector(i)=temp(i);}
    return returnVector;}

    template<class T_VECTOR2>
    void Generalized_Householder_Transform(T& x, T& beta, VECTOR_BASE<T,T_VECTOR2>& v) const
    {Assert_Same_Size(*this,v);
     T alpha=Magnitude()*(sign((*this)(1))+((*this)(1)==0?1:0));
     for(int i=1;i<=Size();i++){v(i)=(*this)(i);}
     if(alpha==0) beta=0;
     else beta=1/(abs(alpha)*(abs(alpha)+abs(v(1))));
     v(1) = v(1)+alpha;x=-alpha;return;}
...
template<class T,class T_VECTOR> bool operator>(const VECTOR_BASE<T,T_VECTOR>& v1,const VECTOR_BASE<T,T_VECTOR>& v2)
{assert(v1.Size()==v2.Size()); for(int i=1;i<=v1.Size();i++) if(v1(i)>v2(i)) return true; else if(v1(i)<v2(i)) return false; return false;}
