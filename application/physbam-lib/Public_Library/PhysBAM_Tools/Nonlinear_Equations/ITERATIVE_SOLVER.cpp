...
Bisection_Root(NONLINEAR_FUNCTION<T(T)>& F,T a,T b)
{
  iterations=0;T Fa=F(a);assert(Fa*F(b)<=0);
    ...
    if(iterations==max_iterations){
        ...
        LOG::filecout("Bisection_Root failed with max iterations\n");
#endif
        return 0;}
    return (T).5*(a+b);
}
...
Newton_Root(NONLINEAR_FUNCTION<T(T)>& F,T x0)
{
    ...
    if(iterations==max_iterations){
#ifndef COMPILE_WITHOUT_READ_WRITE_SUPPORT
        LOG::filecout("Newton_Root failed with max iterations\n");
#endif
        return 0;}
    return x;
}
...
Secant_Root(NONLINEAR_FUNCTION<T(T)>& F,T x0,T x1)
{
    ...
    if(iterations==max_iterations){
#ifndef COMPILE_WITHOUT_READ_WRITE_SUPPORT
        LOG::filecout("Secant_Root failed with max iterations");
#endif
        return 0;}
    return x;
}
...
{
    iterations=0;T Fa=F(a),Fb=F(b),x_old=a,x=b,Fx_old=Fa,Fx=Fb;assert(Fa*Fb<=1e-7);
    while(b-a>tolerance && iterations++<max_iterations){
    ...
    if(iterations==max_iterations){
#ifndef COMPILE_WITHOUT_READ_WRITE_SUPPORT
        LOG::filecout("Bisection_Secant_Root failed with max iterations");
#endif
        return 0;}
    return x;
}
...
Bisection_Newton_Root(NONLINEAR_FUNCTION<T(T)>& F,T a,T b)
{
  iterations=0;T Fa=F(a),x=a,Fx=Fa,Fprime=F.Prime(x);assert(Fa*F(b)<=0);
    ...
    if(iterations==max_iterations){
#ifndef COMPILE_WITHOUT_READ_WRITE_SUPPORT
        LOG::filecout("Newton_Secant_Root failed with max iterations");
#endif
        return 0;}
    return x;
}
...
Golden_Minimum(NONLINEAR_FUNCTION<T(T)>& F,T a,T b)
{
    ...
    if(iterations==max_iterations){
#ifndef COMPILE_WITHOUT_READ_WRITE_SUPPORT
        LOG::filecout("Golden_Minimum failed with max iterations");
#endif
        return 0;}
    return (T).5*(a+b);
}
...
Parabolic_Minimum(NONLINEAR_FUNCTION<T(T)>& F,T x0,T x1,T x2)
{
    ...
    if(iterations==max_iterations){
#ifndef COMPILE_WITHOUT_READ_WRITE_SUPPORT
        LOG::filecout("Parabolic_Minimum failed with max iterations");
#endif
        return 0;}
    return x2;
}
...
Golden_Parabolic_Minimum(NONLINEAR_FUNCTION<T(T)>& F,T a,T b)
{
    ...
    if(iterations==max_iterations){
#ifndef COMPILE_WITHOUT_READ_WRITE_SUPPORT
        LOG::filecout("Golden_Parabolic_Minimum failed with max iterations");
#endif
        return 0;}
    return x2;
}
...
#ifndef COMPILE_WITHOUT_READ_WRITE_SUPPORT
    if(iterations==max_iterations) LOG::filecout("Steepest_Decent failed with max iterations");
#endif
}
//#####################################################################
...
Conjugate_Gradient(NONLINEAR_FUNCTION<T(T,T)>& F,T& x,T& y,const T alpha_max)
{
    ...
    while(sqrt(sqr(partial_x)+sqr(partial_y))>tolerance && iterations<max_iterations){ // do conjugate gradiant n=2 times, then restart with a steepest decent step
        ...
        while(sqrt(sqr(partial_x)+sqr(partial_y))>tolerance && cg_step++<2 && iterations++<max_iterations){
            ...
            partial_x=F.Partial_X(x,y);partial_y=F.Partial_Y(x,y);}}
#ifndef COMPILE_WITHOUT_READ_WRITE_SUPPORT
    if(iterations==max_iterations) LOG::filecout("Conjugate_Gradient failed with max iterations");
#endif
}
//#####################################################################
// Function Conjugate_Gradient
...
{
    //TODO: IF THIS FUNCTION IS USED THIS NEEDS TO BE FIXED TO COMPILE
    PHYSBAM_FATAL_ERROR();
