...
Synchronize_Colors()
{
    if(mpi_grid.mpi_threaded_grid) return Synchronize_Colors_MPI_Threaded();
    ...
    std::stringstream ss;
    ss<<"initial colors: "<<number_of_regions<<" total, "<<global_color_count<<" out of "<<total_global_colors<<" global"<<std::endl;
    LOG::filecout(ss.str());
    ...
    for(CELL_ITERATOR iterator(local_grid,1);iterator.Valid();iterator.Next()){TV_INT cell=iterator.Cell_Index();int color=colors(cell);
        if(color_map(color)!=0 && !global_colors.Contains(color_map(color)) && color_map(color)!=-1) global_colors.Insert(color_map(color));}
    max_global_color=ARRAYS_COMPUTATIONS::Max(global_to_final_color_map);

    // remap colors
    Remap_Colors(color_map,RANGE<typename T_PARALLEL_GRID::VECTOR_INT>::Centered_Box());

    std::stringstream ss1;
    ss1<<"final colors: "<<color_ranks.m<<" global, "<<number_of_regions-color_ranks.m<<" local"<<std::endl;
...
template<class T_GRID> int FLOOD_FILL_MPI<T_GRID>::
Synchronize_Colors_MPI_Threaded()
{
    #ifdef USE_PTHREADS
    ARRAY<RANGE<typename T_PARALLEL_GRID::VECTOR_INT> > boundary_regions;
    mpi_grid.mpi_threaded_grid->Find_Boundary_Regions(boundary_regions,RANGE<typename T_PARALLEL_GRID::VECTOR_INT>::Zero_Box(),false,RANGE<VECTOR<int,1> >(-1,0),false,true,local_grid);
    // figure out which colors are global
    int global_color_count=0;
    ARRAY<int,VECTOR<int,1> > color_map(-1,number_of_regions);color_map(-1)=-1;color_map(0)=0;
    {ARRAY<bool,VECTOR<int,1> > color_is_global(-1,number_of_regions);
    ...
    // send numbers of global colors to everyone
    ARRAY<int> global_color_counts(mpi_grid.mpi_threaded_grid->number_of_processes);
    mpi_grid.mpi_threaded_grid->Allgather(global_color_count,global_color_counts);
    int total_global_colors=ARRAYS_COMPUTATIONS::Sum(global_color_counts);
    int global_color_offset=ARRAYS_COMPUTATIONS::Sum(global_color_counts.Prefix(mpi_grid.mpi_threaded_grid->rank));
    std::stringstream ss;
    ss<<"initial colors: "<<number_of_regions<<" total, "<<global_color_count<<" out of "<<total_global_colors<<" global"<<std::endl;
    LOG::filecout(ss.str());
    if(!total_global_colors){color_ranks.Clean_Memory();return 0;}
    ...
    ARRAY<T_ARRAYS_INT> colors_copy(boundary_regions.m);
    // send left (front) colors
    for(int side=1;side<=T_PARALLEL_GRID::number_of_faces_per_cell;side+=2)if(mpi_grid.mpi_threaded_grid->threaded_grid->side_neighbor_ranks(side)!=-1){
        Resize_Helper(colors_copy(side),local_grid,boundary_regions(side));
        Translate_Local_Colors_To_Global_Colors(color_map,colors_copy(side),boundary_regions(side),global_color_offset);
        THREAD_PACKAGE pack=mpi_grid.mpi_threaded_grid->threaded_grid->Package_Cell_Data(colors_copy(side),boundary_regions(side));pack.recv_tid=mpi_grid.mpi_threaded_grid->threaded_grid->side_neighbor_ranks(side);
        pthread_mutex_lock(mpi_grid.mpi_threaded_grid->lock);
        mpi_grid.mpi_threaded_grid->buffers->Append(pack);
        pthread_mutex_unlock(mpi_grid.mpi_threaded_grid->lock);}
    
    for(int side=1;side<=T_PARALLEL_GRID::number_of_faces_per_cell;side+=2)if(mpi_grid.mpi_threaded_grid->mpi_grid->side_neighbor_ranks(side)!=MPI::PROC_NULL){
        Resize_Helper(colors_copy(side),local_grid,boundary_regions(side));
        Translate_Local_Colors_To_Global_Colors(color_map,colors_copy(side),boundary_regions(side),global_color_offset);
        pthread_mutex_lock(mpi_grid.mpi_threaded_grid->lock);
        MPI_PACKAGE package=mpi_grid.Package_Cell_Data(colors_copy(side),boundary_regions(side));
        mpi_grid.mpi_threaded_grid->mpi_packages_buffers->Append(package);
        mpi_grid.mpi_threaded_grid->mpi_requests_buffers->Append(package.Isend(*mpi_grid.mpi_threaded_grid->mpi_grid->comm,mpi_grid.mpi_threaded_grid->mpi_grid->side_neighbor_ranks(side),mpi_grid.mpi_threaded_grid->Get_Send_Tag(mpi_grid.mpi_threaded_grid->side_neighbor_directions(side))));
        pthread_mutex_unlock(mpi_grid.mpi_threaded_grid->lock);}
    
    // receive right (back) colors and initialize union find
    UNION_FIND<> union_find(total_global_colors);
    pthread_barrier_wait(mpi_grid.mpi_threaded_grid->barr);
    {for(int side=2;side<=T_PARALLEL_GRID::number_of_faces_per_cell;side+=2)if(mpi_grid.mpi_threaded_grid->threaded_grid->side_neighbor_ranks(side)!=-1){
        Resize_Helper(colors_copy(side),local_grid,boundary_regions(side));
        int index=0;
        for(int i=1;i<=mpi_grid.mpi_threaded_grid->buffers->m;i++){
            if((*mpi_grid.mpi_threaded_grid->buffers)(i).send_tid==mpi_grid.mpi_threaded_grid->threaded_grid->side_neighbor_ranks(side) && (*mpi_grid.mpi_threaded_grid->buffers)(i).recv_tid==mpi_grid.mpi_threaded_grid->threaded_grid->rank) index=i;}
        PHYSBAM_ASSERT(index);int position=0;
        for(CELL_ITERATOR iterator(local_grid,boundary_regions(side));iterator.Valid();iterator.Next()) colors_copy(side).Unpack((*mpi_grid.mpi_threaded_grid->buffers)(index).buffer,position,iterator.Cell_Index());
        Find_Color_Matches(color_map,union_find,colors_copy(side),boundary_regions(side),global_color_offset);}}
    pthread_barrier_wait(mpi_grid.mpi_threaded_grid->barr);
    if(mpi_grid.mpi_threaded_grid->tid==1) mpi_grid.mpi_threaded_grid->buffers->m=0;
    pthread_barrier_wait(mpi_grid.mpi_threaded_grid->barr);
    
    {ARRAY<MPI::Request> recv_requests;
    for(int side=2;side<=T_PARALLEL_GRID::number_of_faces_per_cell;side+=2)if(mpi_grid.mpi_threaded_grid->mpi_grid->side_neighbor_ranks(side)!=MPI::PROC_NULL){
        Resize_Helper(colors_copy(side),local_grid,boundary_regions(side));
        pthread_mutex_lock(mpi_grid.mpi_threaded_grid->lock);
        MPI_PACKAGE package=mpi_grid.Package_Cell_Data(colors_copy(side),boundary_regions(side));
        mpi_grid.mpi_threaded_grid->mpi_packages_buffers->Append(package);
        recv_requests.Append(package.Irecv(*mpi_grid.mpi_threaded_grid->mpi_grid->comm,mpi_grid.mpi_threaded_grid->mpi_grid->side_neighbor_ranks(side),mpi_grid.mpi_threaded_grid->Get_Recv_Tag(mpi_grid.mpi_threaded_grid->side_neighbor_directions(side))));
        pthread_mutex_unlock(mpi_grid.mpi_threaded_grid->lock);}
    pthread_mutex_lock(mpi_grid.mpi_threaded_grid->lock);
    bool recv_finish=!MPI_UTILITIES::Wait_Any(recv_requests,*mpi_grid.mpi_threaded_grid->status);
    int status_tag=mpi_grid.mpi_threaded_grid->status->Get_tag();
    pthread_mutex_unlock(mpi_grid.mpi_threaded_grid->lock);
    while(!recv_finish){
        int side;for(side=2;side<=T_PARALLEL_GRID::number_of_faces_per_cell;side+=2)if(mpi_grid.mpi_threaded_grid->Get_Recv_Tag(mpi_grid.mpi_threaded_grid->side_neighbor_directions(side))==status_tag) break;
        Find_Color_Matches(color_map,union_find,colors_copy(side),boundary_regions(side),global_color_offset);
        pthread_mutex_lock(mpi_grid.mpi_threaded_grid->lock);
        recv_finish=!MPI_UTILITIES::Wait_Any(recv_requests,*mpi_grid.mpi_threaded_grid->status);
        status_tag=mpi_grid.mpi_threaded_grid->status->Get_tag();
        pthread_mutex_unlock(mpi_grid.mpi_threaded_grid->lock);}
    pthread_mutex_lock(mpi_grid.mpi_threaded_grid->lock);
    recv_requests.Clean_Memory();
    pthread_mutex_unlock(mpi_grid.mpi_threaded_grid->lock);}
    ...
    pthread_mutex_unlock(mpi_grid.mpi_threaded_grid->lock);}
    pthread_barrier_wait(mpi_grid.mpi_threaded_grid->barr);
    for(int buf=1;buf<=mpi_grid.mpi_threaded_grid->buffers->m;buf++){int position=0;
        ...
        for(int i=1;i<=union_find.parents.m;i++) union_find.parents.Unpack((*mpi_grid.mpi_threaded_grid->buffers)(buf).buffer,position,i);for(int i=1;i<=union_find.ranks.m;i++) union_find.ranks.Unpack((*mpi_grid.mpi_threaded_grid->buffers)(buf).buffer,position,i);
        final_union_find.parents.Resize(union_find.Size());
        final_union_find.ranks.Resize(union_find.ranks.m);
        final_union_find.Merge(union_find);}
    pthread_barrier_wait(mpi_grid.mpi_threaded_grid->barr);
    if(mpi_grid.mpi_threaded_grid->tid==1) mpi_grid.mpi_threaded_grid->buffers->m=0;
    pthread_barrier_wait(mpi_grid.mpi_threaded_grid->barr);}
    
    if(mpi_grid.mpi_threaded_grid->tid==1){
        ARRAY<char> union_find_buffer(MPI_UTILITIES::Pack_Size(final_union_find,*mpi_grid.mpi_threaded_grid->mpi_grid->comm)+1);
        {int position=0;MPI_UTILITIES::Pack(final_union_find,union_find_buffer,position,*mpi_grid.mpi_threaded_grid->mpi_grid->comm);}
        MPI::Datatype union_find_type=MPI::PACKED.Create_contiguous(union_find_buffer.m);union_find_type.Commit();
        MPI::Op union_find_merge_op;union_find_merge_op.Init(Union_Find_Merge_Op,true);
        ARRAY<char> final_union_find_buffer(union_find_buffer.m);
        union_find_merge_op_comm=mpi_grid.mpi_threaded_grid->mpi_grid->comm;
        mpi_grid.mpi_threaded_grid->mpi_grid->comm->Allreduce(union_find_buffer.Get_Array_Pointer(),final_union_find_buffer.Get_Array_Pointer(),1,union_find_type,union_find_merge_op);
        {int position=0;MPI_UTILITIES::Unpack(final_union_find,final_union_find_buffer,position,*mpi_grid.mpi_threaded_grid->mpi_grid->comm);}
        union_find_type.Free();union_find_merge_op.Free();
        {THREAD_PACKAGE final_pack(sizeof(int)*(2+final_union_find.parents.m)+final_union_find.ranks.m);
        int position=0;*(int*)(&final_pack.buffer(position+1))=final_union_find.parents.m;position+=sizeof(int);*(int*)(&final_pack.buffer(position+1))=final_union_find.ranks.m;position+=sizeof(int);
        for(int i=1;i<=final_union_find.parents.m;i++) final_union_find.parents.Pack(final_pack.buffer,position,i);for(int i=1;i<=final_union_find.ranks.m;i++) final_union_find.ranks.Pack(final_pack.buffer,position,i);
        mpi_grid.mpi_threaded_grid->buffers->Append(final_pack);}}
    
    {pthread_barrier_wait(mpi_grid.mpi_threaded_grid->barr);
    assert(mpi_grid.mpi_threaded_grid->buffers->m==1);
    int position=0;
    THREAD_PACKAGE* final_pack=&(*mpi_grid.mpi_threaded_grid->buffers)(1);
    final_union_find.parents.Resize(*(int*)(&final_pack->buffer(position+1)));position+=sizeof(int);
    final_union_find.ranks.Resize(*(int*)(&final_pack->buffer(position+1)));position+=sizeof(int);
    for(int i=1;i<=final_union_find.parents.m;i++) final_union_find.parents.Unpack(final_pack->buffer,position,i);
    for(int i=1;i<=final_union_find.ranks.m;i++) final_union_find.ranks.Unpack(final_pack->buffer,position,i);
    pthread_barrier_wait(mpi_grid.mpi_threaded_grid->barr);
    if(mpi_grid.mpi_threaded_grid->tid==1) mpi_grid.mpi_threaded_grid->buffers->m=0;
    pthread_barrier_wait(mpi_grid.mpi_threaded_grid->barr);}
    ...
    color_ranks.Clean_Memory();
    color_ranks.Resize(number_of_regions);
    for(int r=0;r<mpi_grid.mpi_threaded_grid->number_of_processes;r++){
        int start=end+1;end+=global_color_counts(r+1);
        for(int i=start;i<=end;i++)color_ranks(global_to_final_color_map(i)).Append_Unique(r);}
    for(int color=1;color<=color_ranks.m;color++) assert(color_ranks(color).m>1 || mpi_grid.mpi_threaded_grid->side_neighbor_ranks.Contains(mpi_grid.mpi_threaded_grid->rank));
    ...
    if(color_touches_uncolorable){
        ...
        pthread_barrier_wait(mpi_grid.mpi_threaded_grid->barr);
        for(int buf=1;buf<=mpi_grid.mpi_threaded_grid->buffers->m;buf++){int position=0;
            for(int i=1;i<=global_color_touches_uncolorable.m;i++) global_color_touches_uncolorable.Unpack((*mpi_grid.mpi_threaded_grid->buffers)(buf).buffer,position,i);
            for(int i=1;i<=global_color_touches_uncolorable.m;i++) (*color_touches_uncolorable)(i)|=global_color_touches_uncolorable(i);}
        global_color_touches_uncolorable.Exchange(*color_touches_uncolorable);
        pthread_barrier_wait(mpi_grid.mpi_threaded_grid->barr);
        if(mpi_grid.mpi_threaded_grid->tid==1) mpi_grid.mpi_threaded_grid->buffers->m=0;
        pthread_barrier_wait(mpi_grid.mpi_threaded_grid->barr);
        if(mpi_grid.mpi_threaded_grid->tid==1){
            mpi_grid.mpi_threaded_grid->mpi_grid->comm->Allreduce(&global_color_touches_uncolorable(1),&(*color_touches_uncolorable)(1),color_ranks.m,MPI_UTILITIES::Datatype<bool>(),MPI::LOR);
            THREAD_PACKAGE final_pack(sizeof(bool)*color_ranks.m);int position=0;
            for(int i=1;i<=color_ranks.m;i++) color_touches_uncolorable->Pack(final_pack.buffer,position,i);
            mpi_grid.mpi_threaded_grid->buffers->Append(final_pack);}
        pthread_barrier_wait(mpi_grid.mpi_threaded_grid->barr);
        {int position=0;for(int i=1;i<=color_ranks.m;i++) color_touches_uncolorable->Unpack((*mpi_grid.mpi_threaded_grid->buffers)(1).buffer,position,i);}
        pthread_barrier_wait(mpi_grid.mpi_threaded_grid->barr);
        if(mpi_grid.mpi_threaded_grid->tid==1) mpi_grid.mpi_threaded_grid->buffers->m=0;
        pthread_barrier_wait(mpi_grid.mpi_threaded_grid->barr);}
    ...
    pthread_barrier_wait(mpi_grid.mpi_threaded_grid->barr);
    if(mpi_grid.mpi_threaded_grid->tid==1){
        MPI_UTILITIES::Wait_All(*(mpi_grid.mpi_threaded_grid->mpi_requests_buffers));
        MPI_PACKAGE::Free_All(*(mpi_grid.mpi_threaded_grid->mpi_packages_buffers));}
    pthread_barrier_wait(mpi_grid.mpi_threaded_grid->barr);
...
Find_Global_Colors(ARRAY<bool,VECTOR<int,1> >& color_is_global,const RANGE<TV_INT>&) const
{
    ...
    if(mpi_grid.mpi_threaded_grid) proc_null=-1;
    const ARRAY<int>& side_neighbor_ranks=mpi_grid.mpi_threaded_grid?mpi_grid.mpi_threaded_grid->side_neighbor_ranks:(mpi_grid.threaded_grid?mpi_grid.threaded_grid->side_neighbor_ranks:mpi_grid.side_neighbor_ranks);
