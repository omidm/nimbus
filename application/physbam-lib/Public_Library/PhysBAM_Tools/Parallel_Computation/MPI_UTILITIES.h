...
// Datatype conversion
//#####################################################################
template<class T,class DH_VOID=void> struct DATATYPE_HELPER{typedef int NO_DATATYPE_EXISTS;};
...
template<class T> struct DATATYPE_HELPER<VECTOR<T,0> >:public DATATYPE_HELPER<T>{};
...
#ifdef WIN32 //TODO: is this really required?
template<class TV> struct DATATYPE_HELPER<TWIST<TV> >:public DATATYPE_HELPER<TV>{};
#endif
...
template<class TV_INT> struct DATATYPE_HELPER<PAIR<int,TV_INT> >{static MPI::Datatype Datatype();};
template<class TV_INT> struct DATATYPE_HELPER<VECTOR<PAIR<int,TV_INT>,2> >{static MPI::Datatype Datatype();};
template<class T,class TV_INT> struct DATATYPE_HELPER<PAIR<VECTOR<PAIR<int,TV_INT>,2>,T> >{static MPI::Datatype Datatype();};
template<class TV> struct DATATYPE_HELPER<PAIR<TV,TV> >{static MPI::Datatype Datatype();};
template<int d> struct DATATYPE_HELPER<FACE_INDEX<d> >{static MPI::Datatype Datatype();};
...
template<class T_ARRAY,class T_INDICES> inline void Unpack(INDIRECT_ARRAY<T_ARRAY,T_INDICES>& data,ARRAY_VIEW<const char> buffer,int& position,const MPI::Comm& comm)
{
  ...
  PrintPackTimeStamp("UNPACK END 10");
}
//#####################################################################
...
#ifdef WIN32
//TODO: find equivalent working Pack_Size function if needed
#else
