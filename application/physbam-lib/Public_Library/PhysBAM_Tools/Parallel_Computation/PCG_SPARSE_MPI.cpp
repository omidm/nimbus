...
Serial_Solve(SPARSE_MATRIX_FLAT_NXN<T>& A,VECTOR_ND<T>& x,VECTOR_ND<T>& b,VECTOR_ND<T>& q,VECTOR_ND<T>& s,VECTOR_ND<T>& r,VECTOR_ND<T>& k,VECTOR_ND<T>& z,const int tag,const T tolerance)
{
    ...
    if(rank!=master){
        ...
        if(pcg.show_results){std::stringstream ss;ss<<"solving "<<global_A.n<<" cells to tolerance "<<tolerance<<std::endl;LOG::filecout(ss.str());}
#endif
...
Parallel_Solve(SPARSE_MATRIX_FLAT_NXN<T>& A,VECTOR_ND<T>& x,VECTOR_ND<T>& b,const T tolerance,const bool recompute_preconditioner)
{
    ...
    if(mpi_threaded_grid){Parallel_Solve_MPI_Threaded(A,x,b,tolerance,recompute_preconditioner);return;}
...
//#####################################################################
// Function Parallel_Solve_MPI_Threaded
//#####################################################################
template<class T_GRID> void PCG_SPARSE_MPI<T_GRID>::
Parallel_Solve_MPI_Threaded(SPARSE_MATRIX_FLAT_NXN<T>& A,VECTOR_ND<T>& x,VECTOR_ND<T>& b,const T tolerance,const bool recompute_preconditioner)
{
#ifdef USE_PTHREADS
    Initialize_Datatypes();
    int local_n=A.n,interior_n=partition.interior_indices.Size()+1;
    int global_n=Global_Sum_MPI_Threaded(interior_n);
    T global_tolerance=Global_Max_MPI_Threaded(tolerance);
    int desired_iterations=global_n;if(pcg.enforce_compatibility) desired_iterations--;if(pcg.maximum_iterations) desired_iterations=min(desired_iterations,pcg.maximum_iterations);
    ...
    // adjust x for the null space
    if(pcg.enforce_compatibility && pcg.remove_null_space_solution_component) x_interior-=(T)(Global_Sum_MPI_Threaded(x_interior.Sum_Double_Precision())/global_n);

    // find initial residual, r=b-Ax - reusing b for the residual
    Fill_Ghost_Cells_MPI_Threaded(x);
    A.Times(x,temp);b_interior-=temp_interior;
    if(pcg.enforce_compatibility) b_interior-=(T)(Global_Sum_MPI_Threaded(b_interior.Sum_Double_Precision())/global_n);
    if(Global_Max_MPI_Threaded(b_interior.Max_Abs())<=global_tolerance){
#ifndef COMPILE_WITHOUT_READ_WRITE_SUPPORT
        if(pcg.show_results) LOG::filecout("NO ITERATIONS NEEDED\n");
#endif
        return;}
    ...
    for(int iteration=1;;iteration++){
        ...
        // for Neumann boundary conditions only, make sure z sums to zero
        if(pcg.enforce_compatibility) z_interior-=(T)(Global_Sum_MPI_Threaded(z_interior.Sum_Double_Precision())/global_n);

        // update search direction
        rho_old=rho;rho=Global_Sum_MPI_Threaded(VECTOR_ND<T>::Dot_Product_Double_Precision(z_interior,b_interior));
        ...
        // update solution and residual
        Fill_Ghost_Cells_MPI_Threaded(p);
        A.Times(p,temp);
        T alpha=(T)(rho/Global_Sum_MPI_Threaded(VECTOR_ND<T>::Dot_Product_Double_Precision(p_interior,temp_interior)));
        for(int i=1;i<=interior_n;i++){x_interior(i)+=alpha*p_interior(i);b_interior(i)-=alpha*temp_interior(i);}
...
#ifndef COMPILE_WITHOUT_READ_WRITE_SUPPORT
        T residual=Global_Max_MPI_Threaded(b_interior.Max_Abs());
...
//#####################################################################
// Function Fill_Ghost_Cells_MPI_Threaded
//#####################################################################
template<class T_GRID> void PCG_SPARSE_MPI<T_GRID>::
Fill_Ghost_Cells_MPI_Threaded(VECTOR_ND<T>& v)
{
#ifdef USE_PTHREADS
    Fill_Ghost_Cells_Between_Threads(v);
    for(int s=1;s<=partition.number_of_sides;s++)if(boundary_datatypes(s)!=MPI::DATATYPE_NULL){
        int mpi_neighbor_rank=int(partition.neighbor_ranks(s)/mpi_threaded_grid->threaded_grid->number_of_processes);
        pthread_mutex_lock(mpi_threaded_grid->lock);
        mpi_threaded_grid->mpi_requests_buffers->Append(comm.Isend(v.x-1,1,boundary_datatypes(s),mpi_neighbor_rank,Get_Send_Tag(s)));
        pthread_mutex_unlock(mpi_threaded_grid->lock);}
    for(int s=1;s<=partition.number_of_sides;s++)if(ghost_datatypes(s)!=MPI::DATATYPE_NULL){ 
        int mpi_neighbor_rank=int(partition.neighbor_ranks(s)/mpi_threaded_grid->threaded_grid->number_of_processes);
        pthread_mutex_lock(mpi_threaded_grid->lock);
        mpi_threaded_grid->mpi_requests_buffers->Append(comm.Irecv(v.x-1,1,ghost_datatypes(s),mpi_neighbor_rank,Get_Recv_Tag(s)));
        pthread_mutex_unlock(mpi_threaded_grid->lock);}
    pthread_barrier_wait(mpi_threaded_grid->barr);
    if(mpi_threaded_grid->tid==1) MPI_UTILITIES::Wait_All(*mpi_threaded_grid->mpi_requests_buffers);
    pthread_barrier_wait(mpi_threaded_grid->barr);
#endif
}
//#####################################################################
// Function Get_Send_Tag
//#####################################################################
template<class T_GRID> int PCG_SPARSE_MPI<T_GRID>::
Get_Send_Tag(int s)
{
    TV_INT direction=mpi_threaded_grid->side_neighbor_directions(s);
    return mpi_threaded_grid->Get_Send_Tag(direction);
}
//#####################################################################
// Function Get_Recv_Tag
//#####################################################################
template<class T_GRID> int PCG_SPARSE_MPI<T_GRID>::
Get_Recv_Tag(int s)
{
    TV_INT direction=mpi_threaded_grid->side_neighbor_directions(s);
    return mpi_threaded_grid->Get_Recv_Tag(direction);
}
//#####################################################################
// Function Fill_Ghost_Cells_Between_Threads
//#####################################################################
template<class T_GRID> void PCG_SPARSE_MPI<T_GRID>::
Fill_Ghost_Cells_Between_Threads(VECTOR_ND<T>& x)
{
#ifdef USE_PTHREADS
    int my_mpi_rank=mpi_threaded_grid->mpi_grid->rank;
    // Send out the column values that we owe other people
    for(int s=1;s<=partition.number_of_sides;s++){
         int mpi_neighbor_rank=int(partition.neighbor_ranks(s)/mpi_threaded_grid->threaded_grid->number_of_processes);
         int thread_neighbor_rank=partition.neighbor_ranks(s)%mpi_threaded_grid->threaded_grid->number_of_processes;
         if(partition.neighbor_ranks(s)!=-1 && my_mpi_rank==mpi_neighbor_rank){
             THREAD_PACKAGE pack(partition.boundary_indices(s).m*sizeof(T));
             pack.send_tid=mpi_threaded_grid->tid;pack.recv_tid=thread_neighbor_rank+1;int position=0;
             for(int i=1;i<=partition.boundary_indices(s).m;i++){*(T*)(&pack.buffer(position+1))=x(partition.boundary_indices(s)(i));position+=sizeof(T);}
             pthread_mutex_lock(mpi_threaded_grid->lock);
    ...
    for(int s=1;s<=partition.number_of_sides;s++){
        ...
        int thread_neighbor_rank=partition.neighbor_ranks(s)%mpi_threaded_grid->threaded_grid->number_of_processes;
        if(partition.neighbor_ranks(s)!=-1 && my_mpi_rank==mpi_neighbor_rank){
            int start=partition.ghost_indices(s).min_corner;
            for(int buf=1;buf<=mpi_threaded_grid->buffers->m;buf++){
                if(mpi_threaded_grid->tid!=(*mpi_threaded_grid->buffers)(buf).recv_tid || thread_neighbor_rank!=(*mpi_threaded_grid->buffers)(buf).send_tid-1) continue;
                int position=0;
                for(int i=1;i<=partition.ghost_indices(s).Size()+1;i++){
                    x(start+i-1)=*(T*)(&(*mpi_threaded_grid->buffers)(buf).buffer(position+1));
                    position+=sizeof(T);}}}}
    pthread_barrier_wait(mpi_threaded_grid->barr);
...
{
#ifdef USE_PTHREADS
    if(mpi_threaded_grid) pthread_mutex_lock(mpi_threaded_grid->lock);
#endif
...
#ifdef USE_PTHREADS
    if(mpi_threaded_grid) pthread_mutex_unlock(mpi_threaded_grid->lock);
#endif
    boundary_datatypes.Resize(partition.number_of_sides);ghost_datatypes.Resize(partition.number_of_sides);
#ifdef USE_PTHREADS
    if(mpi_threaded_grid){
        for(int s=1;s<=partition.number_of_sides;s++){ 
            int mpi_neighbor_rank=int(partition.neighbor_ranks(s)/mpi_threaded_grid->threaded_grid->number_of_processes);
            int current_mpi_rank=mpi_threaded_grid->mpi_grid->rank;
            if(partition.neighbor_ranks(s)!=-1 && current_mpi_rank!=mpi_neighbor_rank){
...
Fill_Ghost_Cells_Threaded(VECTOR_ND<T>& x)
{
    ...
    for(int node_rank=1;node_rank<=columns_to_send.m;node_rank++){
        if(node_rank-1!=my_rank){
            ...
            for(int i=1;i<=columns_to_send(node_rank).m;i++){pack.buffer(position+1)=(char)x(columns_to_send(node_rank)(i));position+=sizeof(T);}
            pthread_mutex_lock(thread_grid->lock);
            thread_grid->buffers.Append(pack);
            pthread_mutex_unlock(thread_grid->lock);}}
    // Receive the column values that others owe us
