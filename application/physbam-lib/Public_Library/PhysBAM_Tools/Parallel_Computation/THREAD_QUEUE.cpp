    ...
    :threads(thread_count),active_threads(thread_count),inactive_threads(0),queue(65535)
{
    ...
    for(int i=1;i<=threads.m;i++)
        threadStates.Append(PAIR<THREAD_QUEUE*,int>(this,i));
    for(int i=1;i<=threads.m;i++){
        pthread_create(&threads(i),0,Thread_Routine,&threadStates(i));
#ifndef WIN32 //TODO: Allow setting thread affinity on Windows, non-portable pthread_win32_thread_attach_np() may work
        if(set_affinity){
