...
DATATYPE_HELPER<PAIR<VECTOR<PAIR<int,TV_INT>,2>,T> >::Datatype()
{
    static MPI::Datatype datatype=MPI::DATATYPE_NULL;
    if(datatype==MPI::DATATYPE_NULL){
        static PAIR<VECTOR<PAIR<int,TV_INT>,2>,T> zero_entry;static int lengths[3]={1,1,1};
        static MPI::Aint displacements[3]={0,(char*)&zero_entry.y-(char*)&zero_entry,sizeof(zero_entry)};
        static MPI::Datatype old_types[3]={MPI_UTILITIES::Datatype<VECTOR<PAIR<int,TV_INT>,2> >(),MPI_UTILITIES::Datatype<T>(),MPI::UB};
        datatype=MPI::Datatype::Create_struct(3,lengths,displacements,old_types);
        datatype.Commit();}
    return datatype;
}
