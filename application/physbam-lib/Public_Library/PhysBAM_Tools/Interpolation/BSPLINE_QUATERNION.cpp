...
Create_Closed_Points()
{
    ...
    for(int i=k-1;i>=1;i--){
        ...
        ss1<<"Comparing "<<i<<": "<<control_points(i)<<" and "<<i+1<<": "<<control_points(i+1)<<std::endl;
        LOG::filecout(ss1.str());
#endif
        control_points(i)=ROTATION<TV>::Switch_Hemisphere(control_points(i+1),control_points(i));
#ifndef COMPILE_WITHOUT_READ_WRITE_SUPPORT
        std::stringstream ss2;
        ss2<<"   result: "<<control_points(i)<<std::endl;
        LOG::filecout(ss2.str());
#endif
    }
    if(k>2) for(int i=control_points.m-(k-2);i<control_points.m;i++){
#ifndef COMPILE_WITHOUT_READ_WRITE_SUPPORT
        std::stringstream ss3;
        ss3<<"Comparing "<<i<<": "<<control_points(i)<<" and "<<i+1<<": "<<control_points(i+1)<<std::endl;
        LOG::filecout(ss3.str());
#endif
        control_points(i)=ROTATION<TV>::Switch_Hemisphere(control_points(i+1),control_points(i));
#ifndef COMPILE_WITHOUT_READ_WRITE_SUPPORT
        std::stringstream ss4;
        ss4<<"   result: "<<control_points(i)<<std::endl;
        LOG::filecout(ss4.str());
#endif
    }
}
//#####################################################################
