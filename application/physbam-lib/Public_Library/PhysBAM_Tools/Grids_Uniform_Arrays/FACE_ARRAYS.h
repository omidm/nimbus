...
namespace PhysBAM {
    ...
    template<class T, int d>
        class ARRAY<T, FACE_INDEX<d> > {
            ...
            template<class S> struct ELEMENT_OF {
                typedef typename S::ELEMENT TYPE;
            };

            typedef typename IF<
                IS_VECTOR<T>::value,
                ELEMENT_OF<T>,
                FIRST<UNUSABLE> >::TYPE::TYPE ELEMENT_OF_T;
            ...
            template<class T2> struct REBIND {
                typedef ARRAY<T2, FACE_INDEX<dimension> > TYPE;
            };

            typedef ARRAY_VIEW<T, TV_INT> T_ARRAY_VIEW;
            typedef T ELEMENT;
            typedef FACE_INDEX<d> INDEX;
            ...
            template<class T2>
                ARRAY(const GRID<VECTOR<T2, dimension> >& grid, const int ghost_cells = 0,
            ...
            ARRAY(const ARRAY& old_array) :
                base_pointer(0), buffer_size(0) {
                    Resize(old_array.Domain_Indices(), false, false);
                    Copy(old_array, *this);
                }
            ...
            ARRAY& operator=(const ARRAY& source) {
                if (source.Domain_Indices() != Domain_Indices())
                    Resize(source.Domain_Indices(), false, false);
                Copy(source, *this);
                return *this;
            }
            ...
            virtual void Resize(
                ...
                for (int i = 1; i <= dimension; i++) {
                    ...
                    if (domains(i) != data(i).Domain_Indices())
                        early_return = false;
                }
                if (early_return)
                    return;
                ...
                if (copy_existing_elements) {
                    ...
                    for (int i = 1; i <= dimension; i++) {
                        T_ARRAY_VIEW array_new(domains(i), p_start);
                        T_ARRAY_VIEW::Limited_Shifted_Get(
                                array_new,
                                data(i),
                                TV_INT());
                        p_start += sizes_new(i);
            ...
            void Clean_Memory() {
                Resize(RANGE<TV_INT>(TV_INT::All_Ones_Vector(), TV_INT()), false,
                        false);
            }

            void Delete_Pointers_And_Clean_Memory() {
                for (int i = 0; i < buffer_size; i++)
                    delete base_pointer[i];
                Clean_Memory();
            }
	    ...
	    void Nimbus_Delete_Base_Pointer() {
	      delete[] base_pointer;
            ...
            bool Valid_Index(const FACE_INDEX<dimension>& index) const {
                return (1 <= index.axis && index.axis <= dimension)
                    && Component(index.axis).Valid_Index(index.index);
            }

            T_ARRAY_VIEW& Component(const int axis) {
                assert(1 <= axis && axis <= dimension);
                return *(&data.x + (axis - 1));
            }
            ...
            TV Cell_Centered_Average(const TV_INT& cell_index) const {
                ...
                for (int i = 1; i <= dimension; i++)
                    average(i) = (T) .5
                        * (data(i)(cell_index)
                                + data(i)(cell_index + TV_INT::Axis_Vector(i)));
                return average;
            }

            void Set_All_Faces(const T& value, const TV_INT& cell_index) {
                for (int i = 1; i <= dimension; i++)
                    data(i)(cell_index) = data(i)(cell_index + TV_INT::Axis_Vector(i)) =
                        value;
            }

            static void Extract_Dimension(const ARRAY& old_array,
                    ARRAY<ELEMENT_OF_T, FACE_INDEX<dimension> >& extracted_array,
                    const TV_INT& dimensions_to_extract) {
                STATIC_ASSERT(IS_VECTOR<T>::value);
                for (int i = 1; i <= dimension; i++)
                    T_ARRAY_VIEW::Extract_Dimension(old_array.data(i),
                            extracted_array.data(i), dimensions_to_extract(i));
            }

            void Fill(const T& constant) {
                for (int i = 1; i <= dimension; i++)
                    data(i).Fill(constant);
            }

            static void Copy(const ARRAY& old_copy, ARRAY& new_copy) {
                for (int i = 1; i <= dimension; i++)
                    T_ARRAY_VIEW::Copy(old_copy.data(i), new_copy.data(i));
            }

            template<class T2>
                static void Copy(const T2 c, const ARRAY& old, ARRAY& result) {
                    for (int i = 1; i <= dimension; i++)
                        T_ARRAY_VIEW::Copy(c, old.data(i), result.data(i));
                }
            ...
            template<class T2>
                static void Copy(const T2 c1, const ARRAY& v1, const T2 c2, const ARRAY& v2,
                        ARRAY& result) {
                    for (int i = 1; i <= dimension; i++)
                        T_ARRAY_VIEW::Copy(c1, v1.data(i), c2, v2.data(i), result.data(i));
                }

            static void Put(const ARRAY& old_copy, ARRAY& new_copy) {
                for (int i = 1; i <= dimension; i++)
                    T_ARRAY_VIEW::Put(old_copy.data(i), new_copy.data(i));
            }

            TV Maxabs() const {
                TV maxabs_vector;
                for (int i = 1; i <= dimension; i++)
                    maxabs_vector(i) = data(i).Maxabs();
                return maxabs_vector;
            }

            static void Exchange_Arrays(ARRAY& a, ARRAY& b) {
                exchange(a.domain_indices, b.domain_indices);
                exchange(a.base_pointer, b.base_pointer);
                exchange(a.buffer_size, b.buffer_size);
                for (int i = 1; i <= dimension; i++)
		...
		T_ARRAY_VIEW::Nimbus_Clear_Array(a.data(i));
            ...
            template<class T_INDICES>
                INDIRECT_ARRAY<const ARRAY, T_INDICES&> Subset(
                        const T_INDICES& indices) const {
