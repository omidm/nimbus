...
public:
    ...
    MATRIX_MXN<T> Inverse()
    {if(Rows()>=Columns()){MATRIX_MXN<T> ATA=Transposed()*(*this);ATA.LU_Inverse(ATA);ATA=ATA*Transposed();return ATA;}
    else {MATRIX_MXN<T> AAT=(*this)*Transposed();AAT.LU_Inverse(AAT);AAT=Transposed()*AAT;return AAT;}}
