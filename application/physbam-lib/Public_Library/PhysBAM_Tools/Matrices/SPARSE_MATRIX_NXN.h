...
public:
    ...
    for(int i=1;i<=n;i++){
        ...
        if(row(i)<=0){
            ...
            std::stringstream ss;
            ss<<"Diagonal Entry "<<i<<" Contains Non Positive Element: "<<row(i)<<std::endl<<row;
            LOG::filecout(ss.str());
#endif
            return false;}
...
#ifndef COMPILE_WITHOUT_READ_WRITE_SUPPORT
            std::stringstream ss;
            ss<<"Sum Of Row "<<i<<" Is Negative: "<<sum<<std::endl<<row;
            LOG::filecout(ss.str());
#endif
            return_value=false;}}
    return return_value;}
    ...
    {std::stringstream ss;for(int i=1;i<=n;i++)ss<<A(i)->number_of_active_indices<<" ";ss<<std::endl;LOG::filecout(ss.str());}
#endif
