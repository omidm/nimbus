//#####################################################################
// Copyright 2007-2012, Ronald Fedkiw, Geoffrey Irving, Igor Neverov, Craig Schroeder, Rahul Sheth, Tamar Shinar, Eftychios Sifakis, Huamin Wang, Rachel Weinstein.
...
public:
    ...
    template<class T_VECTOR> 
    typename PRODUCT<typename VECTOR_TYPE<T_VECTOR>::TYPE,T_MATRIX>::TYPE Left_Multiply_By_Vector(const VECTOR_BASE<T,T_VECTOR>& v) const
    {WARN_IF_NOT_EFFICIENT(T_MATRIX);assert(v.Size()==Rows());typename PRODUCT<typename VECTOR_TYPE<T_VECTOR>::TYPE,T_MATRIX>::TYPE b((INITIAL_SIZE)Columns());
    for(int i=1;i<=b.Size();i++){b(i)=0;for(int j=1;j<=Rows();j++){b(i)+=v(j)*(*this)(j,i);}}return b;}
    ...
    template<class T_VECTOR>
    void Get_Diagonal(VECTOR_BASE<T,T_VECTOR>& v, const int index=0){
    if(-Rows()<=index && index<=0){
        if(Rows()+index>Columns()) assert(v.Size()==Columns());
        else assert(v.Size()==Rows()+index);
        for(int i=1;i<=v.Size();i++){v(i)=(*this)(i-index,i);}}
    else if(1<=index && index<=Columns()){
        if(Rows()>Columns()-index) assert(v.Size()==Columns()-index);
        else assert(v.Size()==Rows());
        for(int i=1;i<=v.Size();i++){v(i)=(*this)(i,i+index);}}
    else assert(false);}

    void Get_Diagonal(VECTOR_ND<T>& v, const int index=0){
    if(-Rows()<=index && index<=0){
        if(Rows()+index>Columns()) v.Resize(Columns());
        else v.Resize(Rows()+index);
        for(int i=1;i<=v.Size();i++){v(i)=(*this)(i-index,i);}}
    else if(1<=index && index<=Columns()){
        if(Rows()>Columns()-index) v.Resize(Columns()-index);
        else v.Resize(Rows());
        for(int i=1;i<=v.Size();i++){v(i)=(*this)(i,i+index);}}}

    template<class T_VECTOR>
    void Set_Diagonal(const VECTOR_BASE<T,T_VECTOR>& v, const int index=0){
    if(-Rows()<=index && index<=0){
        if(Rows()+index>Columns()) assert(v.Size()==Columns());
        else assert(v.Size()==Rows()+index);
        for(int i=1;i<=v.Size();i++){(*this)(i-index,i)=v(i);}}
    else if(1<=index && index<=Columns()){
        if(Rows()>Columns()-index) assert(v.Size()==Columns()-index);
        else assert(v.Size()==Rows());
        for(int i=1;i<=v.Size();i++){(*this)(i,i+index)=v(i);}}
    else assert(false);}
//#####################################################################
    ...
    template<class T_VECTOR1> void Get_Singular_Values(VECTOR_BASE<T,T_VECTOR1>& v,const T tolerance=(T)1e-10,const unsigned max_iterations=1000000);
    template<class T_MATRIX2, class T_MATRIX3, class T_MATRIX4> void Singular_Value_Decomposition(MATRIX_BASE<T,T_MATRIX2>& U, MATRIX_BASE<T,T_MATRIX3>& E, MATRIX_BASE<T,T_MATRIX4>& V, 
        const T tolerance=(T)1e-10,const unsigned max_iterations=1000000);
    template<class T_MATRIX2, class T_MATRIX3, class T_MATRIX4> void Upper_Bidiagonalization(MATRIX_BASE<T,T_MATRIX2>& U, MATRIX_BASE<T,T_MATRIX3>& B, MATRIX_BASE<T,T_MATRIX4>& V);
    void Flip_Horizontal();
    void Flip_Vertical();    
//#####################################################################
};
template<class T, class T_MATRIX> inline void Pretty_Print(std::ostream& output_stream,const MATRIX_BASE<T,T_MATRIX>& A)
{for(int i=1;i<=A.Rows();i++){output_stream<<"|";for(int j=1;j<=A.Columns();j++){output_stream<<A(i,j)<<"\t";}output_stream<<"|"<<std::endl;}}
...
template<class T,class T_MATRIX> inline std::istream& operator>>(std::istream& input_stream, MATRIX_BASE<T,T_MATRIX>& A)
{char garbage;input_stream>>garbage; /*get rid of "["*/for(int i=1;i<=A.Rows();i++){for(int j=1;j<=A.Columns();j++){input_stream>>A(i,j);}if(i<A.Rows()) input_stream>>garbage;/*";"*/;}input_stream>>garbage;/*"]"*/return input_stream;}
