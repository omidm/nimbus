...
Update_Upper_Triangular_Matrix_After_Column_Shift(MATRIX_MXN<T>& A,MATRIX_MXN<T>& N,VECTOR_ND<T>& b,const int column,const T tolerance,const bool check_last_column)
{
    ...
    if(check_last_column && abs(A(A.n,A.n))<tolerance){
        std::stringstream ss;
        ss << "Found near-zero on diagonal during attempt to update upper triangular matrix!" << std::endl;
        ss << A << std::endl;
        LOG::filecout(ss.str());
        /*PHYSBAM_FATAL_ERROR()*/}
#endif
    ...
    const ARRAY<PAIR<bool,T> >& x_min,const ARRAY<PAIR<bool,T> >& x_max,const T tolerance,const T step_tolerance,bool verbose)
{
    ...
    for(;;iteration++){
        ...
        if(verbose){
            ...
            ss<<"permute_B:\n"<<permute_B<<"\npermute_N:\n"<<permute_N<<"\nx_B:\n"<<x_B<<std::endl;
            MATRIX_MXN<T> inverse;B.LU_Inverse(inverse);ss<<"B^-1 * N:\n"<<inverse*N<<"\nB^-1 * b:\n"<<inverse*b<<std::endl;
            LOG::filecout(ss.str());}
#endif
...
#ifndef COMPILE_WITHOUT_READ_WRITE_SUPPORT
        if(verbose) {std::stringstream ss;ss<<"c_B =\n"<<c_B<<"pi =\n"<<pi<<"sigma =\n"<<sigma<<std::endl;LOG::filecout(ss.str());}
#endif
        ...
        if(verbose) {std::stringstream ss;ss<< "Releasing index " << index_to_release << " (muscle " << permute_N(index_to_release) << ")" << std::endl;LOG::filecout(ss.str());}
#endif
...
#ifndef COMPILE_WITHOUT_READ_WRITE_SUPPORT
        if(verbose) {std::stringstream ss;ss<<"Got p_B\n"<<p_B<<"\nGot p_N\n"<<p_N<<std::endl;LOG::filecout(ss.str());}
#endif
        ...
        if(verbose) {
            std::stringstream ss;
            ss<<"alpha="<<alpha<<", limiting_index="<<limiting_index<<", limiting_value="<<limiting_value<<"\nx_in_feasible_region = "<<x_in_feasible_region<<std::endl;
            LOG::filecout(ss.str());}
#endif
...
#ifndef COMPILE_WITHOUT_READ_WRITE_SUPPORT
            std::stringstream ss;
            ss << "No limiting index, picking alpha="<<alpha<<" (reaching index"<<limiting_index<<")"<<std::endl;
            LOG::filecout(ss.str());
#endif
            break;} // TODO: how handle this case?
        ...
        if(limiting_index==-1){ // p_N(i) limits alpha, switch x_N to be opposite end's constraint
            ...
            if(verbose){
                std::stringstream ss;
                ss << "Active constraint " << index_to_release << " (muscle " << permute_N(index_to_release) << ") switched to bound " << limiting_value << std::endl;
                LOG::filecout(ss.str());}
#endif
            ...
            if(verbose){
                std::stringstream ss;
                ss << "limiting_index="<<limiting_index<<" (muscle " << permute_B(limiting_index) << ") getting constrained to " << limiting_value 
                          << ", index_to_release="<<index_to_release<<" (muscle " << permute_N(index_to_release) << ") released from " << x_N(index_to_release) << ")" << std::endl;
                ss << "BEFORE COLUMN SWITCH ("<<limiting_index<<","<<index_to_release<<")\nB:\n"<<B<<"\nN:\n"<<N<<std::endl;
                LOG::filecout(ss.str());}
#endif
            ...
            if(verbose){
                std::stringstream ss;
                ss << "AFTER MAKING UPPER TRIANGULAR\nB:\n"<<B<<"\nN:\n"<<N<<std::endl;
                LOG::filecout(ss.str());}
#endif
        }
    }
...
#ifndef COMPILE_WITHOUT_READ_WRITE_SUPPORT
        LOG::filecout("PHYSBAM_WARNING: clamping values into feasible region after unsuccessful LP solve\n");
#endif
...
#ifndef COMPILE_WITHOUT_READ_WRITE_SUPPORT
    std::stringstream ss;
    ss << "LP finished in " << iteration << " iterations" << std::endl;
    if(verbose) ss<<"LP Result x_B:\n"<<x_B<<"\nLP Result permute:\n"<<permute<<std::endl;
    LOG::filecout(ss.str());
#endif
}
//####################################################################################
