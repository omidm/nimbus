...
public:
    ...
    LOG_ENTRY* Get_New_Scope(LOG_CLASS& instance,const std::string& new_scope_identifier,const std::string& new_name,const int threadid)
    ...
    LOG_ENTRY* Get_New_Item(LOG_CLASS& instance,const std::string& new_name,const int threadid)
    {int entry;end_on_separate_line=true;log_file_end_on_separate_line=true;
    ...
    LOG_ENTRY* Get_Pop_Scope(LOG_CLASS& instance,const int threadid)
    {Stop(instance,threadid);return parent;}

    void Start_XML(const LOG_CLASS& instance) PHYSBAM_OVERRIDE
