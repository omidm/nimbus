...
public:
    DUALCONTOUR_2D(LEVELSET_2D<GRID<VECTOR<T,2> > >& levelset_input,const T contour_value_input=0,const bool is_distance_field_input=false)
...
//#####################################################################
    static SEGMENTED_CURVE_2D<T>* Create_Surface_From_Levelset(LEVELSET_2D<GRID<TV> >& levelset)
    {return Create_Segmented_Curve_From_Levelset(levelset);}
};
}
#endif
