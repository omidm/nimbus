...
//#####################################################################
    void Set_External_Velocities(ARRAY_VIEW<TV> V,ARRAY_VIEW<T_SPIN> angular_velocity,const T velocity_time,const T current_position_time);
    void Set_External_Velocities(ARRAY_VIEW<TWIST<TV> > twist,const T velocity_time,const T current_position_time);
    virtual void Set_External_Velocities(TV& V,T_SPIN& angular_velocity,const T time,const int id);
    virtual void Set_Kinematic_Velocities(TV& V,T_SPIN& angular_velocity,const T frame_dt,const T time,const int id);
