...
//#####################################################################
    ...
    virtual void Set_External_Velocities(ARRAY_VIEW<TV> V,ARRAY_VIEW<T_SPIN> angular_velocity,const T velocity_time,const T current_position_time); // velocity and angular velocity
    virtual bool Set_Kinematic_Positions(FRAME<TV>& frame,const T time,const int id); //Return true if frame was set
    virtual bool Set_Kinematic_Velocities(TV& V,T_SPIN& angular_velocity,const T time,const int id); // Return true if twist was set
