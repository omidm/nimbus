...
Closest_Point_On_Boundary(const TV& location,const T max_distance,const T thickness_over_2,const int max_iterations,int* simplex_id,T* distance) const
{
    /*if(rigid_geometry.implicit_object && rigid_geometry.simplicial_object){
        ...
        if(INTERSECTION::Intersects(ray,*rigid_geometry.simplicial_object,thickness_over_2)){
            ...
            return Simplex_Closest_Point_On_Boundary(location,max_distance,(T).5*collision_thickness,simplex_id,distance);
    }
    else*/
...
//##################################################################### 
template<class T> bool Simplex_Closest_Non_Intersecting_Point_Helper(RIGID_GEOMETRY<VECTOR<T,1> >& rigid_geometry,RAY<VECTOR<T,1> >& ray,const T collision_thickness_over_two)
{
    RAY<VECTOR<T,1> > object_space_ray=rigid_geometry.Object_Space_Ray(ray);
    if(INTERSECTION::Closest_Non_Intersecting_Point(object_space_ray,*rigid_geometry.simplicial_object,collision_thickness_over_two)){
        ray.semi_infinite=false;ray.t_max=object_space_ray.t_max;ray.aggregate_id=object_space_ray.aggregate_id;return true;}
    return false;
}
template<class T> bool Simplex_Closest_Non_Intersecting_Point_Helper(RIGID_GEOMETRY<VECTOR<T,2> >& rigid_geometry,RAY<VECTOR<T,2> >& ray,const T collision_thickness_over_two)
{
...
template<class T> bool Simplex_Closest_Non_Intersecting_Point_Helper(RIGID_GEOMETRY<VECTOR<T,3> >& rigid_geometry,RAY<VECTOR<T,3> >& ray,const T collision_thickness_over_two)
{
...
{
    return Simplex_Closest_Non_Intersecting_Point_Helper(rigid_geometry,ray,(T).5*collision_thickness);
}
//##################################################################### 
// Function Inside_Any_Simplex
//##################################################################### 
template<class T> bool Inside_Any_Simplex_Helper(RIGID_GEOMETRY<VECTOR<T,1> >& rigid_geometry,const VECTOR<T,1>& location,int& segment_id,const T collision_thickness_over_two)
{
    return rigid_geometry.simplicial_object->Inside_Any_Simplex(rigid_geometry.Object_Space_Point(location),segment_id,collision_thickness_over_two);
}
template<class T> bool Inside_Any_Simplex_Helper(RIGID_GEOMETRY<VECTOR<T,2> >& rigid_geometry,const VECTOR<T,2>& location,int& segment_id,const T collision_thickness_over_two)
{
    return rigid_geometry.simplicial_object->Inside_Any_Segment(rigid_geometry.Object_Space_Point(location),segment_id,collision_thickness_over_two);
}
template<class T> bool Inside_Any_Simplex_Helper(RIGID_GEOMETRY<VECTOR<T,3> >& rigid_geometry,const VECTOR<T,3>& location,int& triangle_id,const T collision_thickness_over_two)
{
    return rigid_geometry.simplicial_object->Inside_Any_Triangle(rigid_geometry.Object_Space_Point(location),triangle_id,collision_thickness_over_two);
}
template<class TV> bool RIGID_COLLISION_GEOMETRY_BASE<TV>::
Inside_Any_Simplex(const TV& location,int& simplex_id) const
{
    return Inside_Any_Simplex_Helper(rigid_geometry,location,simplex_id,(T).5*collision_thickness);
}
//##################################################################### 
// Function Inside
//##################################################################### 
...
{
    if(rigid_geometry.implicit_object) return Implicit_Geometry_Lazy_Inside(location,thickness_over_two);
