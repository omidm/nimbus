...
Initialize(int point_number)
{
    ...
    Initialize_Sampling();
    std::stringstream ss;
    ss<<"Initializing Vicinity Triangles"<<std::endl;Initialize_Vicinity_Triangles();
    ss<<"Initializing Segment Mesh"<<std::endl;surface.mesh.Initialize_Segment_Mesh();
    LOG::filecout(ss.str());
}
//##########################################################################################
...
Initialize_Vicinity_Triangles()
{
    ...
    std::stringstream ss;
    ss<<"Vicinity triangle list size: "<<vicinity_triangles.m<<std::endl;
    for(int t=1;t<=vicinity_triangles.m;++t){
        ...
        new_progress=(int)100.0*t/vicinity_triangles.m;if(new_progress>progress){progress=new_progress;ss<<progress<<"%"<<" ";}}
    ss<<std::endl;LOG::filecout(ss.str());average_number_vicinity_triangles/=vicinity_triangles.m;
}
//##########################################################################################
