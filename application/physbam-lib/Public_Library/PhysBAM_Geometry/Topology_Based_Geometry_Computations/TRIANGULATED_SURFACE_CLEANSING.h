...
#ifndef COMPILE_WITHOUT_READ_WRITE_SUPPORT
        if(verbose) ss<<"MERGING VERTICES (threshold="<<merge_coincident_vertices_threshold<<")"<<std::endl;
#endif
        int number_merged=0;
        ...
        while(particle_spatial_partition.Get_Next_Particles_Potentially_Within_Interaction_Radius(index1,nearby_particle_indices)) if((*ts.mesh.node_on_boundary)(index1)){
            ...
            if(closest_index2){ // merge index1 to closest_index2
                ...
                if(closest_distance_squared<=sqr(real_threshold)){
                    number_merged++;
#ifndef COMPILE_WITHOUT_READ_WRITE_SUPPORT
                    if(verbose) ss<<index1<<"->"<<closest_index2<<" "<<std::flush;
#endif
...
#ifndef COMPILE_WITHOUT_READ_WRITE_SUPPORT
        if(verbose) ss<<std::endl<<number_merged<<" vertices merged"<<std::endl<<std::endl;
#endif
        ts.Refresh_Auxiliary_Structures();}

    if(fill_holes){
#ifndef COMPILE_WITHOUT_READ_WRITE_SUPPORT
        if(verbose) ss<<"HOLE FILLING"<<std::endl;
#endif
...
#ifndef COMPILE_WITHOUT_READ_WRITE_SUPPORT
            if(verbose){ss<<"Adding particle "<<new_particle_index<<": "<<ts.particles.X(new_particle_index)<<std::endl;ss<<"Adding triangles: "<<std::flush;}
#endif
...
#ifndef COMPILE_WITHOUT_READ_WRITE_SUPPORT
                if(verbose) ss<<"("<<node2<<","<<node1<<","<<new_particle_index<<") "<<std::flush;
#endif
                ts.mesh.elements.Append(VECTOR<int,3>(node2,node1,new_particle_index));}
#ifndef COMPILE_WITHOUT_READ_WRITE_SUPPORT
            if(verbose) ss<<std::endl<<std::endl;
#endif
        }
