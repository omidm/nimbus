...
Calculate_Boundary_From_Levelset_On_Nodes(EMBEDDED_OBJECT<TV,d>& eo,ARRAY<T>& phi,const bool discard_elements_outside_levelset,const bool verbose)
{
    ...
    if(eo.embedded_particles.active_indices.m){
        ...
        std::stringstream ss;
        ss<<"Calculate_Boundary_From_Levelset_On_Nodes cannot be called with existing embedded particles"<<std::endl;
        LOG::filecout(ss.str());
#endif
        PHYSBAM_FATAL_ERROR();}
    ...
    if(verbose){
        std::stringstream ss;
        ss << "total particles = " << eo.particles.array_collection->Size() << std::endl;
        ss << "total elements = " << eo.simplicial_object.mesh.elements.m << std::endl;
        ss << "total embedded subelements = " << eo.embedded_mesh.elements.m << std::endl;
        LOG::filecout(ss.str());}
#endif
}
}
