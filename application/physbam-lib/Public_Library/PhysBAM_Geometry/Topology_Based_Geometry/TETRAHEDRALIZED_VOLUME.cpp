...
Check_Signed_Volumes_And_Make_Consistent(bool verbose)
{
    for(int t=1;t<=mesh.elements.m;t++){int i,j,k,l;mesh.elements(t).Get(i,j,k,l);
        ...
        if(sign_of_volume < 0){
            ...
            if(verbose){std::stringstream ss;ss<<"tetrahedron number "<<t<<" is oriented improperly."<<std::endl;LOG::filecout(ss.str());}
#endif
...
//#####################################################################
template<class T> template<class T2> bool TETRAHEDRALIZED_VOLUME<T>::
Barycentric_Interpolate(const VECTOR<T,3>& x,ARRAY_VIEW<T2> values,T2& result)const
{
    int element_index=Inside(x);
    if(element_index==0) return false;
    VECTOR<T,4> weights=TETRAHEDRON<T>::Barycentric_Coordinates(x,particles.X.Subset(mesh.elements(element_index)));
    INDIRECT_ARRAY<ARRAY_VIEW<T2>,VECTOR<int,4>&> sub_values=values.Subset(mesh.elements(element_index));
    result=weights(1)*sub_values(1)+weights(2)*sub_values(2)+weights(3)*sub_values(3)+weights(4)*sub_values(4);
    return true;
}
//#####################################################################
template class TETRAHEDRALIZED_VOLUME<float>;
template bool TETRAHEDRALIZED_VOLUME<float>::Barycentric_Interpolate(const VECTOR<float,3>&,ARRAY_VIEW<float>,float&)const;
template bool TETRAHEDRALIZED_VOLUME<float>::Barycentric_Interpolate(const VECTOR<float,3>&,ARRAY_VIEW<VECTOR<float,3> >,VECTOR<float,3>&)const;
#ifndef COMPILE_WITHOUT_DOUBLE_SUPPORT
template class TETRAHEDRALIZED_VOLUME<double>;
template bool TETRAHEDRALIZED_VOLUME<double>::Barycentric_Interpolate(const VECTOR<double,3>&,ARRAY_VIEW<double>,double&)const;
template bool TETRAHEDRALIZED_VOLUME<double>::Barycentric_Interpolate(const VECTOR<double,3>&,ARRAY_VIEW<VECTOR<double,3> >,VECTOR<double,3>&)const;
#endif
