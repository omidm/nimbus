...
Create_Dyadic_Levelset(const int max_depth,const int min_depth,T (*phi_function)(const TV& location),const bool verbose)
{
    ...
    if(verbose){std::stringstream ss;ss<<"Creating Dyadic Structure"<<std::endl;LOG::filecout(ss.str());}
#endif
    while(true){
...
#ifndef COMPILE_WITHOUT_READ_WRITE_SUPPORT
    if(verbose){std::stringstream ss;ss<<"Number of nodes="<<phi.m<<std::endl;LOG::filecout(ss.str());}
#endif
