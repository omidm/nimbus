...
Simplex_Intersection(RAY<TV>& ray,const T collision_thickness) const
{
    ...
    if(INTERSECTION::Intersects(object_space_ray,*simplicial_object,(T).5*collision_thickness)){
...
Print_Names(const RIGID_GEOMETRY_COLLECTION<TV>& rigid_geometry_collection,const ARRAY<int>& ids)
{
    ...
    std::stringstream ss;
    ss<<"{";
    for(int i=1;i<=ids.m;i++){ss<<"\""<<rigid_geometry_collection.particles.rigid_geometry(ids(i))->name<<"\"";if(i<ids.m) ss<<", ";}
    ss<<"}";
    LOG::filecout(ss.str());
#endif
}
...
//#####################################################################
template<class TV> typename BASIC_SIMPLEX_POLICY<TV,VECTOR_POLICY<TV>::DIMMINUSONE>::SIMPLEX 
RIGID_GEOMETRY<TV>::World_Space_Simplex(const int id) const
{
    const VECTOR<int,TV::dimension>& elements=simplicial_object->mesh.elements(id);
    VECTOR<TV,TV::dimension> pts;
    for(int i=1;i<=TV::dimension;i++) pts(i)=World_Space_Point(simplicial_object->particles.X(elements(i)));
