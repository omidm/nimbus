...
public:
    ...
    {return rigid_geometry_collection.particles.V(particle_index);}
    
    const TV& V() const PHYSBAM_ALWAYS_INLINE
    {return rigid_geometry_collection.particles.V(particle_index);}

    T_SPIN& Angular_Velocity() PHYSBAM_ALWAYS_INLINE
    {return rigid_geometry_collection.particles.angular_velocity(particle_index);}
    
    const T_SPIN& Angular_Velocity() const PHYSBAM_ALWAYS_INLINE
    {return rigid_geometry_collection.particles.angular_velocity(particle_index);}

    const TWIST<TV> Twist() const PHYSBAM_ALWAYS_INLINE
    {return TWIST<TV>(V(),Angular_Velocity());}

    void Set_Twist(const TWIST<TV>& twist) PHYSBAM_ALWAYS_INLINE
    {V()=twist.linear;Angular_Velocity()=twist.angular;}
    ...
    bool Simplex_Inside(const TV& location,const T collision_thickness_over_two) const
    {return simplicial_object->Inside(Object_Space_Point(location),collision_thickness_over_two);}

    bool Simplex_Outside(const TV& location,const T collision_thickness_over_two) const
    {return simplicial_object->Outside(Object_Space_Point(location),collision_thickness_over_two);}
...
//#####################################################################
    ...
    bool Simplex_Intersection(RAY<TV>& ray,const T collision_thickness_over_two) const;
    RANGE<TV> World_Space_Simplex_Bounding_Box(const int id) const;
    typename BASIC_SIMPLEX_POLICY<TV,VECTOR_POLICY<TV>::DIMMINUSONE>::SIMPLEX World_Space_Simplex(const int id) const;
