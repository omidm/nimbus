...
//#####################################################################
    ...
    int Add_Rigid_Geometry(const STREAM_TYPE stream_type,const std::string& basename,const T scaling_factor=1,const bool read_simplicial_boundary=true,const bool read_implicit_object=true,const bool read_simplicial_interior=true,const bool read_rgd_file=true);
    int Add_Rigid_Geometry(RIGID_GEOMETRY<TV>* rigid_geometry,const STREAM_TYPE stream_type,const std::string& basename,const T scaling_factor,
    ...
    int Add_Rigid_Geometry(RIGID_GEOMETRY<TV>* rigid_geometry);
