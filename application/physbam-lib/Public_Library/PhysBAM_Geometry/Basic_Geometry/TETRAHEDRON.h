...
public:
    ...
    {TV u=x(2)-x(1),v=x(3)-x(1),w=x(4)-x(1),cross_uv=TV::Cross_Product(u,v),cross_wu=TV::Cross_Product(w,u),
                               cross_vw=TV::Cross_Product(v,w);
    T determinant=TV::Dot_Product(cross_uv,w),uu=u.Magnitude_Squared(),vv=v.Magnitude_Squared(),ww=w.Magnitude_Squared();
    return x(1)+(ww*cross_uv+vv*cross_wu+uu*cross_vw)*((T).5/determinant);}

    TV Circumcenter() const
    {return Circumcenter(X[1],X[2],X[3],X[4]);}
    ...
    {if(Check_Orientation()) return false;exchange(X(4),X(3));return true;}
...
//#####################################################################
    ...
    void Clip_To_Simplex(const TETRAHEDRON<T>& simplex,ARRAY<TETRAHEDRON<T> >& clipped_simplices) const;
