...
{
    if(((PLANE<T>)triangle1).Inside(location,thickness_over_two) && ((PLANE<T>)triangle2).Inside(location,thickness_over_two) && ((PLANE<T>)triangle3).Inside(location,thickness_over_two) && ((PLANE<T>)triangle4).Inside(location,thickness_over_two)) 
        return true;
    else return false;
}
//#####################################################################
...
//#####################################################################
// Function Clip_To_Simplex
//#####################################################################
template<class T> void TETRAHEDRON<T>::
Clip_To_Simplex(const TETRAHEDRON<T>& simplex,ARRAY<TETRAHEDRON<T> >& clipped_simplices) const
{
    // cut with all sides of box
    clipped_simplices.Remove_All();
    clipped_simplices.Append(*this);
    for(int side=1;side<=4;side++){
        TRIANGLE_3D<T> triangle(simplex.X(side),simplex.X(side%4+1),simplex.X((side+1)%4+1));
        TV normal=triangle.Normal();
        if(TV::Dot_Product(normal,simplex.X((side+2)%4+1)-simplex.X(side))>0)
            normal*=-1;
        for(int index=clipped_simplices.Size();index>=1;index--)
        {
            Cut_With_Hyperplane_And_Discard_Outside_Simplices(clipped_simplices(index),PLANE<T>(normal,simplex.X(side)),clipped_simplices);
            clipped_simplices.Remove_Index_Lazy(index);
        }
    }
}
//#####################################################################
