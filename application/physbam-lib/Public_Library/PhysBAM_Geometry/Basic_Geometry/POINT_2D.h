...
public:
    ...
    bool Edge_Edge_Interaction(const POINT_2D<T>& point,const TV& v1,const TV& v2,const T interaction_distance,
        T& distance,VECTOR<T,2>& normal,VECTOR<T,2>& weights,T& relative_speed,bool allow_negative_weights,const T small_number,const bool exit_early=false) const
    {if(!Edge_Edge_Interaction(point,interaction_distance,distance,normal)) return false;
    if(!exit_early){
        Edge_Edge_Interaction_Data(point,v1,v2,distance,normal,small_number);
        relative_speed=VECTOR<T,2>::Dot_Product(v1-v2,normal);} // relative speed is in the normal direction
    return true;}
    ...
    bool Edge_Edge_Interaction(const POINT_2D<T>& point,const INDIRECT_ARRAY<T_ARRAY,VECTOR<int,2>&> V_edges,const T interaction_distance,
        T& distance,VECTOR<T,2>& normal,VECTOR<T,2>& weights,T& relative_speed,bool allow_negative_weights,const T small_number,const bool exit_early=false) const
    {return Edge_Edge_Interaction(point,V_edges(1),V_edges(2),interaction_distance,distance,normal,weights,relative_speed,allow_negative_weights,small_number,exit_early);}

    template<class T_ARRAY>
    bool Edge_Edge_Collision(const POINT_2D<T>& point,const T_ARRAY& V_edges,const T dt,const T collision_thickness,T& collision_time,
    ...
    {return CONTINUOUS_COLLISION_DETECTION_COMPUTATIONS::Edge_Edge_Collision(*this,point,V_edges(1),V_edges(2),dt,collision_thickness,collision_time,normal,weights,relative_speed,
