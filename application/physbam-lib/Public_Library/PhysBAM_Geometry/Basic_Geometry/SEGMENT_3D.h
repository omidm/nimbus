...
public:
    ...
    template<class T_ARRAY>
    bool Edge_Edge_Collision(const SEGMENT_3D<T>& segment,const T_ARRAY& V_edges,const T dt,const T collision_thickness,T& collision_time,
        VECTOR<T,3>& normal,VECTOR<T,2>& weights,T& relative_speed,bool allow_negative_weights,const T small_number=0,const bool exit_early=false) const
    {return Edge_Edge_Collision(segment,V_edges(1),V_edges(2),V_edges(3),V_edges(4),dt,collision_thickness,collision_time,normal,weights,relative_speed,allow_negative_weights,small_number,exit_early);}
...
//#####################################################################
    ...
    VECTOR<T,3> Shortest_Vector_Between_Segments(const SEGMENT_3D<T>& segment,VECTOR<T,2>& weights,bool *boundary=0) const;
    ...
    bool Edge_Edge_Collision(const SEGMENT_3D<T>& segment,const TV& v1,const TV& v2,const TV& v3,const TV& v4,const T dt,
        const T collision_thickness,T& collision_time,VECTOR<T,3>& normal,VECTOR<T,2>& weights,T& relative_speed,bool allow_negative_weights,const T small_number=0,const bool exit_early=false) const;
