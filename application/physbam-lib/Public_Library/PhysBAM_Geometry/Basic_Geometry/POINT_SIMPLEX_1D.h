...
public:
    ...
    POINT_SIMPLEX_1D()
        :x1(0),direction(true),normal(1)
    {}

    POINT_SIMPLEX_1D(const TV& x1,const TV& normal)
        :x1(x1),direction(normal(1)>=0?true:false),normal(normal)
    {}

    POINT_SIMPLEX_1D(const TV& x1,const bool direction)
        :x1(x1),direction(direction),normal(direction?(T)1:(T)-1)
    {}

    template<class T_ARRAY>
    explicit POINT_SIMPLEX_1D(const T_ARRAY& X_input)
        :x1(X_input(1)),direction(true),normal(1)
    {
    ...
    void Clip_To_Outside_Box(const RANGE<TV>& box,ARRAY<POINT_SIMPLEX_1D<T> >& clipped_simplices) const
    {clipped_simplices.Remove_All();
    if(box.Lazy_Outside(x1)) clipped_simplices.Append(*this);}
    ...
    static void Cut_With_Hyperplane_And_Discard_Outside_Simplices(const POINT_SIMPLEX_1D<T>& point,const POINT_SIMPLEX_1D<T>& cutting_plane,ARRAY<POINT_SIMPLEX_1D<T> >& negative_points)
...
template<class T> POINT_SIMPLEX_1D<T> operator*(const FRAME<VECTOR<T,1> >& frame,const POINT_SIMPLEX_1D<T>& point)
{return POINT_SIMPLEX_1D<T>(frame*point.x1);}
