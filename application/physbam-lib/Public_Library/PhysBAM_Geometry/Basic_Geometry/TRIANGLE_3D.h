...
public:
    ...
    {TV x1x2=x2-x1,x1x3=x3-x1,m1=(T).5*(x1+x2),m2=(T).5*(x1+x3),m1m2=m2-m1;
    TV face_normal=(TV::Cross_Product(x2-x1,x3-x1)).Normalized();
    TV x1x2_perp=TV::Cross_Product(face_normal,x2-x1);
    ...
    template<class T_ARRAY>
    static TV Center(const T_ARRAY& x)
    {return (T)one_third*(x(1)+x(2)+x(3));}
    ...
    template<class T_ARRAY>
    bool Point_Face_Collision(const TV& x,const TV& v,const T_ARRAY& V_face,const T dt,const T collision_thickness,T& collision_time,TV& normal,
...
//#####################################################################
    ...
    VECTOR<ARRAY<TRIANGLE_3D<T> >,3> Clip_To_Outside_Box_Helper(const int axis,const TV& min,const TV& max,const ARRAY<TRIANGLE_3D<T> >& simplices) const;
    void Clip_To_Outside_Box(const RANGE<TV>& box,ARRAY<TRIANGLE_3D<T> >& clipped_simplices) const;
...
template<class T> TRIANGLE_3D<T> operator*(const FRAME<VECTOR<T,3> >& frame,const TRIANGLE_3D<T>& triangle)
{return TRIANGLE_3D<T>(frame*triangle.x1,frame*triangle.x2,frame*triangle.x3);}
