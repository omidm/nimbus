...
//#####################################################################
// Function Initialize_Sphere_Mesh
//#####################################################################
void TRIANGLE_MESH::
Initialize_Sphere_Mesh(const int m,const int n)
{
    Clean_Memory();int t=0;
    elements.Exact_Resize(2*(m-1)*n);number_nodes=(m-1)*n+2;
    for(int j=1;j<=n;j++){int j_1=j==n?1:j+1;
        for(int i=1;i<=m-2;i++){elements(++t).Set(j+(i-1)*n,j+i*n,j_1+i*n);elements(++t).Set(j+(i-1)*n,j_1+i*n,j_1+(i-1)*n);}
        elements(++t).Set((m-1)*n+1,j,j_1);elements(++t).Set((m-1)*n+2,j_1+(m-2)*n,j+(m-2)*n);}
}
//#####################################################################
