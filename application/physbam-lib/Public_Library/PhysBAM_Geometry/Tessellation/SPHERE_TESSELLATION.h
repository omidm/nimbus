...
template<class T> TRIANGULATED_SURFACE<T>* Generate_Triangles(const ELLIPSOID<T>& ellipsoid,int levels=4);
...
template<class T> SEGMENTED_CURVE_2D<T>* Tessellate_Boundary(const ELLIPSE<T>& ellipse,int levels=4);
//#####################################################################
}
}
#endif
