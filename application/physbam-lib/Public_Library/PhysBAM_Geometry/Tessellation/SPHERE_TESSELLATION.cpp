...
template<class T> TRIANGULATED_SURFACE<T>* Generate_Triangles(const ELLIPSOID<T>& ellipsoid,int levels)
{
    typedef VECTOR<T,3> TV;
    TRIANGULATED_SURFACE<T>* surface=TRIANGULATED_SURFACE<T>::Create();
    GEOMETRY_PARTICLES<TV>& particles=surface->particles;particles.array_collection->Add_Elements(6);
    ...
    surface->mesh.Initialize_Neighbor_Nodes();
    for(int i=1;i<=levels;i++) surface->Root_Three_Subdivide();
    for(int p=1;p<=particles.array_collection->Size();p++){particles.X(p)=ellipsoid.center+ellipsoid.radii*particles.X(p).Normalized();}
    return surface;
}
...
template<class T> SEGMENTED_CURVE_2D<T>* Tessellate_Boundary(const SPHERE<VECTOR<T,2> >& sphere,int levels)
{
    ...
    for(int i=1;i<=n;i++) curve->particles.X(i)=VECTOR<T,2>((T)cos(i*(T)two_pi/n),(T)sin(i*(T)two_pi/n))*sphere.radius+sphere.center;
...
template<class T> SEGMENTED_CURVE_2D<T>* Tessellate_Boundary(const ELLIPSE<T>& ellipse,int levels)
{
    assert(levels>=1);
    SEGMENTED_CURVE_2D<T>* curve=SEGMENTED_CURVE_2D<T>::Create();
    int n=1<<levels;
    curve->particles.array_collection->Add_Elements(n);
    for(int i=1;i<=n;i++) curve->particles.X(i)=VECTOR<T,2>((T)cos(i*(T)two_pi/n)*ellipse.radii(1),(T)sin(i*(T)two_pi/n)*ellipse.radii(2))+ellipse.center;
    for(int i=1;i<n;i++) curve->mesh.elements.Append(VECTOR<int,2>(i,i+1));
    curve->mesh.elements.Append(VECTOR<int,2>(n,1));
    curve->Update_Number_Nodes();
    return curve;
...
template TRIANGULATED_SURFACE<float>* Generate_Triangles(const ELLIPSOID<float>&,int);
...
template SEGMENTED_CURVE_2D<float>* Tessellate_Boundary(const ELLIPSE<float>&,int);
#ifndef COMPILE_WITHOUT_DOUBLE_SUPPORT
template TRIANGULATED_SURFACE<double>* Generate_Triangles(const SPHERE<VECTOR<double,3> >&,int);
template TRIANGULATED_SURFACE<double>* Generate_Triangles(const ELLIPSOID<double>&,int);
...
template SEGMENTED_CURVE_2D<double>* Tessellate_Boundary(const ELLIPSE<double>&,int);
#endif
}
}
