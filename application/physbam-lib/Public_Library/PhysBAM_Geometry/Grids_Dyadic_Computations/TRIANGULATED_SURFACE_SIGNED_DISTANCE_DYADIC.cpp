...
template<class T> void Calculate(TRIANGULATED_SURFACE<T>& surface,const GRID<VECTOR<T,3> >& uniform_grid,OCTREE_GRID<T>& grid,ARRAY<T>& phi,const int maximum_depth,const T half_bandwidth,bool verbose)
{
    ...
    for(int depth=1;;depth++){
        ...
        if(verbose){std::stringstream ss;ss<<"octree depth = "<<depth<<", cells =  "<<grid.number_of_cells<<" cells"<<std::endl;LOG::filecout(ss.str());}
#endif
        // compute phi values
...
#ifndef COMPILE_WITHOUT_READ_WRITE_SUPPORT
    if(verbose){std::stringstream ss;ss<<"total cells = "<<grid.number_of_cells<<" out of "<<(1<<(maximum_depth-1))<<"^3 = "<<(1<<3*(maximum_depth-1))<<" possible"<<std::endl;LOG::filecout(ss.str());}
#endif
