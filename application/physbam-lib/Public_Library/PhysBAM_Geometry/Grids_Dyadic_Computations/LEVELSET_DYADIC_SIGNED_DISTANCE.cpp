...
template<class T_GRID,class T> void Calculate(const LEVELSET_3D<T_GRID>& levelset,OCTREE_GRID<T>& grid,ARRAY<T>& phi,bool verbose)
{
    ...
    while(true){
        ...
        if(verbose){std::stringstream ss;ss<< "total cells = " << grid.number_of_cells << std::endl;LOG::filecout(ss.str());}
#endif
    }
}
//#####################################################################
...
#ifndef COMPILE_WITHOUT_READ_WRITE_SUPPORT
        if(output_statistics){std::stringstream ss;ss<< "octree level = " << level << " - refining " << cells_to_refine.m << " out of " << octree_grid.number_of_cells << " cells" << std::endl;LOG::filecout(ss.str());}
#endif
        if(level < levels)
...
#ifndef COMPILE_WITHOUT_READ_WRITE_SUPPORT
    if(output_statistics){std::stringstream ss;ss<< "total cells = " << octree_grid.number_of_cells << " out of " << pow(2.,level-1) << "^3 = " << pow(pow(2.,level-1),3.) << " possible" << std::endl;LOG::filecout(ss.str());}
#endif
