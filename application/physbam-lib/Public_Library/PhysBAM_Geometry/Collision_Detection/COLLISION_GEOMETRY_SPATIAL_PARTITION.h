...
public:
    ...
    void Get_Potential_Collisions(const ID index,ARRAY<ID>& object_indices,OPERATION_HASH<COLLISION_GEOMETRY_ID>& already_added,bool only_higher_index=false) const
    {POTENTIAL_COLLISIONS::Get_Potential_Collisions<T_COLLISION_GEOMETRY,T_ARRAY,ID>(index,voxel_range(index),hashtable,bodies_not_in_partition,collision_bodies,object_indices,only_higher_index,already_added);}

    void Get_Potential_Collisions_Using_Current_Position(const ID index,ARRAY<ID>& object_indices,OPERATION_HASH<COLLISION_GEOMETRY_ID>& already_added,bool only_higher_index=false) const
    {POTENTIAL_COLLISIONS::Get_Potential_Collisions<T_COLLISION_GEOMETRY,T_ARRAY,ID>(index,Voxel_Range(index),hashtable,bodies_not_in_partition,collision_bodies,object_indices,only_higher_index,already_added);}

    template<class T_RETURN>
    void Get_Potential_Collisions(const TV& location,ARRAY<T_RETURN>& objects,OPERATION_HASH<COLLISION_GEOMETRY_ID>& already_added) const
    {POTENTIAL_COLLISIONS::Get_Potential_Collisions<T_COLLISION_GEOMETRY,T_ARRAY,ID>(Voxel(location),hashtable,bodies_not_in_partition,collision_bodies,objects,already_added);}

    template<class T_RETURN>
    void Get_Potential_Collisions(const RANGE<TV>& box,ARRAY<T_RETURN>& objects,OPERATION_HASH<COLLISION_GEOMETRY_ID>& already_added) const
    {POTENTIAL_COLLISIONS::Get_Potential_Collisions<T_COLLISION_GEOMETRY,T_ARRAY,ID>(ID(-1),Voxel(box),hashtable,bodies_not_in_partition,collision_bodies,objects,false,already_added);}
