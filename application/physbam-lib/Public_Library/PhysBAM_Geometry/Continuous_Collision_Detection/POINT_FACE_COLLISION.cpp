    ...
    T& collision_time,VECTOR<T,3>& normal,VECTOR<T,3>& weights,T& relative_speed,const bool exit_early)
{
    ...
    for(int k=1;k<=num_intervals;k++){
        ...
        if(triangle.Point_Face_Interaction(x+collision_time*v,v,v1,v2,v3,collision_thickness,distance,normal,weights,relative_speed,false,exit_early)) return true;}
    ...
    T& collision_time,VECTOR<T,2>& normal,VECTOR<T,2>& weights,T& relative_speed,const bool exit_early)
{
    ...
    if(abs(1e3*quadratic.a)<abs(quadratic.b)){
        ...
        else if(quadratic.roots==-1){std::stringstream ss;ss<<"VERY SINGULAR ON QUADRATIC SOLVE"<<std::endl;LOG::filecout(ss.str());collision_time_temp=0;}
#endif
