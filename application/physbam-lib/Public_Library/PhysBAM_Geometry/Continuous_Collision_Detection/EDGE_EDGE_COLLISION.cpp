...
Edge_Edge_Collision(const POINT_2D<T>& pt,const POINT_2D<T>& point,const VECTOR<T,2>& v1,const VECTOR<T,2>& v2,const T dt,const T collision_thickness,T& collision_time,
...
{
    VECTOR<T,2> x2_minus_x1=point-pt,v2_minus_v1=v2-v1;
    ...
    if(quadratic.roots==0)return false;
        ...
        std::stringstream ss;
        ss<<"VERY SINGULAR ON QUADRATIC SOLVE"<<std::endl;
        LOG::filecout(ss.str());
#endif
        collision_time=0;}
    else if(quadratic.roots==1)collision_time=(T)quadratic.root1;
    else collision_time=min((T)quadratic.root1,(T)quadratic.root2);
    POINT_2D<T> new_point(pt+collision_time*v1);
    T distance;
    return new_point.Edge_Edge_Interaction(POINT_2D<T>(point+collision_time*v2),v1,v2,collision_thickness,distance,normal,weights,relative_speed,true,small_number,exit_early);
}
//#####################################################################
// Function Edge_Edge_Collision
//#####################################################################
...
Edge_Edge_Collision(const SEGMENT_3D<T>& seg_fault,const SEGMENT_3D<T>& segment,const VECTOR<T,3>& v1,const VECTOR<T,3>& v2,const VECTOR<T,3>& v3,const VECTOR<T,3>& v4,const T dt,
    const T collision_thickness,T& collision_time,VECTOR<T,3>& normal,VECTOR<T,2>& weights,T& relative_speed,bool allow_negative_weights,const T small_number,const bool exit_early)
{
    ...
    cubic.Compute_Roots_In_Interval(xmin,xmax);
    num_intervals=cubic.roots;
    if(!num_intervals) return false;
    
    T distance;
    for(int k=1;k<=num_intervals;k++){
        collision_time=dt*(T)(k==1?cubic.root1:(k==2?cubic.root2:cubic.root3));
        SEGMENT_3D<T> segment2(seg_fault.x1+collision_time*v1,seg_fault.x2+collision_time*v2);
        if(segment2.Edge_Edge_Interaction(SEGMENT_3D<T>(segment.x1+collision_time*v3,segment.x2+collision_time*v4),v1,v2,v3,v4,collision_thickness,distance,normal,weights,relative_speed,false,
                small_number,exit_early)) return true;}
...
template bool CONTINUOUS_COLLISION_DETECTION_COMPUTATIONS::Edge_Edge_Collision<float>(SEGMENT_3D<float> const&,SEGMENT_3D<float> const&,VECTOR<float,3> const&,VECTOR<float,3> const&,
    VECTOR<float,3> const&,VECTOR<float,3> const&,float,float,float&,VECTOR<float,3>&,VECTOR<float,2>&,float&,bool,float,bool const);
template bool CONTINUOUS_COLLISION_DETECTION_COMPUTATIONS::Edge_Edge_Collision<float>(POINT_2D<float> const&,POINT_2D<float> const&,
    VECTOR<float,2> const&,VECTOR<float,2> const&,float const,float const,float&,VECTOR<float,2>&,VECTOR<float,2>&,float&,bool,float const,bool const);
...
template bool CONTINUOUS_COLLISION_DETECTION_COMPUTATIONS::Edge_Edge_Collision<double>(SEGMENT_3D<double> const&,SEGMENT_3D<double> const&,VECTOR<double,3> const&,VECTOR<double,3> const&,
    VECTOR<double,3> const&,VECTOR<double,3> const&,double,double,double&,VECTOR<double,3>&,VECTOR<double,2>&,double&,bool,double,bool const);
template bool CONTINUOUS_COLLISION_DETECTION_COMPUTATIONS::Edge_Edge_Collision<double>(POINT_2D<double> const&,POINT_2D<double> const&,
    VECTOR<double,2> const&,VECTOR<double,2> const&,double const,double const,double&,VECTOR<double,2>&,VECTOR<double,2>&,double&,bool,double const,bool const);
#endif
