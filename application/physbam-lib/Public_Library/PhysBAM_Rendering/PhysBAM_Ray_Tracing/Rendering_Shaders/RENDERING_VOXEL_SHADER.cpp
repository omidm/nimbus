    ...
    use_constant_emission_color(use_constant_emission_color_input),constant_emission_color(constant_emission_color_input)
{
    ...
    std::stringstream ss;
    ss<<"absorption="<<absorption<<", scattering="<<scattering<<", inscattering_amplification="<<inscattering_amplification<<std::endl;
    ss<<"emission_amplification="<<emission_amplification<<", white_point_temperature="<<white_point_temperature<<std::endl;
    ss<<"use_lms_scaling="<<use_lms_scaling<<", use_blackbody_ramp="<<use_blackbody_ramp<<std::endl;
    ss<<"use_constant_emission_color="<<use_constant_emission_color<<", constant_emission_color="<<constant_emission_color<<std::endl;
    LOG::filecout(ss.str());
}
//#####################################################################
// Destructor
...
Attenuate_Light(const RENDERING_RAY<T>& ray,const RENDERING_OBJECT<T>& object,const RENDERING_LIGHT<T>& light,const TV& light_color)
{
    ...
    while(!last_segment){
        ...
        if((!use_empty_implicit_surface_for_light_attenuation || !empty_implicit_surface || !empty_implicit_surface->Lazy_Inside(sample_point)) && !empty_multiple_region){
            ...
            current_t+=current_volumetric_step;}
    //std::stringstream ss;
    //ss<<"Did "<<step_count<<" steps in volumetric voxels"<<std::endl;
    //LOG::filecout(ss.str());
