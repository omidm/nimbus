...
Initialize_Sample_Locations_From_Vertices(const TRIANGULATED_SURFACE<T>& triangulated_surface)
{
    ...
    if(half_samples){
        ...
        for(int ii=1;ii<=samples.m;++ii){
            ...
            if((*triangulated_surface.mesh.incident_elements)(p).m<1) {ss<<"sorry vertex "<<p<<" is broken"<<std::endl;LOG::filecout(ss.str());ss.str("");}
            vertex_area/=3;
...
Build_Octree()
{
    ...
    std::stringstream ss;
    ss<<"Num Irradiance Sample points: "<<samples.m<<std::endl;
    LOG::filecout(ss.str());
