...
public:
         ...
         blue_core_levelset(blue_core_levelset_input),world(world_input)
    {
        ...
        if(use_lms_scaling){
            ...
            ss<<"*****************NOT Using lms"<<std::endl;
            world_xyz_to_display_xyz=MATRIX<T,3>(T(1)/xyz_white_point.x,0,0,0,T(1)/xyz_white_point.y,0,0,0,T(1)/xyz_white_point.z);}
        ss<<"absorption="<<absorption<<", scattering="<<scattering<<", inscattering_amplification_factor="<<inscattering_amplification_factor<<std::endl;
        ss<<"isotropic_scattering="<<isotropic_scattering<<", phase_function_g="<<phase_function_g<<std::endl;
        ss<<"emission_amplification="<<emission_amplification<<", temperature_scale="<<temperature_scale<<", temperature_offset="<<temperature_offset<<std::endl;
        ss<<"clamp_low_temperature="<<clamp_low_temperature<<", temperature_lowest="<<temperature_lowest<<std::endl;
        ss<<"density_scale="<<density_scale<<", density_offset="<<density_offset<<", clamp_low_density="<<clamp_low_density<<", density_lowest="<<density_lowest<<std::endl;
        LOG::filecout(ss.str());
    }
