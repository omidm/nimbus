    ...
    if(mode==GL_SELECT){ 
        if(slice && slice->Is_Slice_Mode()){
            ...
            for(DYADIC_GRID_ITERATOR_FACE<OCTREE_GRID<T> > iterator(grid,grid.Map_All_Faces());iterator.Valid();iterator.Next()){
                ...
                for(int k=0;k<4;k++) OpenGL_Vertex(grid.Node_Location(iterator.Face_Node(k)),vertices);
            ...
            for(int i=1;i<=grid.number_of_nodes;i++){
                ...
                glLoadName(i);
                ARRAY<typename OPENGL_POLICY<T>::T_GL> vertices;
                OpenGL_Vertex(grid.Node_Location(i),vertices);
...
Draw_Coarse_Cells_In_Slice_Mode() const
{
    ...
    if(slice->axis==1){/*if(abs(grid.uniform_grid.z(index[slice->axis])-slice->position)<1e-5)*/{
        ...
        for (j=1,y=grid.uniform_grid.domain.min_corner.y;j<=n+1;j++,y+=grid.uniform_grid.dX.y){OpenGL_Line(VECTOR<T,3>(x,y,grid.uniform_grid.domain.min_corner.z),VECTOR<T,3>(x,y,grid.uniform_grid.domain.max_corner.z),vertices);}
        for (ij=1,z=grid.uniform_grid.domain.min_corner.z;ij<=mn+1;ij++,z+=grid.uniform_grid.dX.z){OpenGL_Line(VECTOR<T,3>(x,grid.uniform_grid.domain.min_corner.y,z),VECTOR<T,3>(x,grid.uniform_grid.domain.max_corner.y,z),vertices);}}}
        ...
        for (i=1,x=grid.uniform_grid.domain.min_corner.x;i<=m+1;i++,x+=grid.uniform_grid.dX.x){OpenGL_Line(VECTOR<T,3>(x,y,grid.uniform_grid.domain.min_corner.z),VECTOR<T,3>(x,y,grid.uniform_grid.domain.max_corner.z),vertices);}
        for (ij=1,z=grid.uniform_grid.domain.min_corner.z;ij<=mn+1;ij++,z+=grid.uniform_grid.dX.z){OpenGL_Line(VECTOR<T,3>(grid.uniform_grid.domain.min_corner.x,y,z),VECTOR<T,3>(grid.uniform_grid.domain.max_corner.x,y,z),vertices);}}}
        ...
        for (i=1,x=grid.uniform_grid.domain.min_corner.x;i<=m+1;i++,x+=grid.uniform_grid.dX.x){OpenGL_Line(VECTOR<T,3>(x,grid.uniform_grid.domain.min_corner.y,z),VECTOR<T,3>(x,grid.uniform_grid.domain.max_corner.y,z),vertices);}
        for (j=1,y=grid.uniform_grid.domain.min_corner.y;j<=n+1;j++,y+=grid.uniform_grid.dX.y){OpenGL_Line(VECTOR<T,3>(grid.uniform_grid.domain.min_corner.x,y,z),VECTOR<T,3>(grid.uniform_grid.domain.max_corner.x,y,z),vertices);}}}
...
Draw_Refined_Cells() const
{
    ...
    for(DYADIC_GRID_ITERATOR_FACE<OCTREE_GRID<T> > iterator(grid,grid.Map_All_Faces());iterator.Valid();iterator.Next()){
        ...
        OpenGL_Line(grid.Node_Location(iterator.Face_Node(0)),grid.Node_Location(iterator.Face_Node(1)),vertices);
        OpenGL_Line(grid.Node_Location(iterator.Face_Node(0)),grid.Node_Location(iterator.Face_Node(2)),vertices);
        OpenGL_Line(grid.Node_Location(iterator.Face_Node(2)),grid.Node_Location(iterator.Face_Node(3)),vertices);
        OpenGL_Line(grid.Node_Location(iterator.Face_Node(1)),grid.Node_Location(iterator.Face_Node(3)),vertices);
    }
