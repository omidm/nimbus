...
Print_Triangles_Incident_On_Current_Node()
{
    ...
    {std::stringstream ss;
    ss<<"number of incident triangles to node "<<node<<"="<<(*surface.mesh.incident_elements)(node).m<<std::endl;
    LOG::filecout(ss.str());}
    for(int t=1;t<=(*surface.mesh.incident_elements)(node).m;t++){
        ...
        std::stringstream ss;
        ss<<"triangle "<<triangle<<"=("<<i<<","<<j<<","<<k<<")"<<std::endl;
        LOG::filecout(ss.str());
    }
}
//#####################################################################
...
Print_Neighbor_Nodes_Of_Current_Node()
{
    ...
    if(!surface.mesh.neighbor_nodes) surface.mesh.Initialize_Neighbor_Nodes();
    std::stringstream ss;
    ss<<"number of neighbors of node "<<node<<"="<<(*surface.mesh.neighbor_nodes)(node).m<<std::endl;
    ss<<"neighbors of node "<<node<<"={";
    for(int t=1;t<=(*surface.mesh.neighbor_nodes)(node).m;t++) ss<<(*surface.mesh.neighbor_nodes)(node)(t)<<",";
    ss<<"}"<<std::endl;
    LOG::filecout(ss.str());
}
//#####################################################################
...
Display(const int in_color) const
{
    ...
    if(two_sided){
        ...
        if(in_color){
#ifdef USE_OPENGLES
            front_material.Send_To_GL_Pipeline(GL_FRONT_AND_BACK); //OPENGLES compatibility
#else
...
#ifdef USE_OPENGLES
            front_material_gray.Send_To_GL_Pipeline(GL_FRONT_AND_BACK);
#else
