...
public:
    ...
    using OPENGL_VECTOR_FIELD_3D<T>::is_moving_grid;using OPENGL_VECTOR_FIELD_3D<T>::rigid_grid_frame; 
    ...
    T_FACE_ARRAYS_SCALAR* face_velocities_simulated;
    ARRAY_VIEW<T,VECTOR<int,3> > &u, &v, &w;

    OPENGL_MAC_VELOCITY_FIELD_3D(GRID<TV> &grid,bool is_moving_grid_input=false);
