...
public:
    ...
    void operator() () PHYSBAM_OVERRIDE
    {
        ...
        {std::stringstream ss;
        ss<<"Filename for saving image in .ppm or .jpg (if supported) formats (return for default, - for cancel)\n";
        ss<<"[%s]: "<<defaultname;
        LOG::filecout(ss.str());}
...
public:
    ...
    void operator() () PHYSBAM_OVERRIDE
    {
        ...
        std::stringstream ss;
        ss<<"saving to %s..."<<filename<<std::endl;
        LOG::filecout(ss.str());
...
//#####################################################################
// Class OPENGL_CALLBACK_REDISPLAY
//#####################################################################
// GL-specific
class OPENGL_CALLBACK_REDISPLAY:public OPENGL_CALLBACK
{
    OPENGL_WORLD &world;
public:

    OPENGL_CALLBACK_REDISPLAY(OPENGL_WORLD &world)
        :world(world)
    {}

    void operator() () PHYSBAM_OVERRIDE
    {}
    void Print(std::ostream& out) { out << "Redisplay"; }
};
