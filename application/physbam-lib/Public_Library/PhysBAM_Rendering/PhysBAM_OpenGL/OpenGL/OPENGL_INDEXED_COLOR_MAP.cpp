...
{
    OPENGL_INDEXED_COLOR_MAP* map=new OPENGL_INDEXED_COLOR_MAP;
    map->Set_Color(1,OPENGL_COLOR::Red(0.6f));
    map->Set_Color(2,OPENGL_COLOR::Green(0.6f));
    map->Set_Color(3,OPENGL_COLOR::Blue(0.6f));
    map->Set_Color(4,OPENGL_COLOR::Magenta(0.6f));
    map->Set_Color(5,OPENGL_COLOR::Cyan(0.6f));
    map->Set_Color(6,OPENGL_COLOR::Gray(0.25f));
    map->Set_Color(7,OPENGL_COLOR::Gray(0.75f));
    map->Set_Color(8,OPENGL_COLOR::Yellow(0.6f));
    return map;
}
OPENGL_INDEXED_COLOR_MAP* OPENGL_INDEXED_COLOR_MAP::
Particle_Multiple_Color_Map()
{
    ...
    for(int i=map->color_map.domain.min_corner.x;i<=map->color_map.domain.max_corner.x;i++){for(int j=0;j<=2;j++) map->color_map(i).rgba[j]+=(GLfloat).35;map->color_map(i).rgba[3]=1;}
    return map;
}
OPENGL_INDEXED_COLOR_MAP* OPENGL_INDEXED_COLOR_MAP::
