...
Display(const int in_color) const
{
    ...
    if(draw_points){ARRAY<typename OPENGL_POLICY<T>::T_GL> vertices;
        for(DYADIC_GRID_ITERATOR_FACE<QUADTREE_GRID<T> > iterator(grid,grid.Map_All_Faces());iterator.Valid();iterator.Next()){
            color_map->Lookup(value(iterator.Face_Index())).Send_To_GL_Pipeline();
            OpenGL_Vertex(iterator.Location(),vertices);}
        OpenGL_Draw_Arrays(GL_POINTS,2,vertices);}
    else{ARRAY<typename OPENGL_POLICY<T>::T_GL> vertices;
        for(DYADIC_GRID_ITERATOR_FACE<QUADTREE_GRID<T> > iterator(grid,grid.Map_All_Faces());iterator.Valid();iterator.Next()){
            T2 val=value(iterator.Face_Index());
            color_map->Lookup(val).Send_To_GL_Pipeline();
            OpenGL_Line(iterator.Location(),iterator.Location()+val*line_size*VECTOR<T,2>::Axis_Vector(iterator.Axis()),vertices);}
