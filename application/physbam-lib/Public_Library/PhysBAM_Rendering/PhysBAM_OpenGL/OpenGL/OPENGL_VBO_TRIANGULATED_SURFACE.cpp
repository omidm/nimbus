...
void OPENGL_VBO_TRIANGULATED_SURFACE<T>::Create_VBO(void)
{
    ...
    if (gluxIsAvailable("GL_ARB_vertex_buffer_object") == GLUX_AVAILABLE) {
        ...
        if(!useVBO){
            std::stringstream ss;
            ss<<"VBOs to create: "<<useVBOnum<<std::endl;
            LOG::filecout(ss.str());
        ...
        {std::stringstream ss;
        ss<<"Total number of primitives: "<<triangles.m<<std::endl;
        LOG::filecout(ss.str());}
        ...
        std::stringstream ss;
        ss<<"Vertex Buffer Objects NOT SUPPORTED! Defaulting to Display Lists."<<std::endl;
        LOG::filecout(ss.str());
        Create_Display_List();
        useVBO = false;
    }
