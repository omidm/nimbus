...
public:
    T_LEVELSET* levelset,*levelset_simulated;
private:
...
public:
    OPENGL_LEVELSET_MULTIVIEW()
        ...
        use_marching_tetrahedra(false),levelset(0),levelset_simulated(0),levelset_implicit_surface(0),triangulated_surface(0),i_own_levelset(true),i_own_triangulated_surface(true),
        opengl_triangulated_surface(0),opengl_scalar_field(0),two_sided(true)
    {
        back_surface_material=OPENGL_MATERIAL::Plastic(OPENGL_COLOR(0.8f,0,0));
        front_surface_material=OPENGL_MATERIAL::Plastic(OPENGL_COLOR(0,0.6f,0.9f));
        overlayed_surface_material=OPENGL_MATERIAL::Plastic(OPENGL_COLOR::Gray(0.6f,0.5f));
        outside_slice_color=OPENGL_COLOR::Red(0.5f);
