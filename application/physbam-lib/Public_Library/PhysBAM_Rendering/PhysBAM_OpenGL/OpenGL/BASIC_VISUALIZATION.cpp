...
{
    std::stringstream ss;
    ss<<"Using Component '"<<name<<"'"<<std::endl;
    LOG::filecout(ss.str());
...
Parse_Arguments(PARSE_ARGS& parse_args)
{
    ...
    fovy=(float)parse_args.Get_Double_Value("-fov");
...
{
    opengl_world.Set_Ambient_Light(0.3f);
    opengl_world.Add_Light(new OPENGL_LIGHT(VECTOR<double,3>(0.3,0.3,1),.3f));
    opengl_world.Add_Light(new OPENGL_LIGHT(VECTOR<double,3>(-0.3,0.3,1),.3f));
...
Initialize_Scene()
{
    if(!initialization_key_sequence.empty()){
        std::stringstream ss;
        ss<<"Initialization key sequence: '"<<initialization_key_sequence<<"'"<<std::endl;
        LOG::filecout(ss.str());
