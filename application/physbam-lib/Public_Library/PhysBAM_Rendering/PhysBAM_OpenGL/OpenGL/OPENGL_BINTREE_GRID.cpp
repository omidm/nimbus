...
Display(const int in_color) const
{
    ...
    if(mode==GL_SELECT){
        ...
        for(DYADIC_GRID_ITERATOR_CELL<BINTREE_GRID<T> > iterator(grid,grid.number_of_ghost_cells);iterator.Valid();iterator.Next()){
            ...
            OpenGL_Quad_2D(grid.Node_Location(0,cell).Insert((T)-.05,2),grid.Node_Location(1,cell).Insert((T).05,2),vertices);
...
#endif
    {
        ...
        for(UNIFORM_GRID_ITERATOR_CELL<VECTOR<T,1> > iterator(grid.uniform_grid,grid.number_of_ghost_cells);iterator.Valid();iterator.Next())
            OpenGL_Line(iterator.Bounding_Box().min_corner.Insert((T)0,2).Insert((T)0,3),iterator.Bounding_Box().max_corner.Insert((T)0,2).Insert((T)0,3),vertices);
        ...
        for(DYADIC_GRID_ITERATOR_CELL<BINTREE_GRID<T> > iterator(grid,grid.number_of_ghost_cells);iterator.Valid();iterator.Next()){
            BINTREE_CELL<T>* cell=iterator.Cell_Pointer();
            OpenGL_Line(grid.Node_Location(0,cell).Insert((T)-.05,2),grid.Node_Location(1,cell).Insert((T)-.05,2),vertices);
            OpenGL_Line(grid.Node_Location(1,cell).Insert((T)-.05,2),grid.Node_Location(1,cell).Insert((T) .05,2),vertices);
            OpenGL_Line(grid.Node_Location(1,cell).Insert((T) .05,2),grid.Node_Location(0,cell).Insert((T) .05,2),vertices);
            OpenGL_Line(grid.Node_Location(0,cell).Insert((T) .05,2),grid.Node_Location(0,cell).Insert((T)-.05,2),vertices);}
