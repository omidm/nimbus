...
OPENGL_SEGMENTED_CURVE_2D(const SEGMENTED_CURVE_2D<T>& curve_input,const OPENGL_COLOR &color_input)
    ...
    vertex_color(OPENGL_COLOR::Green(0.9f)),vertex_position_color(OPENGL_COLOR::Magenta()),velocity_color(OPENGL_COLOR::Cyan()),
    draw_vertices(false),draw_vertex_positions(false),draw_velocities(false),velocity_scale((T)0.025),current_selection(0)
{}
//#####################################################################
// Function Display
//#####################################################################
...
Bounding_Box() const
{
    ...
    xmin=xmax=(float)curve.particles.X(1).x;
    ymin=ymax=(float)curve.particles.X(1).y;
