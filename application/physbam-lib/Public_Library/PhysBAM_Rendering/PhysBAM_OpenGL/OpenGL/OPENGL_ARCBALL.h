...
private:
    ...
    for (i=2;i<NSEGS;i++) pts[i]=(pts[i-1]*(T)dot)-pts[i-2];
    ...
    T added_diameter=2*visual_radius-2*sphere.radius;T thickness=visual_radius/(T)4.;
    ...
    if(rotation_axis==0){
        ...
        else{std::stringstream ss;ss<<"Failure"<<std::endl;LOG::filecout(ss.str());return previous_result;}}
    if(rotation_axis==1){
    ...
    if(rotation_axis==4){
        ...
        PLANE<T> view_plane(TV((T)project_x,(T)project_y,(T)project_z).Normalized(),sphere.center);
        if(INTERSECTION::Intersects(ray,view_plane)){TV point=ray.Point(ray.t_max);return (point-sphere.center).Normalized()*visual_radius*(T)1.1;}
    ...
    if(rotation_axis==-1&&INTERSECTION::Intersects(ray,sphere,added_diameter)){
        ...
        if(INTERSECTION::Intersects(ray,x_circ)){
            std::stringstream ss;
            ss<<"X axis init"<<std::endl;rotation_axis=0;
            LOG::filecout(ss.str());
            rotation_axis=0;
            ...
            std::stringstream ss;
            ss<<"Y axis init"<<std::endl;rotation_axis=1;
            LOG::filecout(ss.str());
            rotation_axis=1;
            TV point=ray.Point(ray.t_max);
            T magnitude=(point-sphere.center).Magnitude();
            ...
            std::stringstream ss;
            ss<<"Z axis init"<<std::endl;rotation_axis=2;
            LOG::filecout(ss.str());
            rotation_axis=2;
            TV point=ray.Point(ray.t_max);
            T magnitude=(point-sphere.center).Magnitude();
        ...
        std::stringstream ss;
        ss<<"Middle init"<<std::endl;rotation_axis=3;
        LOG::filecout(ss.str());
        rotation_axis=3;
        ...
        ray=opengl_world.Ray_Through_Normalized_Image_Coordinate(mouse);
        std::stringstream ss;
        ss<<"Outer Rim"<<std::endl;rotation_axis=4;
        LOG::filecout(ss.str());
        rotation_axis=4;
        GLdouble project_x,project_y,project_z;
        GLdouble model_view[16];glGetDoublev(GL_MODELVIEW_MATRIX,model_view);
