...
Bounding_Box() const
{
    if (valid && draw) return RANGE<VECTOR<float,3> >((float)grid.domain.min_corner.x,(float)grid.domain.min_corner.y,0,(float)grid.domain.max_corner.x,(float)grid.domain.max_corner.y,0);
