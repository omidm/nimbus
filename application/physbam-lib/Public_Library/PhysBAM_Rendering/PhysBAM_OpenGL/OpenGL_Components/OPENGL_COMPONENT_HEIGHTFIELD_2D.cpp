...
OPENGL_COMPONENT_HEIGHTFIELD_2D(const GRID<TV> &grid_input, 
    ...
    initial_grid(grid_input), grid(grid_input), height(grid.Domain_Indices()),mask(grid.Domain_Indices()), 
    opengl_vector_field(vector_field, vector_locations, OPENGL_COLOR::Green(), 0.025, true, false, true),
    levelset(grid,height), grid_filename(""), scale(1), displacement_scale(1), valid(false), 
    draw_velocities(true), use_triangle_strip(true), height_is_levelset(height_is_levelset_input)
{
    ...
    opengl_triangulated_surface.Set_Front_Material(OPENGL_MATERIAL::Plastic(OPENGL_COLOR(0,0.6f,0.9f)));
    opengl_triangulated_surface.Set_Back_Material(OPENGL_MATERIAL::Plastic(OPENGL_COLOR(0.8f,0,0)));
                ...
                for (int j = domain.min_corner.y+1; j <= domain.max_corner.y; j++)
                {
                    ...
                    v2=v4;
                        ...
                        if(vertices.m>2){
                            ...
                            vertices.Resize(0,false);normals.Resize(0,false);
                            if(mask(i,j)){
                                v1=VECTOR<T,3>(grid.Axis_X(i+1,1), scale*(height(i+1,j+1)+vertical_offset), grid.Axis_X(j+1,2));
                                v2=VECTOR<T,3>(grid.Axis_X(i,1), scale*(height(i,j+1)+vertical_offset), grid.Axis_X(j+1,2));
                                OpenGL_Vertex(v1,vertices);
                                OpenGL_Vertex(v2,vertices);
                                j++;
                            }
...
Reinitialize(bool force)
{
    if(draw){
        if(force || (is_animation && frame_loaded != frame) || (!is_animation && frame_loaded < 0)){
            bool success = true;
            valid = false;
            std::string filename = FILE_UTILITIES::Get_Frame_Filename(mask_filename, frame);
            if (FILE_UTILITIES::File_Exists(filename))FILE_UTILITIES::Read_From_File<RW>(filename,mask);
            ...
            if(success){
                ...
                if(FILE_UTILITIES::File_Exists(filename)){
                    if(!height_is_levelset) FILE_UTILITIES::Read_From_File<RW>(filename,height);
                    else FILE_UTILITIES::Read_From_File<RW>(filename,levelset);
