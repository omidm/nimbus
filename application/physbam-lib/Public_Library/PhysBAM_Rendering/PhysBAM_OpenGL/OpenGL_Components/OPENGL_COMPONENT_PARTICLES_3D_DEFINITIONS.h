...
OPENGL_COMPONENT_PARTICLES_3D(const std::string &filename_input, const std::string &filename_set_input, bool use_ids_input, bool particles_stored_per_cell_uniform_input, bool particles_stored_per_cell_adaptive_input,bool use_chimera_grid_input,bool is_moving_grid_input,const std::string rigid_grid_frame_filename_set_input)
    :OPENGL_COMPONENT("Particles 3D"), particles(new T_PARTICLES),opengl_points(new OPENGL_POINTS_3D<T>(*(new ARRAY<VECTOR<T,3> >))),
    opengl_vector_field(velocities,opengl_points->points,OPENGL_COLOR::Cyan()),
    ...
    draw_multiple_particle_sets(false),use_chimera_grid(use_chimera_grid_input),chimera_slice(0),is_moving_grid(is_moving_grid_input),
    rigid_grid_frame_filename_set(rigid_grid_frame_filename_set_input)
{
    number_of_sets=0;
    while(filename_set!=""){
        std::string filename;
        if(!use_chimera_grid)
            filename=STRING_UTILITIES::string_sprintf(filename_set.c_str(),(number_of_sets+1),frame);
        else
            filename=STRING_UTILITIES::string_sprintf(filename_set.c_str(),frame,number_of_sets+1);
        std::stringstream ss;
    ...
    if(use_chimera_grid && is_moving_grid) rigid_grid_frames.Resize(number_of_sets);
...
Display(const int in_color) const
{
    ...
    if(!use_chimera_grid && slice && slice->Is_Slice_Mode()){glPushAttrib(GL_ENABLE_BIT);slice->Enable_Clip_Planes();}
        ...
        if(mode == GL_SELECT){
            if(draw_multiple_particle_sets){
                ...
                for(int i=1;i<=number_of_sets;i++){
                    glLoadName(i);
                    glPushMatrix();
                    T angle;VECTOR<T,3> axis;rigid_grid_frames(i).r.Get_Angle_Axis(angle,axis);angle=(T)(angle/pi*180);
                    glTranslatef((GLfloat)rigid_grid_frames(i).t(1),(GLfloat)rigid_grid_frames(i).t(2),(GLfloat)rigid_grid_frames(i).t(3));glRotatef((GLfloat)angle,(GLfloat)axis(1),(GLfloat)axis(2),(GLfloat)axis(3));
                    if(chimera_slice && chimera_slice->Is_Slice_Mode()) 
                        opengl_points_multiple(i)->Filter_By_Slice_Planes(chimera_slice->plane1_array(i),chimera_slice->plane2_array(i));
                    else if(opengl_points_multiple(i)->draw_mask) opengl_points_multiple(i)->draw_mask->Fill(true);
                    opengl_points_multiple(i)->Display(in_color);
                    glPopMatrix();}
                glPopName();}
            else{ 
                glPushMatrix();
                T angle;VECTOR<T,3> axis;rigid_grid_frames(set).r.Get_Angle_Axis(angle,axis);angle=(T)(angle/pi*180);
                glTranslatef((GLfloat)rigid_grid_frames(set).t(1),(GLfloat)rigid_grid_frames(set).t(2),(GLfloat)rigid_grid_frames(set).t(3));glRotatef((GLfloat)angle,(GLfloat)axis(1),(GLfloat)axis(2),(GLfloat)axis(3));
                if(chimera_slice && chimera_slice->Is_Slice_Mode()) 
                    opengl_points_multiple(set)->Filter_By_Slice_Planes(chimera_slice->plane1_array(set),chimera_slice->plane2_array(set));
                else if(opengl_points_multiple(set)->draw_mask) opengl_points_multiple(set)->draw_mask->Fill(true);
            ...
            if (draw_velocities && have_velocities){ 
                ...
                T angle;VECTOR<T,3> axis;rigid_grid_frames(set).r.Get_Angle_Axis(angle,axis);angle=(T)(angle/pi*180);
                glTranslatef((GLfloat)rigid_grid_frames(set).t(1),(GLfloat)rigid_grid_frames(set).t(2),(GLfloat)rigid_grid_frames(set).t(3));glRotatef((GLfloat)angle,(GLfloat)axis(1),(GLfloat)axis(2),(GLfloat)axis(3));
                opengl_vector_field.Display(in_color);
                glPopMatrix();}}
        else{
            if(draw_multiple_particle_sets) for(int i=1;i<=number_of_sets;i++){
                glPushMatrix();
                T angle;VECTOR<T,3> axis;rigid_grid_frames(i).r.Get_Angle_Axis(angle,axis);angle=(T)(angle/pi*180);
                glTranslatef((GLfloat)rigid_grid_frames(i).t(1),(GLfloat)rigid_grid_frames(i).t(2),(GLfloat)rigid_grid_frames(i).t(3));glRotatef((GLfloat)angle,(GLfloat)axis(1),(GLfloat)axis(2),(GLfloat)axis(3));
                if(chimera_slice && chimera_slice->Is_Slice_Mode()) 
    ...
    if(!use_chimera_grid && slice && slice->Is_Slice_Mode())glPopAttrib();
}
//#####################################################################
...
Reinitialize(bool force)
{
    ...
    for(int i=1;i<=number_of_sets;i++){
        ...
        if(use_sets){ 
            if(!use_chimera_grid)
                frame_filename=STRING_UTILITIES::string_sprintf(filename_set.c_str(),i,frame);
            else
                frame_filename=STRING_UTILITIES::string_sprintf(filename_set.c_str(),frame,i);}
            ...
            if(is_moving_grid){
                for(int i=1;i<=number_of_sets;i++){
                    ...
                    if(FILE_UTILITIES::File_Exists(grid_filename))
                        FILE_UTILITIES::Read_From_File<T>(grid_filename,rigid_grid_frames(i));}}
    ...
    ARRAY_VIEW<VECTOR<T,3> >* temp_velocities=particles->array_collection->template Get_Array<VECTOR<T,3> >(ATTRIBUTE_ID_V);
    if(temp_velocities){
        have_velocities=true;
        velocities.Resize(temp_velocities->Size());
        for(int i=1;i<=velocities.Size();i++)velocities(i)=(*temp_velocities)(i);}
...
Next_Set()
{
    ...
    std::stringstream ss;
    ss<<"viewing particle set "<<set<<std::endl;
    LOG::filecout(ss.str());
...
//#####################################################################
// Function Set_Chimera_Slice
//#####################################################################
template<class T,class T_PARTICLES,class RW> void OPENGL_COMPONENT_PARTICLES_3D<T,T_PARTICLES,RW>::
Set_Chimera_Slice(OPENGL_CHIMERA_SLICE<T>* slice_input)
{
    chimera_slice=slice_input;
    for(int i=1;i<=opengl_points_multiple.m;i++){opengl_points_multiple(i)->Set_Slice(slice_input->opengl_uniform_slices(i));}
}
//#####################################################################
...
{
    opengl_vector_field.Scale_Vector_Size((T)1.1);
}
//#####################################################################
...
{
    opengl_vector_field.Scale_Vector_Size(1/(T)1.1);
}
//#####################################################################
