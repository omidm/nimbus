...
OPENGL_COMPONENT_DEFORMABLE_GEOMETRY_COLLECTION_3D(const bool initialize_geometry)
    :OPENGL_COMPONENT("Deformable Object List"),frame_loaded(-1),valid(false),use_active_list(false),hide_unselected(false),display_mode(0),display_relative_velocity_mode(0),number_of_segmented_curve(1),
    ...
    color_map(OPENGL_INDEXED_COLOR_MAP::Basic_16_Color_Map())
{
    ...
    invalidate_deformable_objects_selection_each_frame=false;

    color_map_relative_velocity=new OPENGL_COLOR_RAMP<T>();
    color_map_relative_velocity->Add_Color((T)-.05,OPENGL_COLOR(1,0,0));
    color_map_relative_velocity->Add_Color(0,OPENGL_COLOR(0.5,0.5,0.5));
    color_map_relative_velocity->Add_Color((T).05,OPENGL_COLOR(0,0,1));
...
//#####################################################################
template<class T,class RW> void OPENGL_COMPONENT_DEFORMABLE_GEOMETRY_COLLECTION_3D<T,RW>::
Reinitialize_From_Simulation(bool force,bool read_geometry)
{
    if(!(draw && (force || (is_animation && frame_loaded!=frame) || (!is_animation && frame_loaded<0)))) return;
    static bool first_time_sim=true;
    bool read_static_variables=first_time_sim || !(deformable_geometry && deformable_geometry->structures.m);
    if(read_geometry){
        deformable_geometry->structures.Resize(deformable_geometry_simulated->structures.m);
        for(int i=1;i<=deformable_geometry->structures.m;i++) deformable_geometry->structures(i)=deformable_geometry_simulated->structures(i);
        deformable_geometry->structures=deformable_geometry_simulated->structures;
        for(int i=1;i<=deformable_geometry->particles.array_collection->Size();i++){
            deformable_geometry->particles.X(i)=deformable_geometry_simulated->particles.X(i);
            deformable_geometry->particles.V(i)=deformable_geometry_simulated->particles.V(i);}}
    ...
    if(read_static_variables){
        ...
        for(int i=1;i<=m;i++){
            ...
            if(SEGMENTED_CURVE<TV>* segmented_curve=dynamic_cast<SEGMENTED_CURVE<TV>*>(structure)){
                if(first_time_sim) LOG::cout<<"object "<<i<<": segmented curve\n";
                ...
                if(first_time_sim) LOG::cout<<"object "<<i<<": triangulated surface, range = "<<ARRAYS_COMPUTATIONS::Min(triangulated_surface->mesh.elements.Flattened())<<" "<<ARRAYS_COMPUTATIONS::Max(triangulated_surface->mesh.elements.Flattened())<<"\n";
                ...
                if(first_time_sim) LOG::cout<<"object "<<i<<": tetrahedralized_volume, range = "<<ARRAYS_COMPUTATIONS::Min(tetrahedralized_volume->mesh.elements.Flattened())<<" "<<ARRAYS_COMPUTATIONS::Max(tetrahedralized_volume->mesh.elements.Flattened())<<"\n";
                ...
                if(first_time_sim) LOG::cout<<"object "<<i<<": hexahedralized_volume\n";
            ...
            else{if(first_time_sim) LOG::cout<<"object "<<i<<": object unrecognized at geometry level\n";}}}
...
{
    if(is_interactive){Reinitialize_From_Simulation(force,read_geometry);return;}
    if(!(draw && (force || (is_animation && frame_loaded!=frame) || (!is_animation && frame_loaded<0)))) return;
    ...
    if(read_static_variables){
        ...
        for(int i=1;i<=m;i++){
            ...
            if(SEGMENTED_CURVE<TV>* segmented_curve=dynamic_cast<SEGMENTED_CURVE<TV>*>(structure)){
                ...
                if(first_time) {std::stringstream ss;ss<<"object "<<i<<": triangulated surface, range = "<<ARRAYS_COMPUTATIONS::Min(triangulated_surface->mesh.elements.Flattened())<<" "<<ARRAYS_COMPUTATIONS::Max(triangulated_surface->mesh.elements.Flattened())<<"\n";LOG::filecout(ss.str());}
                ...
                if(first_time) {std::stringstream ss;ss<<"object "<<i<<": tetrahedralized_volume, range = "<<ARRAYS_COMPUTATIONS::Min(tetrahedralized_volume->mesh.elements.Flattened())<<" "<<ARRAYS_COMPUTATIONS::Max(tetrahedralized_volume->mesh.elements.Flattened())<<"\n";LOG::filecout(ss.str());}
                tetrahedralized_volume_objects(i)=new OPENGL_TETRAHEDRALIZED_VOLUME<T>(&tetrahedralized_volume->mesh,&(deformable_geometry->particles),
                    OPENGL_MATERIAL::Metal(OPENGL_COLOR::Red(.7f)));}
            else if(HEXAHEDRALIZED_VOLUME<T>* hexahedralized_volume=dynamic_cast<HEXAHEDRALIZED_VOLUME<T>*>(structure)){
                if(first_time) {std::stringstream ss;ss<<"object "<<i<<": hexahedralized_volume\n";LOG::filecout(ss.str());}
                hexahedralized_volume_objects(i)=new OPENGL_HEXAHEDRALIZED_VOLUME<T>(&hexahedralized_volume->mesh,&(deformable_geometry->particles),
                    OPENGL_MATERIAL::Matte(OPENGL_COLOR::Red()));}
            else if(FREE_PARTICLES<TV>* fp=dynamic_cast<FREE_PARTICLES<TV>*>(structure)){
                free_particles_indirect_arrays(i)=new INDIRECT_ARRAY<ARRAY_VIEW<TV> >(deformable_geometry->particles.X,fp->nodes);
                free_particles_objects(i)=new OPENGL_FREE_PARTICLES<TV>(*deformable_geometry,*free_particles_indirect_arrays(i),color_map->Lookup(color_map_index--));}
            else{if(first_time) {std::stringstream ss;ss<<"object "<<i<<": object unrecognized at geometry level\n";LOG::filecout(ss.str());}}}}
    for(int i=1;i<=deformable_geometry->structures.m;i++){
        std::string i_string=STRING_UTILITIES::string_sprintf("%d",i);
        if(tetrahedralized_volume_objects(i)) has_tetrahedralized_volumes=true;
        if(hexahedralized_volume_objects(i)) has_hexahedralized_volumes=true;
...
Create_One_Big_Triangulated_Surface_And_Write_To_File()
{
    ...
    for(int index=1;index<=deformable_geometry->structures.m;index++){
        std::stringstream ss;
        ss<<"Adding "<<index<<"th triangulated surface out of "<<deformable_geometry->structures.m<<" surfaces"<<std::endl;
        LOG::filecout(ss.str());
