...
OPENGL_COMPONENT_LEVELSET_3D(bool write_generated_triangulated_surface_input,
    ...
    : OPENGL_COMPONENT("Levelset 3D"), opengl_levelset_multiview(0), 
      ...
      frame_loaded(-1),valid(false),drawing(false),check_triangulated_surface_file_time(check_triangulated_surface_file_time_input),
     set(1),set_loaded(-1),use_sets(true),draw_multiple_levelsets(true),is_moving_grid(false),rigid_grid_frame_filename_set(""),
     ghost_cells(3),use_chimera_grid(false),chimera_slice(0)
{
...
OPENGL_COMPONENT_LEVELSET_3D(const std::string& levelset_filename_input,
                             ...
                             bool check_triangulated_surface_file_time_input,
                             bool is_moving_grid_input,
                             const std::string rigid_grid_frame_filename_set_input,
                             bool use_chimera_grid_input)
    : OPENGL_COMPONENT("Levelset 3D"), opengl_levelset_multiview(0), 
      ...
      write_generated_triangulated_surface(write_generated_triangulated_surface_input),
      frame_loaded(-1),valid(false),drawing(false),check_triangulated_surface_file_time(check_triangulated_surface_file_time_input),
     set(1),set_loaded(-1),use_sets(true),draw_multiple_levelsets(true),is_moving_grid(is_moving_grid_input),
     rigid_grid_frame_filename_set(rigid_grid_frame_filename_set_input),ghost_cells(3),use_chimera_grid(use_chimera_grid_input),chimera_slice(0)
{
    int number_of_sets=0;
    while(filename_set!=""){
        std::string filename=STRING_UTILITIES::string_sprintf(filename_set.c_str(),frame,(number_of_sets+1));
    ...
    std::stringstream ss;
    ss<<"Found "<<number_of_sets<<" levelsets for multiphase"<<std::endl;
    LOG::filecout(ss.str());
...
Display(const int in_color) const
{
    if(valid && draw){
        ...
        if(!is_moving_grid){
            ...
            if(draw_multiple_levelsets) for(int i=1;i<=opengl_levelset_multiviews.m;i++){
                glPushMatrix();
                T angle;TV axis;rigid_grid_frames(i).r.Get_Angle_Axis(angle,axis);angle=(T)(angle/pi*180);
                ...
                opengl_levelset_multiviews(i)->Display(in_color);
                glPopMatrix();}
            else{
                glPushMatrix();
                T angle;TV axis;rigid_grid_frames(set).r.Get_Angle_Axis(angle,axis);angle=(T)(angle/pi*180);
                ...
                opengl_levelset_multiview->Display(in_color);
                glPopMatrix();}}
        drawing=false;}
}
...
Bounding_Box() const
{
    if(valid && draw) return opengl_levelset_multiview->Bounding_Box();
...
Reinitialize_From_Simulation()
{
    if(!drawing && draw){
        if((is_animation && frame_loaded != frame) || (!is_animation && frame_loaded < 0)){
            valid=false;
            opengl_levelset_multiview->Set_Levelset(*opengl_levelset_multiview->levelset);
            if(opengl_levelset_multiview->levelset_simulated) for(typename GRID<TV>::CELL_ITERATOR iterator(opengl_levelset_multiview->levelset->grid);iterator.Valid();iterator.Next())
                opengl_levelset_multiview->levelset->phi(iterator.Cell_Index())=opengl_levelset_multiview->levelset_simulated->phi(iterator.Cell_Index());
            opengl_levelset_multiview->Generate_Triangulated_Surface();
            opengl_levelset_multiview->Initialize();
            frame_loaded=frame;
            valid=true;}}
}
...
Set_Chimera_Slice(OPENGL_CHIMERA_SLICE<T>* slice_input)
{
    ...
    for(int i=1;i<=opengl_levelset_multiviews.m;i++){opengl_levelset_multiviews(i)->Set_Slice(slice_input->opengl_uniform_slices(i));}
}
