...
#include <PhysBAM_Rendering/PhysBAM_OpenGL/OpenGL_Components/OPENGL_COMPONENT_CHIMERA_SCALAR_FIELD_2D.h>
#include <PhysBAM_Rendering/PhysBAM_OpenGL/OpenGL/OPENGL_CONSTANT_COLOR_MAP.h>
#include <PhysBAM_Tools/Math_Tools/Is_NaN.h>
using namespace PhysBAM;

template<class T,class T2,class RW> OPENGL_COMPONENT_CHIMERA_SCALAR_FIELD_2D<T,T2,RW>::
OPENGL_COMPONENT_CHIMERA_SCALAR_FIELD_2D(ARRAY<GRID<TV> > &grid_input, const std::string &scalar_field_filename_set_input,OPENGL_COLOR_MAP<T2>* color_map_input,bool is_moving_grid_input,const std::string rigid_grid_frame_filename_set_input)
    : OPENGL_COMPONENT("Scalar Field 2D"),scalar_field_filename_set(scalar_field_filename_set_input), frame_loaded(-1), valid(false),current_grid(1), draw_all_grids(true),display_in_bool(false),dynamically_set_scale_range(false),scale_maximum(1),is_moving_grid(is_moving_grid_input),rigid_grid_frame_filename_set(rigid_grid_frame_filename_set_input)
{
    for(int i=1;i<=grid_input.Size();i++){
        OPENGL_SCALAR_FIELD_2D<T,T2>* tmp_opengl_scalar_field=new OPENGL_SCALAR_FIELD_2D<T,T2>(grid_input(i),*new ARRAY<T2,VECTOR<int,2> >,color_map_input,OPENGL_SCALAR_FIELD_2D<T,T2>::DRAW_TEXTURE,is_moving_grid_input);
        OPENGL_SCALAR_FIELD_2D<T,bool>* tmp_opengl_bool_field=new OPENGL_SCALAR_FIELD_2D<T,bool>(grid_input(i),*new ARRAY<bool,VECTOR<int,2> >,new OPENGL_CONSTANT_COLOR_MAP<bool>(OPENGL_COLOR::Magenta()),OPENGL_SCALAR_FIELD_2D<T,bool>::DRAW_TEXTURE,is_moving_grid_input);
...
template<class T,class T2,class RW> OPENGL_COMPONENT_CHIMERA_SCALAR_FIELD_2D<T,T2,RW>::
OPENGL_COMPONENT_CHIMERA_SCALAR_FIELD_2D(ARRAY<GRID<TV> > &grid_input, const std::string &scalar_field_filename_set_input,OPENGL_COLOR_MAP<T2>* color_map_input,typename OPENGL_SCALAR_FIELD_2D<T,T2>::DRAW_MODE draw_mode_input,bool is_moving_grid_input,const std::string rigid_grid_frame_filename_set_input)
    : OPENGL_COMPONENT("Scalar Field 2D"), scalar_field_filename_set(scalar_field_filename_set_input), frame_loaded(-1), valid(false), current_grid(1),draw_all_grids(true),display_in_bool(false),dynamically_set_scale_range(false),scale_maximum(1),is_moving_grid(is_moving_grid_input),rigid_grid_frame_filename_set(rigid_grid_frame_filename_set_input)
{
    for(int i=1;i<=grid_input.Size();i++){
        OPENGL_SCALAR_FIELD_2D<T,T2>* tmp_opengl_scalar_field=new OPENGL_SCALAR_FIELD_2D<T,T2>(grid_input(i),*new ARRAY<T2,VECTOR<int,2> >,color_map_input,draw_mode_input,is_moving_grid_input);
        OPENGL_SCALAR_FIELD_2D<T,bool>* tmp_opengl_bool_field=new OPENGL_SCALAR_FIELD_2D<T,bool>(grid_input(i),*new ARRAY<bool,VECTOR<int,2> >,new OPENGL_CONSTANT_COLOR_MAP<bool>(OPENGL_COLOR::Magenta()),OPENGL_SCALAR_FIELD_2D<T,bool>::DRAW_POINTS,is_moving_grid_input);
        opengl_scalar_fields.Append(tmp_opengl_scalar_field);
        opengl_bool_fields.Append(tmp_opengl_bool_field);}
    if(opengl_scalar_fields.Size()>=1) opengl_scalar_field=opengl_scalar_fields(1);
    if(opengl_bool_fields.Size()>=1) opengl_bool_field=opengl_bool_fields(1);
    is_animation = FILE_UTILITIES::Is_Animated(scalar_field_filename_set);
}
...
template<class T,class T2,class RW> bool OPENGL_COMPONENT_CHIMERA_SCALAR_FIELD_2D<T,T2,RW>::
Valid_Frame(int frame_input) const
{
    return FILE_UTILITIES::File_Exists(STRING_UTILITIES::string_sprintf(scalar_field_filename_set.c_str(),frame_input,1));
}

template<class T,class T2,class RW> void OPENGL_COMPONENT_CHIMERA_SCALAR_FIELD_2D<T,T2,RW>::
...
template<class T,class T2,class RW> RANGE<VECTOR<float,3> > OPENGL_COMPONENT_CHIMERA_SCALAR_FIELD_2D<T,T2,RW>::
Bounding_Box() const
{
...
template<class T,class T2,class RW> template<class T3> inline void OPENGL_COMPONENT_CHIMERA_SCALAR_FIELD_2D<T,T2,RW>::
...
//for bool only
template<class T,class T2,class RW> inline void OPENGL_COMPONENT_CHIMERA_SCALAR_FIELD_2D<T,T2,RW>::
...
//for int, float, double
template<class T,class T2,class RW> template<class T3> inline void OPENGL_COMPONENT_CHIMERA_SCALAR_FIELD_2D<T,T2,RW>::
Set_Min_Max_Values(WORKAROUND_STRUCT<T3>& ws, T2& max_value,T2& min_value,const T2& value_to_check)
{
    if(!Is_NaN(value_to_check)){
        if(value_to_check>max_value) max_value=value_to_check;
        if(value_to_check<min_value) min_value=value_to_check;}
}

//for bool only
template<class T,class T2,class RW> inline void OPENGL_COMPONENT_CHIMERA_SCALAR_FIELD_2D<T,T2,RW>::
Set_Min_Max_Values(WORKAROUND_STRUCT<bool>& ws, T2& max_value,T2& min_value,const T2& value_to_check)
{
    if(!Is_NaN(value_to_check)){
        if(value_to_check==1) max_value=1;
        if(value_to_check==0) min_value=0;}
}
                ...
                if(dynamically_set_scale_range){
                    ...
                    for(int i=1;i<=opengl_scalar_fields.Size();i++)
                        for(CELL_ITERATOR iterator(opengl_scalar_fields(i)->grid);iterator.Valid();iterator.Next()){
                            T2 value=opengl_scalar_fields(i)->values(iterator.Cell_Index());
                            Set_Min_Max_Values(ws,max_value,min_value,value);}
                    for(int i=1;i<=opengl_scalar_fields.Size();i++)
...
{
    for(int i=1;i<=opengl_scalar_fields.Size();++i)
        opengl_scalar_fields(i)->Toggle_Smooth_Texture();
}
...
{
    for(int i=1;i<=opengl_scalar_fields.Size();++i)
        opengl_scalar_fields(i)->Toggle_Draw_Mode();
    for(int i=1;i<=opengl_scalar_fields.Size();++i)
        opengl_bool_fields(i)->Toggle_Draw_Mode();
}
...
{
    for(int i=1;i<=opengl_scalar_fields.Size();++i)
        opengl_scalar_fields(i)->Toggle_Color_Map();
    for(int i=1;i<=opengl_scalar_fields.Size();++i)
        opengl_bool_fields(i)->Toggle_Color_Map();
}
...
{
    for(int i=1;i<=opengl_scalar_fields.Size();i++)
        opengl_scalar_fields(i)->Toggle_Draw_Ghost_Values();
}
...
template<class T,class T2,class RW> template<class T3> void OPENGL_COMPONENT_CHIMERA_SCALAR_FIELD_2D<T,T2,RW>::
...
template class OPENGL_COMPONENT_CHIMERA_SCALAR_FIELD_2D<float,int,float>;
template class OPENGL_COMPONENT_CHIMERA_SCALAR_FIELD_2D<float,bool,float>;
template class OPENGL_COMPONENT_CHIMERA_SCALAR_FIELD_2D<float,float,float>;
#ifndef COMPILE_WITHOUT_DOUBLE_SUPPORT
template class OPENGL_COMPONENT_CHIMERA_SCALAR_FIELD_2D<double,int,double>;
template class OPENGL_COMPONENT_CHIMERA_SCALAR_FIELD_2D<double,bool,double>;
template class OPENGL_COMPONENT_CHIMERA_SCALAR_FIELD_2D<double,double,double>;
#endif
