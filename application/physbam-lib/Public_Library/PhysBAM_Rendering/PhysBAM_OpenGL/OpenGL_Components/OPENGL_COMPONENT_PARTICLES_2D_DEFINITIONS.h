...
OPENGL_COMPONENT_PARTICLES_2D(const std::string &filename_input, const std::string &filename_set_input, bool use_ids_input, bool particles_stored_per_cell_uniform_input, bool is_moving_grid_input, const std::string &rigid_grid_frame_filename_input)
    : OPENGL_COMPONENT("Particles 2D"), particles(new T_PARTICLES),opengl_points(new OPENGL_POINTS_2D<T>(*(new ARRAY<VECTOR<T,2> >))),
     ...
     draw_multiple_particle_sets(false),is_moving_grid(is_moving_grid_input),rigid_grid_frame_filename(rigid_grid_frame_filename_input)
{
    number_of_sets=0;
    while(filename_set!=""){
    ...
    rigid_grid_frames.Resize(number_of_sets);
        ...
        if(mode == GL_SELECT){
            if(draw_multiple_particle_sets){
                ...
                for(int i=1;i<=number_of_sets;i++){
                    ...
                    if(is_moving_grid){
                        glPushMatrix();
                        glTranslatef((GLfloat)rigid_grid_frames(i).t(1),(GLfloat)rigid_grid_frames(i).t(2),0);
                        glRotatef((GLfloat)(rigid_grid_frames(i).r.Angle()/pi*180),0,0,1);
                        Send_Transform_To_GL_Pipeline();}
                    opengl_points_multiple(i)->Display(in_color);
                    if(is_moving_grid) glPopMatrix();}
                glPopName();}
            else{
                if(is_moving_grid){
                    glPushMatrix();
                    glTranslatef((GLfloat)rigid_grid_frames(set).t(1),(GLfloat)rigid_grid_frames(set).t(2),0);
                    glRotatef((GLfloat)(rigid_grid_frames(set).r.Angle()/pi*180),0,0,1);
                    Send_Transform_To_GL_Pipeline();}
...
Reinitialize(bool force)
{
    ...
    if(is_moving_grid){
        if(!use_sets){
            ...
            if(FILE_UTILITIES::File_Exists(grid_filename)) FILE_UTILITIES::Read_From_File<T>(grid_filename,rigid_grid_frames(1));}
        else{
            std::string grid_filename_set=rigid_grid_frame_filename+"_%d";
            for(int i=1;i<=number_of_sets;i++){
                std::string grid_filename=STRING_UTILITIES::string_sprintf(grid_filename_set.c_str(),frame,i);
                if(FILE_UTILITIES::File_Exists(grid_filename)) FILE_UTILITIES::Read_From_File<T>(grid_filename,rigid_grid_frames(i));}}}
    for(int i=1;i<=number_of_sets;i++){
    ...
    ARRAY_VIEW<VECTOR<T,2> >* temp_velocities=particles->array_collection->template Get_Array<VECTOR<T,2> >(ATTRIBUTE_ID_V);
    if(temp_velocities){
...
template<class T,class T_PARTICLES,class RW> void OPENGL_COMPONENT_PARTICLES_2D<T,T_PARTICLES,RW>::
Toggle_Increase_Vector_Size()
{
    opengl_vector_field.Scale_Vector_Size((T)1.2);
}

template<class T,class T_PARTICLES,class RW> void OPENGL_COMPONENT_PARTICLES_2D<T,T_PARTICLES,RW>::
Toggle_Decrease_Vector_Size()
{
    opengl_vector_field.Scale_Vector_Size(1/(T)1.2);
}
