...
OPENGL_COMPONENT_MAC_VELOCITY_FIELD_3D(const GRID<TV> &grid)
    : OPENGL_COMPONENT("MAC Velocity Field"),
     ...
     draw_vorticity(false),velocity_filename(""),valid(false),min_vorticity(FLT_MAX),max_vorticity(FLT_MIN)
{
    is_animation = true;
...
Reinitialize_From_Simulation()
{
    if(draw){
        if (!valid || (is_animation && frame_loaded != frame) || (!is_animation && frame_loaded < 0)){
            valid = false;
            if(opengl_mac_velocity_field.face_velocities_simulated) opengl_mac_velocity_field.face_velocities=*opengl_mac_velocity_field.face_velocities_simulated;
...
Reinitialize()
{
    if(is_interactive) Reinitialize_From_Simulation();
        ...
        if (!valid || (is_animation && frame_loaded != frame) || (!is_animation && frame_loaded < 0)){
            ...
            if (FILE_UTILITIES::File_Exists(tmp_filename)) FILE_UTILITIES::Read_From_File<RW>(tmp_filename,opengl_mac_velocity_field.face_velocities);//u,opengl_mac_velocity_field.v,opengl_mac_velocity_field.w);
            else return;
            opengl_mac_velocity_field.Update();
            if(draw_vorticity){
                Update_Vorticity();
...
{
    opengl_mac_velocity_field.Scale_Vector_Size((T)1.1);
}

template<class T,class RW> void OPENGL_COMPONENT_MAC_VELOCITY_FIELD_3D<T,RW>::
Decrease_Vector_Size()
{
    opengl_mac_velocity_field.Scale_Vector_Size(1/(T)1.1);
}
