        ...
        if(show_object_names){
            ...
            for(int i(1);i<=rigid_geometry_collection->particles.array_collection->Size();i++){
                if(draw_object(i) && rigid_geometry_collection->Rigid_Geometry(i).name.length()){
                    OpenGL_String(rigid_geometry_collection->particles.X(i),STRING_UTILITIES::string_sprintf("%s %f",rigid_geometry_collection->Rigid_Geometry(i).name.c_str(),rigid_geometry_collection->particles.V(i).x));}}}
#endif
        glPopAttrib();}
}
//#####################################################################
