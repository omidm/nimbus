    ...
    for(int i(1);i<=rigid_geometry_collection->particles.array_collection->Size();i++){
        if(draw_object(i)){
            ...
            if(opengl_segmented_curve(i)){
                if(output_positions){
                    opengl_segmented_curve(i)->Set_Name(STRING_UTILITIES::string_sprintf("%s <%.3f %.3f> [w=%.3f]",rigid_geometry_collection->Rigid_Geometry(i).name.c_str(),rigid_geometry_collection->particles.X(i).x,rigid_geometry_collection->particles.X(i).y,rigid_geometry_collection->particles.angular_velocity(i).x));}
