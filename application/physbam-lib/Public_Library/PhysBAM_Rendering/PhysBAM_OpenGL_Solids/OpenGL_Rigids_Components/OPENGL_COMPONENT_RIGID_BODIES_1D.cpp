        ...
        if(show_object_names){
            ...
            for(int i(1);i<=rigid_body_collection.rigid_body_particle.array_collection->Size();i++){
                if(draw_object(i) && rigid_body_collection.Rigid_Body(i).name.length()){
                    OpenGL_String(rigid_body_collection.rigid_body_particle.X(i),STRING_UTILITIES::string_sprintf("%s %f",rigid_body_collection.Rigid_Body(i).name.c_str(),rigid_body_collection.rigid_body_particle.V(i).x));}}}
        glPopAttrib();}
}
//#####################################################################
