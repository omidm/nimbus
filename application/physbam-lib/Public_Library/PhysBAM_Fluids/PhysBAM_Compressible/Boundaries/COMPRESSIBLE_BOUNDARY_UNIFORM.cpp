//#####################################################################
// Copyright 2002-2011, Mridul Aanajneya, Ronald Fedkiw, Geoffrey Irving, Frank Losasso, Andrew Selle, Tamar Shinar.
...
#include <PhysBAM_Fluids/PhysBAM_Compressible/Boundaries/COMPRESSIBLE_BOUNDARY_UNIFORM.h>
using namespace PhysBAM;
//#####################################################################
// Constructor
//#####################################################################
template<class T_GRID> COMPRESSIBLE_BOUNDARY_UNIFORM<T_GRID>::
COMPRESSIBLE_BOUNDARY_UNIFORM(EOS<T>* eos_input,VECTOR<EIGENSYSTEM<T,TV_DIMENSION>*,T_GRID::dimension>& eigensystem_input,T_MPI_GRID* mpi_grid_input,
    const T_FACE_VECTOR rho_far_field,const T_FACE_VECTOR p_far_field,const TV_FACE_VECTOR velocity_far_field,const T inflow_attenuation_input,
    const TV_SIDES& constant_extrapolation,const bool always_attenuate_input,const T_FACE_VECTOR linear_attenuations_input,
    const T_FACE_VECTOR_BOOL linear_attenuation_faces_input) : eigensystem(eigensystem_input)
{
    eos=eos_input;
    ...
    T gamma=dynamic_cast<EOS_GAMMA<T>*>(eos)->gamma;
    for(int side=1;side<=2*T_GRID::dimension;side++){
        T e_far_field=eos->e_From_p_And_rho(p_far_field(side),rho_far_field(side));
        T c_far_field=eos->c(rho_far_field(side),e_far_field);
        S_far_field(side)=eos->S(rho_far_field(side),e_far_field);
...
//#####################################################################
template<class T_GRID> void COMPRESSIBLE_BOUNDARY_UNIFORM<T_GRID>::
...
Attenuate_To_Far_Field_Values_Using_Characteristics(const T_ARRAYS_DIMENSION_BASE& u_ghost,const TV_INT& node_index,const int side,TV_DIMENSION &U,const T dt) const
{
    ...
    // apply L
    eigensystem[axis]->Eigenvectors(U_1d,node_index[axis],L,R);
    eigensystem[axis]->Eigenvalues(U_1d,node_index[axis],lambda,lambda_left,lambda_right);
...
Fill_Single_Ghost_Region(const T_GRID& grid,T_ARRAYS_DIMENSION_BASE& u_ghost,const RANGE<TV_INT>& region,const int side,const T dt,const T time,const int number_of_ghost_cells) const
{
    ...
    if(Constant_Extrapolation(side)){
        ...
        for(CELL_ITERATOR iterator(grid,region);iterator.Valid();iterator.Next()){TV_INT cell=iterator.Cell_Index();
            ...
            u_ghost(cell)=u_ghost(reflected_node);
            u_ghost(cell)(axis+1)*=-1;}}
}
//#####################################################################
...
Apply_Boundary_Condition_Single_Side(const T_GRID& grid,T_ARRAYS_DIMENSION_BASE& u,const int side,const T time) const 
{
    ...
    if(!mpi_grid||!mpi_grid->Neighbor(axis,axis_side)){
...
//#####################################################################
template class COMPRESSIBLE_BOUNDARY_UNIFORM<GRID<VECTOR<float,1> > >;
template class COMPRESSIBLE_BOUNDARY_UNIFORM<GRID<VECTOR<float,2> > >;
template class COMPRESSIBLE_BOUNDARY_UNIFORM<GRID<VECTOR<float,3> > >;
#ifndef COMPILE_WITHOUT_DOUBLE_SUPPORT
template class COMPRESSIBLE_BOUNDARY_UNIFORM<GRID<VECTOR<double,1> > >;
template class COMPRESSIBLE_BOUNDARY_UNIFORM<GRID<VECTOR<double,2> > >;
template class COMPRESSIBLE_BOUNDARY_UNIFORM<GRID<VECTOR<double,3> > >;
#endif
