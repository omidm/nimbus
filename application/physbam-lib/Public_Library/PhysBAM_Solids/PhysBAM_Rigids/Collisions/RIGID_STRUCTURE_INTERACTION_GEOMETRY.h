...
//#####################################################################
#ifndef __RIGID_STRUCTURE_INTERACTION_GEOMETRY__
#define __RIGID_STRUCTURE_INTERACTION_GEOMETRY__
...
public:
    ...
    ARRAY<TV> X,V,X_self_collision_free,V_self_collision_free;
private:
    bool need_destroy_segmented_curve;
    UNUSABLE unusable;
public:

    RIGID_STRUCTURE_INTERACTION_GEOMETRY(GEOMETRY_PARTICLES<TV>& full_particles_input)
...
//#####################################################################
    ...
    void Save_Current_State(const TV& X,const ROTATION<TV>& rotation,const TV& V);
    void Save_Self_Collision_Free_State(const TV& X,const ROTATION<TV>& rotation,const TV& V); // assumes mass does not change
    void Save_Self_Collision_Free_State(); // assumes mass does not change
    void Save_Pseudo_State(const T dt); // assumes mass does not change
