...
def GetPartitionIds(params, num):
    domain = params[0]
    pnum   = params[1]
    ghostw = params[2]
    ps_dj  = [0, 0, 0]
    ...
    for dim in range(0, 3):
        if domain[dim]/pnum[dim] < 2*ghostw[dim]:
            ...
            sys.exit(2)
        psl_dj[dim] = domain[dim]/pnum[dim]
        psl_wg[dim] = domain[dim]/pnum[dim] + 2*ghostw[dim]
        if domain[dim]%pnum[dim] == 0:
            ps_dj[dim] = psl_dj[dim]
        else:
            print "\nWarning: Partitions for dimension " + str(dim) + \
                    " at line " + str(num) + " are not of equal size."
            ps_dj[dim] = psl_dj[dim]+1
    psize_dj  = {0:[0]*pnum[0], 1:[0]*pnum[1], 2:[0]*pnum[2]}
    pstart_dj = {0:[0]*pnum[0], 1:[0]*pnum[1], 2:[0]*pnum[2]}
    for dim in range(0, 3):
        for i in range(0, pnum[dim], 1):
            psize_dj[dim][i] = ps[dim]
        psize_dj[dim][pnum[dim]-1] = psl[dim]
        pstart_dj[dim][0] = 0
        for i in range(1, pnum[dim], 1):
            pstart_dj[dim][i] = pstart_dj[dim][i-1] + psize_dj[dim][i-1]
    pidset = set()
...
print "\nReading partition configuration file " + reg_config_file + " ..."
